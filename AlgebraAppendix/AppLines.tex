...
\tlabel(1.5,3){\small $P\left(x_0, y_0\right)$}
\tlabel(3.5,1){\small $Q\left(x_1, y_1\right)$}
\end{mfpic}
...
\begin{eqn} \label{slope} The \index{slope ! definition} \textbf{slope} $m$ of the line containing the points $P\left(x_0, y_0\right)$ and $Q\left(x_1, y_1\right)$ is: \index{line ! slope of} \index{slope ! of a line}

\[ m  = \dfrac{y_1 - y_0}{x_1 - x_0} = \dfrac{\Delta y}{\Delta x},\]
...
\tlabel[t](-1.5,1.5){$\left(x_0, y_0\right)$}
...
\begin{eqn} \label{pointslope} The\index{point-slope form of a line}\index{line ! point-slope form} \textbf{point-slope form} of the line with slope $m$ containing the point $\left(x_0, y_0\right)$ is the equation \[y - y_0  =  m\left(x - x_0\right)\] $ $ \vspace{-.2in}
\end{eqn}
...
\textbf{NOTE:} $m_1 m_2 = -1$  is equivalent to $m_2 = -\dfrac{1}{m_1}$, so that perpendicular lines have slopes which are `opposite reciprocals' of one another.
...
\tlabel[cc](5,-0.5){\scriptsize $x$}
\tlabel[cc](0.5,5){\scriptsize $y$}
\tlabel[cc](4.5,4){\scriptsize $L_1$}
\tlabel[cc](3.5,5){\scriptsize $L_2$}
\arrow \reverse \arrow \polyline{(-1,-1), (4,4)}
\arrow \reverse \arrow \polyline{(-2,0), (3,5)} 
\tlpointsep{4pt}
\tcaption{$L_1 \parallel L_2$, $m_1 = m_2$}
\axes
...
\tlpointsep{4pt}
\axes
\tcaption{$L_1 \perp L_2$, $m_1 m_2 = -1$}
