...
\begin{document}
	\author{Stitz-Zeager}
	\xmtitle{Absolute Value Equations and Inequalities}{}
...
% Labels
\node at (-2.5,1.35) {\scriptsize distance is $5$ units};
\node at ( 2.5,1.35) {\scriptsize distance is $5$ units};
...
% Label
\node at (0,1.35) {\scriptsize distance is $2$ units};
...
% Labels
\node at (-2,1.35) {\scriptsize $3$ units};
\node at ( 2,1.35) {\scriptsize $3$ units};

\end{tikzpicture}

The solutions to $|x| = 3$ are $x=\pm3$.
...
% Solution interval
\draw[line width=1.5pt] (-4,0) -- (4,0);

% Open endpoints
\draw[line width=1.2pt] (-4,0) circle (2pt);
\draw[line width=1.2pt] (4,0) circle (2pt);
...
% Rays
\draw[->, line width=1.5pt] (-4,0) -- (-6.2,0);
\draw[->, line width=1.5pt] (4,0) -- (6.2,0);
...
|2x+1| & \leq & \dfrac{-4-4\sqrt{3}}{-2} & \text{Divide by $-2$,  reverse the inequality} \\  
...
\left| \dfrac{8-4x}{3} \right| & < & \varepsilon & \\  

\dfrac{|8 - 4x|}{|3|} & < & \varepsilon & \text{Quotient Rule}\\  

\dfrac{|-4(x-2)|}{3} & < & \varepsilon & \text{Factor} \\  

\dfrac{|-4| |x-2|}{3} & < & \varepsilon & \text{Product Rule} \\  

\dfrac{4 |x-2|}{3} & < & \varepsilon & \\  

\dfrac{3}{4} \cdot \dfrac{4 |x-2|}{3} & < & \dfrac{3}{4} \cdot \varepsilon & \text{Multiply by $\dfrac{3}{4}$} \\  
