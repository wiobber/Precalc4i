...
%% \colorbox{ResultColor}{\bbm
 ...
 \begin{definition} \label{ellipsedefn} Given two distinct points $F_1$ and $F_2$ in the plane and a fixed distance $d$, an \index{ellipse ! definition of}  \textbf{ellipse} is the set of all points $(x, y)$ in the plane such that the sum of each of the distances from $F_1$ and $F_2$ to $(x, y)$ is $d$.  The points $F_1$ and $F_2$ are called the \index{ellipse ! foci} \index{focus (foci) ! of an ellipse} \textbf{foci}\footnote{the plural of `focus'} of the ellipse.
...
% https://www.geogebra.org/classic/bcpfyra3
\begin{center}
\geogebra{bcpfyra3}{800}{600}
\end{center}
...
% --- Parameters ---
\def\a{3.6} % semi-major axis
\def\b{2.6} % semi-minor axis
\pgfmathsetmacro{\c}{sqrt(\a*\a-\b*\b)} % distance from center to focus

% --- Ellipse ---
\draw[very thick] (0,0) ellipse [x radius=\a, y radius=\b];

% --- Dotted axis lines ---
\draw[dotted, thick] (-\a-0.4,0) -- (\a+0.4,0);
\draw[dotted, thick] (0,-\b-0.4) -- (0,\b+0.4);

% --- Center ---
\node at (0,0) {$\ast$};
\node[below] at (0,-0.25) {$C$};

% --- Vertices ---
\fill (-\a,0) circle (2.5pt);
\fill (\a,0) circle (2.5pt);
\node[left] at (-\a-0.1,0) {$V_1$};
\node[right] at (\a+0.1,0) {$V_2$};

% --- Foci ---
\node at (-\c,0) {$\ast$};
\node at (\c,0) {$\ast$};
\node[below] at (-\c,-0.25) {$F_1$};
\node[below] at (\c,-0.25) {$F_2$};

% --- Axis labels ---
\node[rotate=90] at (-0.25,1.7) {Minor Axis};
\node at (1.6,0.3) {Major Axis};

\end{tikzpicture}
\end{image}

\begin{center}
 \captionof{figure}{An ellipse with center $C$; foci $F_1$, $F_2$; and vertices $V_1$, $V_2$}
\end{center} 
...
% --- Parameters ---
\def\a{4.0} % semi-major axis
\def\b{3.0} % semi-minor axis
\pgfmathsetmacro{\c}{sqrt(\a*\a-\b*\b)} % distance from center to focus
\def\angle{50} % angle for point (x,y) on the ellipse

% --- Axes ---
\draw[->] (-\a-0.8,0) -- (\a+0.9,0) node[right] {$x$};
\draw[->] (0,-\b-0.8) -- (0,\b+0.9) node[above] {$y$};

% --- Ellipse ---
\draw[very thick] (0,0) ellipse [x radius=\a, y radius=\b];

% --- Origin, marked with a small cross ---
\node at (0,0) {$\times$};

% --- Vertices on x-axis ---
\fill (-\a,0) circle (2.5pt);
\fill (\a,0) circle (2.5pt);
\node[below left] at (-\a,-0.05) {$(-a,0)$};
\node[below right] at (\a,-0.05) {$(a,0)$};

% --- Vertices on y-axis ---
\fill (0,\b) circle (2.5pt);
\fill (0,-\b) circle (2.5pt);
\node[above right] at (0,\b) {$(0,b)$};
\node[below right] at (0,-\b) {$(0,-b)$};

% --- Foci ---
\node at (-\c,0) {$\ast$};
\node at (\c,0) {$\ast$};
\node[below] at (-\c,-0.25) {$(-c,0)$};
\node[below] at (\c,-0.25) {$(c,0)$};
...
\node[right] at (\px+0.1,\py) {$(x,y)$};

% --- Dashed segments from each focus to (x,y) ---
\draw[dashed] (-\c,0) -- (\px,\py);
\draw[dashed] (\c,0) -- (\px,\py);
...
\begin{formula}  \label{standardellipse} \index{ellipse ! standard equation} \textbf{The Standard Equation of an Ellipse:}  
...
% --- Ellipse ---
\draw[very thick, blue] (\cx,\cy) ellipse [x radius=\rx, y radius=\ry];

% --- Key points ---
\fill (0,0) circle (2.5pt);
\node[above left] at (-0.1,0.15) {$(0,0)$};

\fill (6,0) circle (2.5pt);
\node[above right] at (6.1,0.15) {$(6,0)$};

\fill (1,3) circle (2.5pt);
\node[right] at (1.15,3) {$(1,3)$};
...
\[  25 (x+1)^2 + 9 (y-2)^2 = 225 \rightarrow \frac{(x-(-1))^2}{9}  + \frac{(y-2)^2}{25} = 1 \leftrightarrow  \frac{(x-(-1))^2}{(3)^2}  + \frac{(y-2)^2}{(5)^2} = 1.  \]
...
As an aid to sketching, we draw a rectangle matching this description, called a \index{ellipse ! guide rectangle}\index{guide rectangle ! for an ellipse}\textbf{guide rectangle}, and sketch the ellipse inside this rectangle as seen below.
...
% --- Axes ---
\draw[->] (-5.1,0) -- (2.6,0) node[right] {$x$};
\draw[->] (0,-2.9) -- (0,7.7) node[above] {$y$};
...
\draw (2,-0.08) -- (2,0.08); % unlabeled tick at vertex x = 2
...
% --- Dotted bounding box ---
\draw[dotted, thick] (\cx-\rx,\cy-\ry) rectangle (\cx+\rx,\cy+\ry);

% --- Ellipse ---
\draw[very thick, blue] (\cx,\cy) ellipse [x radius=\rx, y radius=\ry];

% --- Center, marked with a small cross ---
\node at (\cx,\cy) {$\times$};

% --- Vertices ---
\fill (\cx,\cy+\ry) circle (2.5pt);
\node[above,black] at (\cx,\cy+\ry+0.05) {$(-1,7)$};

\fill (\cx,\cy-\ry) circle (2.5pt);
\node[below, black] at (\cx,\cy-\ry-0.05) {$(-1,-3)$};

\fill (\cx-\rx,\cy) circle (2.5pt);
\node[left, black] at (\cx-\rx-0.1,\cy) {$(-4,2)$};

\fill (\cx+\rx,\cy) circle (2.5pt);
\node[right, black] at (\cx+\rx+0.1,\cy) {$(2,2)$};
...
To find the foci, we find $c = \sqrt{25-9} = \sqrt{16} = 4$, which means the foci lie $4$ units from the center. Since the major axis is vertical, the foci lie $4$ units above and below the center, at $(-1, 2-4) = (-1,-2)$ and $(-1, 2+4) = (-1,6)$.  Our final graph appears below.
...
\def\cx{-1}  % center x
\def\cy{2}   % center y
\def\rx{3}   % x radius (minor semi-axis)
\def\ry{5}   % y radius (major semi-axis)
\pgfmathsetmacro{\c}{sqrt(\ry*\ry-\rx*\rx)} % distance from center to focus
...
% --- Dotted major/minor axis lines through the center ---
\draw[dotted, thick] (\cx-\rx,\cy) -- (\cx+\rx,\cy);
\draw[dotted, thick] (\cx,\cy-\ry) -- (\cx,\cy+\ry);
...
% --- Foci, on the major (vertical) axis ---
\node at (\cx,\cy+\c) {$\ast$};
\node at (\cx,\cy-\c) {$\ast$};

% --- Vertices ---
\fill (\cx,\cy+\ry) circle (2.5pt);
\node[above] at (\cx,\cy+\ry+0.05) {$(-1,7)$};

\fill (\cx,\cy-\ry) circle (2.5pt);
\node[below] at (\cx,\cy-\ry-0.05) {$(-1,-3)$};

\fill (\cx-\rx,\cy) circle (2.5pt);
\node[left] at (\cx-\rx-0.1,\cy) {$(-4,2)$};

\fill (\cx+\rx,\cy) circle (2.5pt);
\node[right] at (\cx+\rx+0.1,\cy) {$(2,2)$};
...
(x-1)^2 + 4(y+3)^2  &  = & 4 & \text{factor}  \\   

\frac{(x-1)^2 + 4(y+3)^2}{4}  &  = & \frac{4}{4} & \text{divide through by $4$} \\ 

 \frac{(x-1)^2}{4} + (y+3)^2 & = & 1 & \\  
 
 \frac{(x-1)^2}{(2)^2} + \frac{(y-(-3))^2}{(1)^2} & = & 1 &  \text{Rewrite in the desired form.} \\  \end{array} \]
...
Now that this equation is in the standard form of Equation \ref{standardellipse}, identify $h = 1$ and  $k = -3$ so our ellipse is centered at $(1,-3)$.  With $a=2$,  we move $2$ units left and right from the center to get two points on the ellipse:  $(1-2,-3) = (-1,-3)$ and $(1+2, -3) = (3,-3)$.  Since $b=1$, we move $1$ unit up and down from the center to obtain two additional points on the ellipse:  $(1,-3+1) = (1,-2)$ and $(1,-3-1) = (1,-4)$.  
 ...
 To find the foci, we find $c = \sqrt{4-1} = \sqrt{3}$, which means the foci lie $\sqrt{3}$ units from the center. Since the major axis is horizontal, the foci lie $\sqrt{3}$ units to the left and right of the center, at $(1-\sqrt{3},-3)$ and $(1+\sqrt{3},-3)$.  Plotting all of this information gives the graph below.
...
\def\cx{1}   % center x
\def\cy{-3}  % center y
\def\rx{2}   % x radius (major semi-axis)
\def\ry{1}   % y radius (minor semi-axis)
\pgfmathsetmacro{\c}{sqrt(\rx*\rx-\ry*\ry)} % distance from center to focus

% --- Axes ---
\draw[->] (-1.6,0) -- (4.9,0) node[right] {$x$};
\draw[->] (0,-4.9) -- (0,0.9) node[above] {$y$};
...
% --- Foci, on the major (horizontal) axis ---
\node at (\cx-\c,\cy) {$\ast$};
\node at (\cx+\c,\cy) {$\ast$};

% --- Vertices ---
\fill (\cx,\cy+\ry) circle (2.5pt);
\node[above] at (\cx,\cy+\ry+0.05) {$(1,-2)$};

\fill (\cx,\cy-\ry) circle (2.5pt);
\node[below] at (\cx,\cy-\ry-0.05) {$(1,-4)$};

\fill (\cx-\rx,\cy) circle (2.5pt);
\node[left] at (\cx-\rx-0.1,\cy) {$(-1,-3)$};

\fill (\cx+\rx,\cy) circle (2.5pt);
\node[right] at (\cx+\rx+0.1,\cy) {$(3,-3)$};
...
4(x+2)^2 + (y-1)^2  &  = & 4 & \text{factor}  \\   

\frac{4(x+2)^2 + (y-1)^2}{4}  &  = & \frac{4}{4} & \text{divide through by $4$} \\ 

(x+2)^2 + \frac{(y-1)^2}{4} & = & 1 & \\  
 
 \frac{(x-(-2))^2}{(1)^2} + \frac{(y-1)^2}{(2)^2} & = & 1 &  \text{Rewrite in the desired form.} \\  \end{array} \]
...
% --- Axes ---
\draw[->] (-3.6,0) -- (1.9,0) node[right] {$x$};
\draw[->] (0,-1.9) -- (0,3.9) node[above] {$y$};
...
% --- Ellipse: solid upper half, dotted lower half ---
\draw[very thick, blue] (\cx+\rx,\cy) arc [start angle=0, end angle=180, x radius=\rx, y radius=\ry];
\draw[dotted, thick] (\cx-\rx,\cy) arc [start angle=180, end angle=360, x radius=\rx, y radius=\ry];

% --- Key points ---
\fill (\cx,\cy+\ry) circle (2.5pt);
\node[above] at (\cx,\cy+\ry+0.05) {$(-2,3)$};

\fill (\cx-\rx,\cy) circle (2.5pt);
\node[left] at (\cx-\rx-0.1,\cy) {$(-3,1)$};

\fill (\cx+\rx,\cy) circle (2.5pt);
\node[left] at (\cx+\rx-0.1,\cy) {$(-1,1)$};
...
\item  We plot the information given to us below and notice immediately that the major axis is horizontal, which means $a > b$.   
...
\def\cx{3}   % center x
\def\cy{1}  % center y
\def\rx{4}   % x radius (major semi-axis)
\def\ry{1}   % y radius (minor semi-axis)
\pgfmathsetmacro{\c}{sqrt(\rx*\rx-\ry*\ry)} % distance from center to focus

% --- Axes ---
\draw[->] (-1.6,0) -- (6,0) node[right] {$x$};
...
Since the center is the midpoint of the foci, we know the center of the ellipse is  $(3, 1)$ which means  $h = 3$ and $k = 1$. Since one vertex is at $(0,1)$, which is $3$ units from the center,  we have that $a = 3$, so $a^2 =  9$.   At this point, all that remains is to find $b^2$. 

Since the foci, $(2,1)$ and $(4,1)$,  are $1$ unit away from the center, we have $c=1$.  Putting this together with the fact that  $a > b$, we get $c = \sqrt{a^2-b^2}$, or $1 = \sqrt{9-b^2}$.  Squaring both sides gives $1 = 9-b^2$ or   $b^2 = 8$.  Feeding all of this data into Equation \ref{standardellipse}, we get our final answer:  \[\frac{(x-3)^2}{9}+\frac{(y-1)^2}{8}=1.\]
...
While it certainly \textit{appears} that the vertices are $(3, \pm 4)$, in which case we'd have $b = 4$, these points aren't labeled.  Instead, we use the labeled point $(1,3)$ to calculate $b^2$.  At this stage, we know the equation of the ellipse is \[ \frac{(x-3)^2}{9} + \frac{y^2}{b^2} = 1,\] so upon substituting $x=1$ and $y=3$, we obtain $\frac{4}{9} + \frac{9}{b^2} = 1$.  Solving this equation, we get $b^2 = \frac{81}{5}$.  Hence, our final answer is:

 \[\frac{(x-3)^2}{9}+\frac{5(y-1)^2}{81}=1.\]

Note the vertices of the ellipse are $\left(3, \pm \sqrt{\frac{81}{5}} \right) \approx (3, \pm 4)$ but they are not \textit{exactly} $(3, \pm 4)$.  
...
% https://www.geogebra.org/classic/d6hg4zbs
\begin{center}
\geogebra{d6hg4zbs}{800}{600}
\end{center}
...
\begin{explanation}  Per Kepler's Laws, the orbit of Mercury is an ellipse with the Sun at one focus.  Since we are told to assume the Sun is positioned at $(0,0)$, we are free to choose if the major axis of the ellipse lies on the $x$- or $y$-axis.  We choose the former.
...
% --- Vertices ---
\fill (4.34,0) circle (2.5pt);
\fill (-2.86,0) circle (2.5pt);
\node[above] at (4.34,0.1) {$(43.4,0)$};
\node[above] at (-2.86,0.1) {$(-28.6,0)$};
...
\[   e = \frac{\mbox{distance from the center to a focus}}{\mbox{distance from the center to a vertex}}  = \frac{7.4}{36} \approx 0.205. \]

To our delight, we find answer agrees with  \href{http://nssdc.gsfc.nasa.gov/planetary/factsheet/planet_table_british.html}{\underline{NASA}}. With an eccentricity of $0.205$, we expect Mercury's orbit to be fairly `round.'   In Exercise \ref{MercuryOrbitGraph}, we invite the reader to find the equation of Mercury's orbit to confirm this conclusion graphically.  
\end{explanation}
\end{example}
...
% --- Ellipse ---
\draw[very thick](\a,0)arc [start angle=0, end angle=180, x radius=\a, y radius=\b];
...
\begin{explanation} Ultimately, we are looking for information about the relative position of foci and the vertices, so we assume the whispering gallery can be represented by the upper half of an ellipse centered at the origin with its major axis along the $x$-axis. Graphing the data yields the schematic below.
