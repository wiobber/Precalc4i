...
\begin{definition} \label{hyperboladefn} Given two distinct points $F_1$ and $F_2$ in the plane and a fixed distance $d$, a \index{hyperbola ! definition of}  \textbf{hyperbola} is the set of all points $(x, y)$ in the plane such that the absolute value of the difference of each of the distances from $F_1$ and $F_2$  to $(x, y)$ is $d$. The points $F_1$ and $F_2$ are called the \index{hyperbola ! foci} \index{focus (foci) ! of a hyperbola}\textbf{foci} of the hyperbola.
...
% https://www.geogebra.org/classic/d7m5ra6d

\begin{center}
\geogebra{d7m5ra6d}{800}{600}
\end{center}
...
% --- Parameters ---
\def\a{1.4}    % distance center to vertex
\def\b{2.6}    % controls asymptote slope (rise over run = b/a)
\pgfmathsetmacro{\c}{sqrt(\a*\a+\b*\b)} % distance center to focus
\def\xext{2} % how far the asymptotes extend horizontally

% --- Asymptotes: y = (b/a)x and y = -(b/a)x, small arrowheads at both ends ---
\draw[dashed]
    (-\xext,{-\b/\a*\xext}) -- (\xext,{\b/\a*\xext});
\draw[dashed]
    (-\xext,{\b/\a*\xext}) -- (\xext,{-\b/\a*\xext});

% --- Hyperbola branches: x = ±a*cosh(t), y = b*sinh(t) ---
\def\tmax{1.05}
\draw[very thick, domain=-\tmax:\tmax, samples=60, smooth, variable=\t]
    plot ({\a*cosh(\t)}, {\b*sinh(\t)});
\draw[very thick, domain=-\tmax:\tmax, samples=60, smooth, variable=\t]
    plot ({-\a*cosh(\t)}, {\b*sinh(\t)});

% --- Transverse axis (dotted, through the vertices) ---
\draw[dotted, thick] (-\a-0.2,0) -- (\a+0.2,0);
\node at (0,0.55) {Transverse Axis};
...
\fill (\a,0) circle (2.5pt);
\node[left] at (-\a,-0.25) {$V_1$};
\node[right] at (\a,-0.25) {$V_2$};

% --- Foci ---
\node at (-\c,0) {$\ast$};
\node at (\c,0) {$\ast$};
\node[above] at (-\c,0.15) {$F_1$};
\node[above] at (\c,0.15) {$F_2$};
...
\begin{center}
\captionof{figure}{A hyperbola with center $C$; foci $F_1$, $F_2$; and vertices $V_1$, $V_2$ and asymptotes (dashed)}
...
% --- Dotted auxiliary rectangle, corners at (±a, ±b) ---
\draw[dotted, thick] (-\a,-\b) rectangle (\a,\b);
...
% white rectangle
\fill[white](-0.5,-2)--(-0.5,2)--(-0.1,2)--(-0.1,-2)--cycle;

% --- Conjugate axis (dotted, vertical through the center) ---
\draw[dotted, thick] (0,-\b) -- (0,\b);
\node[rotate=90] at (-0.3,0) {Conjugate Axis};


% --- Center ---
\node at (0,0) {$\times$};
\node[right] at (0.15,-0.25) {$C$};

% --- Vertices ---
\fill (-\a,0) circle (2.5pt);
\fill (\a,0) circle (2.5pt);
\node[left] at (-\a-0.15,0) {$V_1$};
\node[right] at (\a+0.15,0) {$V_2$};
...
% https://www.geogebra.org/classic/fyff2hg2
\begin{center}
\geogebra{fyff2hg2}{800}{600}
\end{center}
...
\frac{y^2}{x^2} & = &  \frac{\left(c^2 - a^2\right)}{a^2} -  \frac{\left(c^2 - a^2\right)}{x^2}& \\ \end{array}\]
...
\frac{x^2}{a^2} - \frac{y^2}{b^2} & = & 1 & \end{array} \]
...
\begin{formula} \label{standardhhyperbola} \index{hyperbola ! standard equation ! horizontal} \textbf{The Standard Equation of a Horizontal\footnote{That is, a hyperbola whose branches open to the left and right} Hyperbola}  
...
\begin{formula}  \label{standardvhyperbola} \index{hyperbola ! standard equation ! vertical} \textbf{The Standard Equation of a Vertical Hyperbola}  
...
% --- Parameters: (y-k)^2/a^2 - (x-h)^2/b^2 = 1 ---
\def\h{3}
\def\k{-1}
\def\a{2}
\pgfmathsetmacro{\bb}{sqrt(3)} % b = sqrt(3)

% --- Axes ---
\draw[->] (-1.1,0) -- (7.3,0) node[right] {$x$};
\draw[->] (0,-5.9) -- (0,4.9) node[above] {$y$};
...
% --- Parameter range: x = h + b*sinh(t), extend a bit past x=0 and x=6 ---
\pgfmathsetmacro{\tmax}{ln(sqrt(3)+2)*1.08} % slightly past the frame edges

% --- Upper branch: y = k + a*cosh(t), x = h + b*sinh(t) ---
\draw[very thick,blue, domain=-\tmax:\tmax, samples=80, smooth, variable=\t]
    plot ({\h + \bb*sinh(\t)}, {\k + \a*cosh(\t)});

% --- Lower branch: y = k - a*cosh(t), x = h + b*sinh(t) ---
\draw[very thick,blue, domain=-\tmax:\tmax, samples=80, smooth, variable=\t]
    plot ({\h + \bb*sinh(\t)}, {\k - \a*cosh(\t)});

% --- Key points ---
\fill (0,3) circle (2.5pt);
%\node[left] at (-0.15,3) {$(0,3)$};

\fill (3,1) circle (2.5pt);
\node[below] at (3,0.8) {$(3,1)$};

\fill (0,-5) circle (2.5pt);
%\node[left] at (-0.15,-5) {$(0,-5)$};

\fill (3,-3) circle (2.5pt);
\node[above] at (3,-2.8) {$(3,-3)$};
...
\[ 25(x-2)^2 - 4y^2 = 100 \rightarrow \frac{(x-2)^2}{4} - \frac{(y-0)^2}{25} = 1 \leftrightarrow \frac{(x-2)^2}{(2)^2} - \frac{(y-0)^2}{(5)^2} = 1. \]
...
% https://www.desmos.com/calculator/drpno6desm
\begin{center}
    \desmos{drpno6desm}{800}{600}
\end{center}
...
Lastly, to determine the equations of the asymptotes, recall that the asymptotes pass through the center of the hyperbola, $(2,0)$, as well as the corners of guide rectangle.  As such,  they have slopes of $\pm \frac{b}{a} = \pm \frac{5}{2}$.  Feeding this information into  the point-slope equation of a line, Equation \ref{pointslope}, we get $y -0 = \pm \frac{5}{2} (x - 2)$, so the asymptotes are  $y = \frac{5}{2}x - 5$ and $y = -\frac{5}{2}x + 5$.  Putting it all together, we get our final graph below.

% https://www.desmos.com/calculator/r0duejrdpw
\begin{center}
\desmos{r0duejrdpw}{800}{600}
\end{center}
...
\[ \begin{array}{rclr} 9y^2-x^2-6x & = & 10 & \\ 
9y^2-1\left(x^2+6x\right) &  = & 10 &  \text{factor out leading coefficient of $x^2$}  \\ 
9y^2-1 \left(x^2+6x + \underline{9}\right) &  = & 10 + (-1)\left( \underline{9} \right) & \text{complete the square in $x$}  \\ 
9y^2-(x+3)^2 &  = & 1 & \text{factor}   \\  
\frac{y^2}{\frac{1}{9}} - \frac{(x+3)^2}{1} & = & 1 & \text{rewrite} \\  
\frac{(y-0)^2}{\left(\frac{1}{3}\right)^2} - \frac{(x-(-3))^2}{(1)^2} & = & 1 & \text{Write in the desired form.}  \\
 \end{array} \]
...
\smallskip
 ...
 % https://www.desmos.com/calculator/yrsh5icoxx
\begin{center}
\desmos{yrsh5icoxx}{800}{600} 
\end{center}
 
 \end{enumerate}

\item Graphing $f(x) = \sqrt{x^2-2x-3}$ amounts to graphing the equation $y= \sqrt{x^2-2x-3}$. In order to use the tools we've learned in this chapter, we first square both sides to get a quadratic equation in two variables:  $y^2 = (\sqrt{x^2-2x-3})^2$.  We get $y^2 = x^2-2x-3$ or $y^2 - x^2 + 2x = -3$.  We now set about transforming this equation into the form stated in  Equation \ref{standardhhyperbola} or  Equation \ref{standardvhyperbola}.

\[ \begin{array}{rclr} y^2 - x^2 + 2x & = & -3 & \\ 
y^2-1\left(x^2- 2x\right) &  = & -3  &  \text{factor out leading coefficient of $x^2$}  \\ 
y^2-1 \left(x^2-2x + \underline{1}\right) &  = & -3 + (-1)\left( \underline{1} \right) & \text{complete the square in $x$}  \\ 
y^2-(x-1)^2 &  = & -4 & \text{factor}   \\  
 - \frac{y^2}{4} + \frac{(x-1)^2}{4} & = & 1 & \text{divide through by $-4$}  \\  
\frac{(x-1)^2}{(2)^2} - \frac{(y-0)^2}{(2)^2} & = & 1 & \text{Write in the desired form.}  \\
 \end{array} \]
...
We know  since $f$ is a function, the graph of $f$ cannot be the \textit{entire} hyperbola, otherwise  the graph would  fail the vertical line test.  Since, by definition, $\sqrt{x^2-2x-3} \geq 0$,  we know $f(x) \geq 0$.  Hence the graph of $f$ is  the \textit{upper} half of the hyperbola, as shown below. 


% https://www.desmos.com/calculator/iyo8aspbyf
\begin{center}
\desmos{iyo8aspbyf}{800}{600}
\end{center}
...
\foreach \x in {-5,-4,...,5}
{
    \draw[line width=1pt] (\x,-0.18) -- (\x,0.18);
}

% Tick marks on y-axis
\foreach \y in {-5,-4,...,5}
{
    \draw[line width=1pt] (-0.18,\y) -- (0.18,\y);
}

% Dashed diagonal lines
\draw[dashed, line width=1.2pt, dash pattern=on 8pt off 7pt, <->]
    (-3.1,-5.8) -- (3.1,5.8);

\draw[dashed, line width=1.2pt, dash pattern=on 8pt off 7pt, <->]
    (-3.1,5.8) -- (3.1,-5.8);

% Filled points at (-5,0) and (5,0)
\fill (-5,0) circle (4.5pt);
\fill (5,0) circle (4.5pt);

% Axis labels
\node[below=5pt] at (-5,0) {$-5$};
\node[below=5pt] at (5,0) {$5$};

\node[left=5pt] at (0,5) {$5$};
\node[left=5pt] at (0,-5) {$-5$};
...
At this stage, we know the equation of the hyperbola is \[ \frac{(y+1)^2}{4} - \frac{(x-3)^2}{a^2} = 1,\] so substituting $x=0$ and $y=3$ into this equation, we get $\frac{16}{4} - \frac{9}{a^2} = 1$ so $a^2 = 3$.  Hence, our final answer is  \[ \frac{(y+1)^2}{4} - \frac{(x-3)^2}{3} = 1.\] We leave it to the reader to check. \qed
...
% https://www.geogebra.org/classic/nnwjq4mb
\begin{center}
\geogebra{nnwjq4mb}{800}{600}
\end{center}
...
\item  Kai and Jeff are at the foci of a second hyperbola where the fixed distance $d$ is:  \[760 \, \frac{\mbox{miles}}{\mbox{hour}} \times \frac{ 1 \, \mbox{hour}}{ 3600 \, \mbox{seconds}} \times 18 \, \mbox{seconds}  = 3.8 \, \mbox{miles}\]  
...
Kai heard the Sasquatch call after Jeff, so Kai is farther from Sasquatch than Jeff.  Thus Sasquatch must lie on the southern branch of the hyperbola \[ \frac{(y-3)^2}{3.61} - \frac{(x+5)^2}{5.39} = 1.\] 

Using GeoGebra, we find exactly one point which lies both on the western branch of the hyperbola determined by Jeff and Carl along with the southern branch of the hyperbola determined by Kai and Jeff.  We find\footnote{To determine the coordinates of this point of intersection exactly, we would need techniques for solving systems of non-linear equations (which we won't see until Section \ref{NonLinearEquations}), so we use a graphing utility, in this case, GeoGebra.} Sasquatch was at approximately at $(-0.9629, -0.8113)$ when it called.

% https://www.geogebra.org/classic/sjsdyuqz
\begin{center}
\geogebra{sjsdyuqz}{800}{600}
\end{center}

Note that at this point, the Sasquatch is approximately $6.02$ miles from Carl, $4.12$ miles from Jeff, and $7.92$ miles from Kai.  We check that $|6.02 - 4.12| = 1.9$ and $|4.12 - 7.92| = 3.8$, proving the point $(-0.9629, -0.8113)$ is indeed on both hyperbolas. \qed   
