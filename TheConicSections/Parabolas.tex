...
% https://www.geogebra.org/classic/swpj4meu
\begin{center}
\geogebra{swpj4meu}{800}{600}
\end{center}

  The point suggestively labeled $V$ is, as you may expect, the \textbf{vertex}.  The \index{parabola ! vertex} \index{vertex ! of a parabola} vertex is the point on the parabola closest to the focus.  
...
\def\p{0.6}
\def\xpt{2.1}
\pgfmathsetmacro{\ypt}{\xpt*\xpt/(4*\p)}

% --- Axes ---
\draw[->] (-3.2, 0) -- (3.4, 0) node[right] {$x$};
\draw[->] (0, 0) -- (0, 3.6) node[above] {$y$};

% --- Directrix (dashed horizontal line at y = -p) ---
\draw[-] (-2.9, -\p) -- (2.9, -\p);
\node at (0, -1.0) {$y = -p$};

% --- Parabola ---
\draw[thick, domain=-2.55:0, samples=60, smooth, variable=\t]
    plot (\t, {\t*\t/(4*\p)});
\draw[thick, domain=0:2.55, samples=60, smooth, variable=\t]
    plot (\t, {\t*\t/(4*\p)});
...
% Focus
\node at (0,\p) {\Large $\ast$};
\node[left] at (0,\p) {$(0,p)$};

% Point on parabola
\fill (\xpt,\ypt) circle (2pt);
\node[right] at (\xpt,\ypt) {$(x,y)$};

% Point on directrix below (x,y)
\fill (\xpt,-\p) circle (2pt);
\node[below right] at (\xpt,-\p) {$(x,-p)$};

% --- Dashed segments ---
% Focus to (x,y)
\draw[dashed] (0,\p) -- (\xpt,\ypt);
% Vertical drop from (x,y) to (x,-p)
\draw[dashed] (\xpt,\ypt) -- (\xpt,-\p);
...
\begin{formula}  \label{standardvparabola} \index{parabola ! standard equation ! vertical}  \textbf{The Standard Equation of a Vertical\footnote{That is, a parabola which opens either upwards or downwards.}  Parabola in the $xy$-plane:}  
...
%% Extra thick graph
\addplot[smooth, line width=3.5pt, penColor1, domain] {-0.5*(x+1)^2+2};
...
\begin{formula}  \label{standardhparabola} \index{parabola ! standard equation ! horizontal} \textbf{The Standard Equation of a Horizontal Parabola:}  
...
%% Extra thick graph
\addplot[smooth, line width=3.5pt, penColor1, samples=200] {(3/1.41)*(x+2)^0.5+3};
\addplot[smooth, line width=3.5pt, penColor1, samples=200] {-(3/1.41)*(x+2)^0.5+3};
...
Since the focal diameter is $|4p|$ is $8$, the parabola is $8$ units wide at the focus.  Starting at the focus $(-1,-2)$ we move down $4$ units and get $(-1,-2-4) = (-1,-6)$.  Moving up $4$ units from the focus we get $(-1,-2+4) = (-1,2)$.  Hence, $(-1,-6)$ and $(-1,2)$ are the endpoints of the latus rectum. The final graph is below.


% https://www.desmos.com/calculator/2fh7y50igq
\begin{center}
\desmos{2fh7y50igq}{800}{600}
\end{center}
...
To use the quadratic formula, we need to set the equation to $0$:  $y^2 + 4y + 8x-4 = 0$.  Since we are solving for $y$, we identify $a = 1$, $b=4$ and $c = 8x-4$.  We find the discriminant $b^2-4ac = (4)^2 - 4(1)(8x-4) = 32-32x$ so \[ y = \frac{-4 \pm \sqrt{32-32x}}{2} = \frac{-4 \pm \sqrt{32(1-x)}}{2}  = \frac{-4 \pm 4\sqrt{2(1-x)}}{2} = -2 \pm 2 \sqrt{2-2x}. \]
...
% https://www.desmos.com/3d/ua0nafkkly
% \begin{center}
%  \desmosThreeD{ua0nafkkly}{600}
%  \end{center}

% https://www.geogebra.org/3d/xrvhewz7
\begin{center}
  \geogebra{xrvhewz7}{800}{600}  
\end{center}
...
% https://www.geogebra.org/classic/kqyxjura

\begin{center}
\geogebra{kqyxjura}{800}{600}
\end{center}
...
% --- Parameters ---
% Parabola: y = x^2/(4p), focus at (0,p)
\def\p{2}
\def\xmax{6}
\pgfmathsetmacro{\ymax}{\xmax*\xmax/(4*\p)} % y-value at x = 6

% --- Axes ---
\draw[->] (-7.2, 0) -- (7.2, 0) node[right] {$x$};
\draw[->] (0, -0.6) -- (0, 6.2) node[above] {$y$};
...
% --- Parabola, extending past the marked points with arrows ---
\draw[thick, domain=-6.6:0, samples=60, smooth, variable=\t,
      {Latex[length=3mm]}-]
    plot (\t, {\t*\t/(4*\p)});
\draw[thick, domain=0:6.6, samples=60, smooth, variable=\t,
      -{Latex[length=3mm]}]
    plot (\t, {\t*\t/(4*\p)});
...
% Focus, marked with asterisk, label "2" to its left
\node at (0,\p) {\Large $\ast$};
\node[left] at (-0.15,\p) {$2$};

% Points at x = -6 and x = 6
\fill (-\xmax,\ymax) circle (2pt);
\fill (\xmax,\ymax) circle (2pt);
\node[left] at (\xmax-0.5,\ymax) {$(6,y)$};

% --- "12 units wide" double arrow, above the curve ---
\draw[<->] (-\xmax, \ymax+0.8) -- (\xmax, \ymax+0.8);
\node at (1, \ymax+1.15) {12 units wide};

% --- "?" double arrow on the right, from x-axis to (6,y) ---
\draw[<->] (\xmax, 0+0.3) -- (\xmax, \ymax-0.3);
\node[right] at (\xmax+0.1, \ymax/2) {$?$};
