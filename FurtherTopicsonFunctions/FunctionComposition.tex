...
% Translucent filled ellipses (different sizes/positions, slightly rotated to overlap like the original)
\filldraw[fill=blue, fill opacity=0.35, draw=black]
  (-4,0) ellipse (1.5 and 2);
\filldraw[fill=orange, fill opacity=0.35, draw=black, rotate around={20:(0,-0.2)}]
  (0,-0.2) ellipse (1.1 and 1.5);
\filldraw[fill=red, fill opacity=0.35, draw=black]
  (3.5,-0.5) ellipse (1.3 and 1.3);

% Points
\coordinate (X) at (-3.4,0.1);
\coordinate (U) at (0,-0.3);
\coordinate (FU) at (3.3,-0.6);
\fill[black] (X) circle (1.3pt);
\fill[black] (U) circle (1.3pt);
\fill[black] (FU) circle (1.3pt);

% Upper-left arc: x -> u(x)
\draw[->, thick] (X) to[out=70, in=160] node[midway, above=10pt] {$f$} (U);

% Upper-right arc: u(x) -> f(u(x))
\draw[->, thick] (U) to[out=60, in=140] node[midway, above=10pt] {$g$} (FU);

% Bottom long arc: x -> f(u(x))
\draw[->, thick] (X) to[out=290, in=250] node[midway, below=6pt] {$g\circ f$} (FU);

% Labels
\node[left=4pt] at (X) {$x$};
\node[below=4pt] at (U) {$f(x)$};
\node[right=4pt] at (FU) {$g(f(x))$};
...
\begin{example}  Let $f(x) = x^2-4x$, $g(t) = 2-\sqrt{t+3}$, and $h(s) = \frac{2s}{s+1}$.  
...
\begin{tikzpicture}[x=20pt,y=20pt]
  \draw[{Latex[length=5pt,width=3pt]}-{Latex[length=5pt,width=3pt]}, line width=1.25pt] (-1,0) -- (5,0);
  \draw (1,-0.12) -- (1,0.12);
  \draw (3,-0.12) -- (3,0.12);
  \node at (0, 0.5) {$(+)$};
  \node at (1,-0.5) {$1$};
...
\begin{longtable}{rclr} $(f \circ g)(t)$ & = & $f(g(t))$ = $f\left(2-\sqrt{t+3}\right)$ & \\  
...
\begin{longtable}{rclr} $(f \circ g)(t)$ & = & $f(g(t))$ = $\left(g(t)\right)^2 - 4\left(g(t)\right)$& \\  
 & = & $\left(2-\sqrt{t+3}\right)^2 - 4\left(2-\sqrt{t+3}\right)$ & \\  
...
\begin{longtable}{rclr} $(g \circ h)(s)$ & = & $g(h(s))$ = $g\left(\frac{2s}{s+1}\right)$  & \\ [12pt]
 & = & $2 - \sqrt{\left(\frac{2s}{s+1}\right)+3}$ & \\[12pt] 
 & = & $2 - \sqrt{\frac{2s}{s+1} + \frac{3(s+1)}{s+1}}$ & get common denominators\\ [12pt]
 & = & $2 - \sqrt{\frac{5s+3}{s+1}}$ & \\
 \end{longtable}

\item  \textit{outside in}:  We use the formula for $g(t)$ first to get
\begin{longtable}{rclr} $(g \circ h)(s)$ & = & $g(h(s))$ = $2 - \sqrt{h(s)+3}$& \\  
  & = & $2 - \sqrt{\left(\frac{2s}{s+1}\right)+3}$ & \\[12pt] 
...
\frac{2s}{s+1} & \geq & -3 & \\ [10pt]
\frac{2s}{s+1} +3 &   \geq & 0 & \\ [10pt]
\frac{5s+3}{s+1}  & \geq &  0  & \text{get common denominators as before} \\ \end{array} \]
...
\begin{longtable}{rclr} $(h \circ g)(t)$ & = & $h(g(t))$ =$h\left(2-\sqrt{t+3}\right)$ & \\  
 & = & $\frac{2 \left(2-\sqrt{t+3} \right)}{\left(2-\sqrt{t+3}\right)+1}$ & \\[12pt] 
 & = & $\frac{4-2\sqrt{t+3}}{3-\sqrt{t+3}}$ & \\
  \end{longtable}

\item  \textit{outside in}:  We use the formula for $h(s)$ first to get
\begin{longtable}{rclr} $(h \circ g)(t)$ & = & $h(g(t))$ = $\frac{2 \left(g(t)\right)}{\left( g(t)\right) + 1}$ & \\ [12pt]
 & = & $\frac{2 \left(2-\sqrt{t+3} \right)}{\left(2-\sqrt{t+3}\right)+1}$ & \\[12pt] 
 & = & $\frac{4-2\sqrt{t+3}}{3-\sqrt{t+3}}$ & \\
  \end{longtable}
...
\begin{longtable}{rclr} $(h \circ h)(s)$ & $=$ & $h(h(s)) =h\left(\frac{2s}{s+1}\right)$ & \\ [15pt]
&=& $\frac{2\left(\frac{2s}{s+1}\right)}{\left(\frac{2s}{s+1}\right)+1}$ & \\ [25pt]
 & = & $\frac{\frac{4s}{s+1}}{\frac{2s}{s+1}+1} \cdot \frac{(s+1)}{(s+1)}$ & \\ [25pt]
& = & $\frac{\frac{4s}{s+1} \cdot (s+1)}{\left(\frac{2s}{s+1}\right)\cdot(s+1)+1\cdot(s+1)}$ & \\ [25pt]
& = & $\frac{\frac{4s}{\cancelto{1}{(s+1})} \cdot \cancel{(s+1)}}{\frac{2s}{\cancelto{1}{(s+1)}}\cdot\cancel{(s+1)}+s+1}$ & \\ [25pt]
& = & $\frac{4s}{3s+1}$ & \\ 
 \end{longtable}

\item  \textit{outside in}: This approach yields
\begin{longtable}{rclr} $(h \circ h)(s)$ & = & $h(h(s)) = \frac{2 (h(s))}{h(s) + 1}$ & \\ [.25in]
& = & $\frac{2\left(\frac{2s}{s+1}\right)}{\left(\frac{2s}{s+1}\right)+1}$ & \\[.35in] 
...
\item  \textit{outside in}:  We use the formula for $h(s)$ first to get
\begin{longtable}{rclr} $(h \circ (g \circ f))(x)$ & = & $h((g \circ f)(x))=\frac{2 \left( (g \circ f)(x)\right)}{  \left( (g \circ f)(x)\right) + 1}$  & \\ [15pt]
& = & $\frac{2 \left(2 - \sqrt{x^2-4x+3}\right)}{\left(2 - \sqrt{x^2-4x+3}\right)+1}$ & \\  
 & = & $\frac{4 - 2\sqrt{x^2-4x+3}}{3 - \sqrt{x^2-4x+3}}$ & \\
 \end{longtable}
...
((h \circ g) \circ f)(x) & = & (h \circ g)(f(x)) = (h \circ g)\left(x^2-4x\right) & \\  
                         ...
                         & = & \frac{4 - 2\sqrt{x^2-4x+3}}{3 - \sqrt{x^2-4x+3}} & \\ \end{array}\]

\item  \textit{outside in}:  We use the formula for $(h \circ g)(t)$ first to get
\begin{longtable}{rclr} $((h \circ g) \circ f)(x)$ & = & $(h \circ g)(f(x))=\frac{4-2\sqrt{(f(x))+3}}{3-\sqrt{f(x))+3}}$  & \\  
  & = & $\frac{4 - 2\sqrt{\left(x^2-4x\right)+3}}{3 - \sqrt{\left(x^2-4x\right)+3}}$ & \\ 
 & = & $\frac{4 - 2\sqrt{x^2-4x+3}}{3 - \sqrt{x^2-4x+3}}$ & \\
 \end{longtable}
...
\draw[-,line width=3pt,blue](-2,3)--(1,3);
\draw[-,line width=3pt,blue](1,3)--(2,-2);
\draw[-,line width=3pt,blue](2,-2)--(5,0);
...
coordinates {
 ...
 \node[anchor=north, color=blue] at (2,3) {$y=s(t)$};
      \end{axis}
        \end{tikzpicture}
\end{image}
\end{itemize}
...
\begin{tikzpicture}
\begin{axis}[fplot, xmin=-2, xmax=7.9, ymin=-2, ymax=9.9, domain=-0.3:6.3,]
  \addplot[fgraph, domain=-0.3:6.3] {6*x - x^2};
  \addplot[fgraph, domain=-2:8] {-1};
  \addplot[fgraph, domain=-2:8] {3};
 % \addplot[fgraph, only marks, mark=*,black, mark size=2pt] coordinates {(3,9) (0,0) (6,0)};
  \addplot[fgraph, only marks, mark=*,black, mark size=2pt] coordinates {(3 - sqrt(10), -1) (3 + sqrt(10), -1) (3 - sqrt(6), 3) (3 + sqrt(6), 3)};
...
\item $G(t) = \frac{2}{t^2+1}$

\item  $H(s) = \frac{\sqrt{s}+1}{\sqrt{s}-1}$

\end{enumerate}


\item For $F(x) = \sqrt{\frac{2x-1}{x^2+4}}$, find functions $f$, $g$, and $h$ to decompose $F$ nontrivially as $F = f \circ \left(\frac{g}{h} \right)$.


\end{enumerate}

\begin{explanation} There are many approaches to this kind of problem, and we showcase a different methodology in each of the solutions below.
...
\item  If we look $H(s) = \frac{\sqrt{s}+1}{\sqrt{s}-1}$ with an eye towards building a complicated function from simpler functions, we see the expression $\sqrt{s}$ is a simple piece of the larger function.  If we define $f(s) = \sqrt{s}$, we have $H(s) = \frac{f(s)+1}{f(s)-1}$.  If we want to decompose $H = g \circ f$, then we can glean the formula for $g(s)$ by looking at what is being done to $f(s)$.  We take $g(s) = \frac{s+1}{s-1}$, and check below:  \[(g \circ f)(s) = g(f(s)) = \frac{f(s)+1}{f(s)-1} = \frac{\sqrt{s}+1}{\sqrt{s}-1} = H(s).\]  

\end{enumerate}

\item To write $F = f \circ \left(\frac{g}{h} \right)$ means \[F(x) =  \sqrt{\frac{2x-1}{x^2+4}} = \left( f \circ \left(\frac{g}{h} \right) \right)(x) = f \left(  \left(\frac{g}{h} \right)(x) \right) = f \left( \frac{g(x)}{h(x)} \right).\]

Working from the inside out, we have a rational expression with  numerator $g(x)$ and denominator $h(x)$.  Looking at the formula for $F(x)$, one choice is $g(x) = 2x-1$ and $h(x) = x^2+4$. Making these identifications, we have  \[ F(x) = \sqrt{\frac{2x-1}{x^2+4}} = \sqrt{\frac{g(x)}{h(x)}}.\]
...
\begin{explanation} It is important to note that as we work through the expressions below, the variables $r$ and $\Delta r$ as well as $t$ and $\Delta t$ are distinct. That is, they do not combine as `like terms.'

\begin{enumerate}

\item  We start by simplifying $\frac{\Delta[S(r)]}{\Delta r} = \frac{S(r+ \Delta r) - S(r)}{\Delta r}$:

\begin{longtable}{rcl}

 $\frac{\Delta[S(r)]}{\Delta r}$ & $=$ &$\frac{S(r+ \Delta r) - S(r)}{\Delta r}$   \\[10pt]
                       & $=$ &   $\frac{4 \pi (r + \Delta r)^2 - 4 \pi r^2}{\Delta r}$ \\[10pt]
                       & $=$ &$ \frac{4 \pi \left[r^2  + 2 r \, \Delta r + (\Delta r)^2\right] - 4 \pi r^2}{\Delta r}$ \\[10pt]   
                         &$=$ &    $\frac{4 \pi r^2  + 8 \pi  r \, \Delta r + 4 \pi (\Delta r)^2 - 4 \pi r^2}{\Delta r}$  \\[10pt]   
                         & $=$ &  $\frac{8 \pi  r \, \Delta r + 4 \pi (\Delta r)^2 }{\Delta r}$   \\[10pt]
                         & $=$ &  $\frac{(\Delta r) \left(8 \pi  r + 4 \pi \, \Delta r \right) }{\Delta r}$  \\[10pt]
                         & $=$ &  $\frac{\cancel{(\Delta r)} \left(8 \pi  r + 4 \pi \, \Delta r \right) }{\cancelto{1}{(\Delta r)}}$   \\[10pt]
                         & $=$ & $8 \pi r + 4 \pi \, \Delta r$ \\ \end{longtable}
                         
To find the average rate of change of $S$ over the interval $[1,3]$, we take $r = 1$ and $\Delta r = 3  - 1 = 2$: \[ \frac{\Delta[S(r)]}{\Delta r} = 8 \pi (1) + 4\pi (2) = 16 \pi.\]  
...
\item  Next, we simplify  $\frac{\Delta[r(t)]}{\Delta t} = \frac{r(t+ \Delta t) - r(t)}{\Delta t}$:

\begin{longtable}{rcl}

$ \frac{\Delta[r(t)]}{\Delta t}$ & $=$ & $\frac{r(t+ \Delta t) - r(t)}{\Delta t}$ \\[10pt]
                          &$=$ &   $\frac{2(t+\Delta t)- 2t}{\Delta t}$ \\[10pt]
                                  & $=$ &   $\frac{2t + 2 \Delta t - 2t}{\Delta t}$ \\[10pt]
                                     & $=$ &   $\frac{ 2 \cancel{\Delta t} }{\cancelto{1}{(\Delta t)}}$ \\[10pt]
                         &$=$ &    $2$ \\ \end{longtable}
...
\item  To find  $\frac{\Delta[S(t)]}{\Delta t} = \frac{S(t+ \Delta t) - S(t)}{\Delta t}$, we start with our answer from Example \ref{surfaceareaex1}:  $S(t) =  16 \pi t^{2}$ :

\begin{longtable}{rcl}

 $\frac{\Delta[S(t)]}{\Delta t}$ & $=$ &$\frac{S(t+ \Delta t) - S(t)}{\Delta t}$   \\[10pt]
                         & $=$ & $\frac{16 \pi (t+ \Delta t)^2 - 16 \pi t^2}{\Delta t}$ \\[10pt]
                         & $=$ & $\frac{16 \pi \left[ t^2 + 2 t \, \Delta t + (\Delta t)^2\right]  - 16 \pi t^2}{\Delta t}$ \\[10pt]
                         & $=$ & $\frac{16 \pi t^2 + 32 \pi t \, \Delta t + 16 \pi (\Delta t)^2   - 16 \pi t^2}{\Delta t}$ \\[10pt]
                         & $=$ & $\frac{32 \pi t \, \Delta t + 16 \pi (\Delta t)^2}{\Delta t}$ \\[10pt]
                         & $=$ & $\frac{(\Delta t) (32 \pi t + 16 \pi \, \Delta t)}{\Delta t}$ \\[10pt]
                         & $=$ & $\frac{\cancel{(\Delta t)} (32 \pi t + 16 \pi \, \Delta t)}{\cancelto{1}{(\Delta t)}}$ \\[10pt]
...
\frac{\Delta[S(r)]}{\Delta r}  \cdot \frac{\Delta[r(t)]}{\Delta t} & = & (8 \pi r + 4 \pi \, \Delta r)(2),   \\ \end{array} \]

 which doesn't look like much unless we substitute $r = 2t$ and $\Delta r = 2 \Delta t$.  We get:

\begin{longtable}{rcl}

$\frac{\Delta[S(r)]}{\Delta r}  \cdot \frac{\Delta[r(t)]}{\Delta t}$ & $=$ & $(8 \pi r + 4 \pi \, \Delta r)(2)$   \\[10pt]
                 & $=$ & $(8 \pi (2t) + 4 \pi \ (2 \Delta t)(2)$\\[10pt]
                 & $=$ & $(16 \pi t + 8 \pi \,  \Delta t)(2)$\\[10pt]
                  & $=$ & $32 \pi t + 16 \pi \,  \Delta t$ \\ \end{longtable} 
                     
We find in this case,   \[ \frac{\Delta[S(r)]}{\Delta r}  \cdot \frac{\Delta[r(t)]}{\Delta t}   =  \frac{\Delta[S(r)]}{\Delta t}.\] 

Moreover, we note that the time interval $\frac{1}{2} \leq t \leq \frac{3}{2}$ corresponds to the interval $1 \leq r \leq 3$ so it makes sense to multiply our numerical answers as well: \[ \frac{\Delta[S(r)]}{\Delta r}  \cdot \frac{\Delta[r(t)]}{\Delta t} = 16 \pi \, \text{cm} \cdot 2 \, \frac{\text{cm}}{\text{min}} = 32 \pi \, \frac{\text{cm}^2}{\text{min}}.\]
...
\[ \frac{ \Delta[f(x)]}{\Delta x} =  \frac{ \Delta[f(u)]}{\Delta u}  \cdot  \frac{ \Delta[u(x)]}{\Delta x}, \quad \Delta x \neq 0,  \Delta u  \neq 0. \]
...
% Upper-left arc: x -> u(x)
\draw[->, thick] (X) to[out=70, in=160] node[midway, above=10pt] {$\frac{\Delta[u(x)]}{\Delta x}$} (U);

% Upper-right arc: u(x) -> f(u(x))
\draw[->, thick] (U) to[out=60, in=140] node[midway, above=10pt] {$\frac{\Delta[f(u)]}{\Delta u}$} (FU);

% Bottom long arc: x -> f(u(x))
\draw[->, thick] (X) to[out=290, in=250] node[midway, below=6pt] {$\frac{\Delta[f(x)]}{\Delta x}$} (FU);

% Labels
\node[left=4pt] at (X) {$x$};
\node[below=4pt] at (U) {$u(x)$};
\node[right=4pt] at (FU) {$f(u(x))$};
...
\begin{explanation} In this scenario, the drag force, $F$ depends directly on the speed, $v$ and the speed, $v$ depends directly on time.  (The longer the book falls, the faster it falls.\footnote{Well until it reaches \href{https://en.wikipedia.org/wiki/Terminal_velocity}{\underline{terminal velocity}} \ldots})  By Theorem \ref{relatedratesaroc}, we know \[ \frac{\Delta[F(t)]}{\Delta t} = \frac{\Delta[F(v)]}{\Delta v} \cdot \frac{\Delta v}{\Delta t}.\]
...
\[ \frac{\Delta[F(v)]}{\Delta v} = \frac{F(6) - F(5)}{6-5} = \frac{0.6 (6)^2 - 0.6 (5)^2}{1} = 6.6.\]
...
Hence, \[ \frac{\Delta[F(t)]}{\Delta t} =  \frac{\Delta[F(v)]}{\Delta v} \cdot \frac{\Delta v}{\Delta t} = \left(6.6 \,  \frac{\text{N s}}{\text{m}} \right) \left( 9.8  \, \frac{\text{m/s}}{\text{s}} \right) = 64.68 \, \frac{\text{N}}{\text{s}} .\]
