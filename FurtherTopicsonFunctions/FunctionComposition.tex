...
% Upper-left arc: x -> u(x)
\draw[->, thick] (X) to[out=70, in=160] node[midway, above=10pt] {$\dfrac{\Delta[u(x)]}{\Delta x}$} (U);

% Upper-right arc: u(x) -> f(u(x))
\draw[->, thick] (U) to[out=60, in=140] node[midway, above=10pt] {$\dfrac{\Delta[f(u)]}{\Delta u}$} (FU);

% Bottom long arc: x -> f(u(x))
\draw[->, thick] (X) to[out=290, in=250] node[midway, below=6pt] {$\dfrac{\Delta[f(x)]}{\Delta x}$} (FU);
