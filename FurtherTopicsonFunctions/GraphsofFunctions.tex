...
\begin{tikzpicture}
\begin{axis}[fplot, xmin=-5, xmax=4.9, ymin=-4, ymax=3.9, width=6in, height=6in,domain=-4:4]
  \addplot[fgraph,line width=3pt, domain=-4:4] {3*cos(deg(pi*x/4))};
  \addplot[fgraph, only marks, mark=*, black,mark size=3pt] coordinates {(-2,0) (2,0) (4,-3) (-4,-3) (0,3)};
  \node[flabel, label=above left:{$\left( -2, 0 \right)$}]  at (axis cs:-2, 0) {};
...
\begin{axis}[fplot, xmin=-5, xmax=4.9, ymin=-4, ymax=3.9, width=6in, height=6in,domain=-4:4]
  \addplot[fgraph,line width=3pt, domain=-4:4] {3*cos(deg(pi*x/4))};
  \addplot[fgraph, only marks, mark=*, black,mark size=3pt] coordinates {(-2,0) (2,0) (4,-3) (-4,-3) (0,3)};
  \addplot[fgraph, dashed, domain=-5:5] {1};
  \node[flabel, label=above left:{$\left( -2, 0 \right)$}]  at (axis cs:-2, 0) {};
  \node[flabel, label=above right:{$\left( 2, 0 \right)$}]   at (axis cs: 2, 0) {};
  \node[flabel, label=below right:{$\left( 4, -3 \right)$}]  at (axis cs: 4,-3) {};
  \node[flabel, label=below left :{$\left(-4, -3 \right)$}]  at (axis cs:-4,-3) {};
...
\item  As we move from left to right, the graph falls from $(0,3)$ to $(4,-3)$.  This means $f$ is decreasing on the interval $[0,4]$.  (Again, the answer here is an interval on the $x$-axis.) 
...
\begin{tikzpicture}
\begin{axis}[fplot, xmin=-5, xmax=7.9, ymin=-10, ymax=7.9,width=6in, height=4in]
  \addplot[fgraph,line width=3pt, domain=-4:-2] {0.75*(x^2) + 8.25*x + 18};
  \addplot[fgraph,line width=3pt, domain=-2:3]  {-0.08333*(x^2) - 2.41667*x};
  \addplot[fgraph,line width=3pt, domain=3:4]   {-2*(x^2) + 16*x - 38};
  \addplot[fgraph,line width=3pt, domain=4:5]   {-6};
  \addplot[fgraph,line width=3pt, domain=5:7.01]{-0.25*(x^2) + 8.75*x - 43.5};
  \addplot[fgraph, only marks, black, mark=*, mark size=3pt] coordinates {(-4,-3) (-3,0) (-2,4.5) (3,-8) (4,-6) (5,-6) (6,0) (7,5.5)};
  \addplot[fgraph, only marks, white, mark=*, mark size=3pt] coordinates {(0,0)};
  \addplot[fgraph, only marks,black, mark=o, mark size=3pt, mark options={line width=1pt}] coordinates {(0,0)};
  \node[flabel, label=below left :{$(-4,-3)$}]   at (axis cs:-4,-3) {};
  \node[flabel, label=above left :{$(-3,0)$}]    at (axis cs:-3, 0) {};
  \node[flabel, label=above left:{$(-2,4.5)$}]  at (axis cs:-2,4.5) {};
  \node[flabel, label=above right:{$(0,0)$}]     at (axis cs: 0, 0) {};
  \node[flabel, label=below:{$(3,-8)$}]    at (axis cs: 3,-8) {};
  \node[flabel, label=above:{$(4,-6)$}]    at (axis cs: 4,-6) {};
  \node[flabel, label=below right:{$(5,-6)$}]    at (axis cs: 5,-6) {};
  \node[flabel, label=above left:{$(6,0)$}]     at (axis cs: 6, 0) {};
...
\item  Solve $(t^2-25) g(t) = 0$. \vphantom{$\frac{g(t)}{t^2+t-30}= 0$}

\item  Solve $\frac{g(t)}{t^2+t-30} \geq 0$.
...
\node[anchor=north, blue] at (5.5,1) {$(-)$};
\node[anchor=north, red] at (6.5,1) {$(+)$};
      \end{axis}
        \end{tikzpicture}
\end{image}
...
\begin{example} \label{evenoddcompleteexample} 

\begin{enumerate}

\item  Below is the partial graph of $f$.

\begin{image}
% \input{GraphsofFunctions_pic5.tex}
\begin{tikzpicture}
\begin{axis}[fplot, xmin=-5, xmax=5, ymin=-5.9, ymax=5.9, width=6in, height=6in,domain=0:4]
  \addplot[fgraph,line width=3pt, domain=0:4] {-5*sin(deg(pi*x/4))};
  \addplot[fgraph, black,only marks, mark=*, mark size=3pt] coordinates {(0,0) (4,0)};
 %\node[flabel, anchor=north] at (rel axis cs:0.5,-0.08) {Partial graph of $y = f(x)$};
 \node at (0, -7) {Partial graph of $y = f(x)$};
\end{axis}
...
\item  If possible, complete the graph of $f$ assuming $f$ is even.

\item  If possible, complete the graph of $f$ assuming $f$ is odd.


\end{enumerate}

\item  Below is the partial graph of $g$.

 \begin{image}
% \input{GraphsofFunctions_pic6.tex}
\begin{tikzpicture}
\begin{axis}[fplot, xmin=-5, xmax=4.9, ymin=-6, ymax=5.9, width=6in, height=6in,domain=0:4]
  \addplot[fgraph,line width=3pt, domain=0:4] {-5*cos(deg(pi*x/4))};
  \addplot[fgraph, black,only marks, mark=*, mark size=3pt] coordinates {(0,-5) (4,5)};
%  \node[flabel, anchor=north] at (rel axis cs:0.5,-0.08) {Partial graph of $y = g(x)$};'
 \node at (0, -7) {Partial graph of $y = g(x)$};
\end{axis}
\end{tikzpicture}
...
\item  If possible, complete the graph of $g$ assuming $g$ is even.

\item  If possible, complete the graph of $g$ assuming $g$ is odd.
...
\begin{enumerate}
\item \begin{enumerate} \item  If $f$ is even, then the graph of $f$ is symmetric about the $y$-axis.  Hence, to complete the graph, we reflect each point on the partial graph of $f$ about the $y$-axis.

\begin{image}
% \input{GraphsofFunctions_pic7.tex}
\begin{tikzpicture}
\begin{axis}[fplot, xmin=-5, xmax=5, ymin=-6, ymax=6,width=5in, height=6in]
  \addplot[fgraph,line width=3pt,  domain=0:4]  {-5*sin(deg(pi*x/4))};
  \addplot[fgraph,line width=3pt,  domain=-4:0] { 5*sin(deg(pi*x/4))};
  \addplot[fgraph, black, only marks, mark=*, mark size=3pt] coordinates {(0,0) (4,0) (-4,0)};
%  \node[flabel, anchor=north] at (rel axis cs:0.5,-0.08) {The graph of $f$ assuming $f$ is even.};
 \node at (0, -7) {The graph of $f$ assuming $f$ is even.};
\end{axis}
\end{tikzpicture}
...
\item  If $f$ is odd,  then the graph of $f$ is symmetric about the origin.  Hence, to complete the graph of $f$,  we reflect each of the points on the partial graph of $f$ through the origin. We get the following:   


\begin{image}
% \input{GraphsofFunctions_pic9.tex}
\begin{tikzpicture}
\begin{axis}[fplot, xmin=-5, xmax=5, ymin=-6, ymax=5.9, domain=-4:4, width=5in, height=6in]
  \addplot[fgraph,line width=3pt,  domain=-4:4] {-5*sin(deg(pi*x/4))};
  \addplot[fgraph, black, only marks, mark=*, mark size=3pt] coordinates {(0,0) (4,0) (-4,0)};
%  \node[flabel, anchor=north] at (rel axis cs:0.5,-0.08) {The graph of $f$ assuming $f$ is odd.};
 \node at (0, -7) {The graph of $f$ assuming $f$ is odd.};
\end{axis}
\end{tikzpicture}
...
\item \begin{enumerate}  \item  If $g$ is even, then we can proceed as above and complete the graph of $g$ by reflecting each point on the partial graph of $g$ about the $y$-axis to get the graph below.
...
\begin{axis}[fplot, xmin=-5, xmax=5, ymin=-6, ymax=6,width=5in, height=6in]
  \addplot[fgraph,line width=3pt,  domain=0:4]  {-5*cos(deg(pi*x/4))};
  \addplot[fgraph,line width=3pt,  domain=-4:0] {-5*cos(deg(pi*x/4))};
  \addplot[fgraph, black, only marks, mark=*, mark size=3pt] coordinates {(0,-5) (4,5) (-4,5)};
%  \node[flabel, anchor=north] at (rel axis cs:0.5,-0.08) {The graph of $g$ assuming $g$ is even.};
\node at (0, -7) {The graph of $g$ assuming $g$ is even.};
\end{axis}
\end{tikzpicture}
...
\item  Assuming $g$ is odd, we proceed as above and run into a problem. We find the point $(0,-5)$ is reflected to the point $(0,5)$.  Hence, this new graph doesn't pass the vertical line test and hence is not a function.  Therefore, $g$ cannot be odd.\footnote{We leave it as an exercise to show that if a function $f$ is odd and $0$ is in the domain of $f$, then, necessarily, $f(0) = 0$.}

\begin{image}
% \input{GraphsofFunctions_pic10.tex}
\begin{tikzpicture}
\begin{axis}[fplot, xmin=-5, xmax=5, ymin=-6, ymax=5.9]
  \addplot[fgraph,line width=3pt,  domain=0:4]  {-5*cos(deg(pi*x/4))};
  \addplot[fgraph,line width=3pt,  domain=-4:0] { 5*cos(deg(pi*x/4))};
  \addplot[fgraph, black,only marks, mark=*, mark size=3pt] coordinates {(0,-5) (0,5) (4,5) (-4,-5)};
%  \node[flabel, anchor=north] at (rel axis cs:0.5,-0.08) {This graph fails the vertical line test.};
\node at (0, -7) {This graph fails the vertical line test:  $x=0$ (a.k.a. the $y$-axis) intersects the graph twice.};
\end{axis}
\end{tikzpicture}
