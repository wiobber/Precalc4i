...
\addplot[smooth, line width=1.5pt, penColor1, domain=-4:4, samples=100] {sign(x-1)*abs(x-1)^(1/3)+2};
\addplot[
  only marks,
  mark=*,
  mark size=1pt,
...
coordinates {
  (2,3) [$(2,3)$]
      ...
      \end{axis}
      \path (current axis.south) ++(0,-6pt) node {\scriptsize $y=f(x)$};
        \end{tikzpicture}
        \quad\quad\quad\quad
\begin{tikzpicture}[scale=1]font=\small
        \begin{axis}[
...
%% Main ray: f(x) = |x|, x < 0  (i.e. y = -x for x < 0)
\addplot[smooth, line width=1.5pt, penColor1, domain=-4:4, samples=100] {(x-2)^3+1};
...
coordinates {
  (3,2) [$(3,2)$]
      ...
      \end{axis}
      \path (current axis.south) ++(0,-6pt) node {\scriptsize $y=g(x)$ };
...
\addplot[smooth, line width=1.5pt, penColor1, domain=-4:4, samples=100] {(x-2)^3+1};
\addplot[smooth, line width=1.5pt, penColor1, domain=-4:4, samples=100] {sign(x-1)*abs(x-1)^(1/3)+2};
\addplot[smooth, dashed, line width=1pt, domain=-4:4, samples=10] {x};
      \end{axis}
\path (current axis.south) ++(0,-6pt) node {\scriptsize $y=f(x)$, $y=g(x)$, $y=x$ };
        \end{tikzpicture}
\end{image}
\end{enumerate}
