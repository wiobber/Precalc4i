...
\begin{problem}\label{verifyinversehwfirst}
$f(x) = 2x+7$ and $g(x) = \frac{x-7}{2}$

\end{problem}

\begin{problem}
$f(x) = \frac{5-3x}{4}$ and $g(x) = -\frac{4}{3} x + \frac{5}{3}$.

\end{problem}

\begin{problem}
$f(t) = \frac{5}{t-1}$ and $g(t) = \frac{t+5}{t}$


\end{problem}

\begin{problem}
$f(t)  = \frac{t}{t-1}$ and $g(t) = f(t) =  \frac{t}{t-1}$
...
\begin{problem}\label{verifyinversehwlast}
...
\begin{problem} \label{owninverseexample}
$f(x) = 42-x$
...
$g^{-1}(t) = \answer{-\frac{5}{3}t+\frac{1}{3}}$
...
\begin{solution}
$f^{-1}(x) = \frac{(x-5)^2 + 1}{3}, \quad x \ge 5$
\end{solution}
...
\begin{solution}
$g^{-1}(x) = \left(\frac{x+4}{3}\right)^2 + 1, \quad x \ge -4$
\end{solution}
...
\begin{solution}
$g^{-1}(x) = \frac{(1-x)^2}{8} - \frac{5}{2}, \quad x \le 1$
\end{solution}
...
\begin{solution}
$g^{-1}(x) = 5 + \sqrt{x+25}, \quad x \ge -25$
\end{solution}
\end{problem}
...
\begin{solution}
$g^{-1}(x) = -4 - \sqrt{\frac{x+5}{3}}, \quad x \ge -5$
\end{solution}
\end{problem}
...
\begin{solution}
None of the first coordinates of the ordered pairs in $F$ are repeated, so $F$ is a function and none of the second coordinates of the ordered pairs of $F$ are repeated, so $F$ is one-to-one.   

$F^{-1} = \{ (0,0), (1,1), (-1,2), (2,3), (-2,4), (3,5), (-3,6)  \}$
\end{solution}
...
\item 
$Q = \{ (n, n^2) \, | \, \text{$n$ is a \emph{natural} number.} \}$\footnote{Recall this means $n = 0, 1, 2, \ldots$.}

\begin{solution}
To show $Q$ is a function, we note that if we have the same inputs to $Q$, say $n = m$, then the outputs from $Q$, namely $n^2$ and $m^2$ are equal. To show $Q$ is one-to-one, we note that if we get the same output from $Q$, namely $n^2 = m^2$, then $n = \pm m$.  However since $n$ and $m$ are \emph{natural} numbers, both $n$ and $m$ are positive so $n = m$. Hence $Q$ is one-to-one and $Q^{-1} = \{ (n^2, n) \, | \, \text{$n$ is a \emph{natural} number.} \}$.
\end{solution}
...
\begin{problem}
Analytically show that the function $f(x) = x^3 + 3x + 1$ is one-to-one.  Use Theorem \ref{inversefunctionprops} to help you compute $f^{-1}(1), \; f^{-1}(5), \;$ and $f^{-1}(-3)$.  What happens when you attempt to find a formula for $f^{-1}(x)$?
\end{problem}

\begin{problem}
Let $f(x) = \frac{2x}{x^2-1}$.
...
\begin{problem}\label{fcircginverse}
...
\begin{problem}\label{genericinversefirst}
$f(x) = ax + b, \; a \neq 0$
\end{problem}
...
\begin{problem}
$f(x) = ax^{2} + bx + c$ where $a \neq 0, \, x \geq -\frac{b}{2a}$.
\end{problem}

\begin{problem}\label{genericinverselast}
$f(x) = \frac{ax + b}{cx + d},\;$ (See Exercise \ref{whatconditions} below.) 
...
\begin{enumerate}
\item 
Algebraically verify every function of the form: $f(x) = \frac{ax + b}{cx - a}$ is its own inverse.  What assumptions do you need to make about the values of  $a$, $b$, and $c$?
