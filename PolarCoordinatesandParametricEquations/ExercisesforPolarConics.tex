...
\draw[->]
    (4,0)
    arc[start angle=0,end angle=55,radius=4];
\node at (4.5, 3) {\scriptsize $\phi = \frac{\pi}{3}$};
\end{scope}
...
$r = \frac{6}{3 - \cos\left(\theta \right)} = \frac{2}{1 - \frac{1}{3} \cos\left(\theta \right)}$ rotated through $\phi = -\frac{\pi}{4}$ 
...
\draw[thick, blue, smooth, samples=100, domain=0:360] 
    plot ({ (6/(3-cos(\x))) * cos(\x) }, { (6/(3-cos(\x))) * sin(\x) });
...
%Focus and center
\node at (-0.05,0) {\textasteriskcentered};
\node at (1.5,-0.05) {\textasteriskcentered};
\node at (0.75,0) {$+$};

%Points
\fill[blue] (3,0) circle (2pt);
\fill[blue] (-1.5,0) circle (2pt);
\fill[blue] (0.75,2.12) circle (2pt);
\fill[blue] (0.75,-2.12) circle (2pt);
...
\draw[->]
    (4,0)
    arc[start angle=0,end angle=-40,radius=4];
\node at (4.5, -2) {\scriptsize $\phi = -\frac{\pi}{4}$};
