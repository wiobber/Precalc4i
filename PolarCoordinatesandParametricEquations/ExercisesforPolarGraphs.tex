...
% Polar Graph Region  (outer curve) -- (inner curve BACKWARDS) -- cycle 
\filldraw[fill=blue!20, fill opacity=0.5, draw=blue, thick]
plot[domain=-60:60, samples=120, smooth] ({(2*sqrt(3)*sin(\x))*cos(\x)}, {(2*sqrt(3)*sin(\x))*sin(\x)}) -- plot[domain=60:-60, samples=120, smooth] ({(1 + cos(\x))*cos(\x)}, {(1 + cos(\x)))*sin(\x)}) --cycle;

% Polar Graph
% TikZ math engine uses degrees by default
%Outer curve unused portion
\draw[dashed, blue, thick]
plot[domain=30:, samples=120, smooth] ({(2*sqrt(3)*sin(\x))*cos(\x)}, {(2*sqrt(3)*sin(\x))*sin(\x)});
