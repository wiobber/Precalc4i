...
\left| \frac{z}{w} \right| &  = & \left| (z) \left( \frac{1}{w} \right) \right| & \\ [7pt] 
...
& = & \left( \frac{|z|}{|w|}\right) \frac{\text{cis}(\alpha - \beta)}{1}  &  \\ 	 	 
						 & = &  \frac{|z|}{|w|} \text{cis}(\alpha - \beta) \\ \end{array}\]
...
\foreach \x in {1,...,7}
{
    \draw (\x*\scale,0.09)--(\x*\scale,-0.09);
    \node[below] at (\x*\scale,-0.08) {$\x$};
}
...
{
    \draw (0.09,\y*\scale)--(-0.09,\y*\scale);
    \node[left] at (-0.08,\y*\scale) {$\lab$};
}
...
%--------------------------------------------
\draw[dashed]
    (0,0)--({\rB*cos(\ang)*\scale},
            {\rB*sin(\ang)*\scale});

%--------------------------------------------
% Tick marks along the ray
%--------------------------------------------
\foreach \r in {1,...,7}
{
    \draw
    ({\r*cos(\ang)*\scale + 0.08*cos(\ang+90)},
     {\r*sin(\ang)*\scale + 0.08*sin(\ang+90)})
    --
    ({\r*cos(\ang)*\scale + 0.08*cos(\ang-90)},
     {\r*sin(\ang)*\scale + 0.08*sin(\ang-90)});
}
...
at ({\rA*cos(\ang)*\scale},
    {\rA*sin(\ang)*\scale})
{$z=4\operatorname{cis}\!\left(\dfrac{\pi}{6}\right)$};
...
at ({\rB*cos(\ang)*\scale},
    {\rB*sin(\ang)*\scale})
{$z|w|=8\operatorname{cis}\!\left(\dfrac{\pi}{6}\right)$};
...
%--------------------------------------------
\draw[dashed]
(0,0)--({\r*cos(\angA)*\scale},
         {\r*sin(\angA)*\scale});

\foreach \k in {1,...,7}
{
\draw
({\k*cos(\angA)*\scale+0.08*cos(\angA+90)},
 {\k*sin(\angA)*\scale+0.08*sin(\angA+90)})
--
({\k*cos(\angA)*\scale+0.08*cos(\angA-90)},
 {\k*sin(\angA)*\scale+0.08*sin(\angA-90)});
}
...
%--------------------------------------------
\draw[dashed]
(0,0)--({\r*cos(\angB)*\scale},
         {\r*sin(\angB)*\scale});

\foreach \k in {1,...,7}
{
\draw
({\k*cos(\angB)*\scale+0.08*cos(\angB+90)},
 {\k*sin(\angB)*\scale+0.08*sin(\angB+90)})
--
({\k*cos(\angB)*\scale+0.08*cos(\angB-90)},
 {\k*sin(\angB)*\scale+0.08*sin(\angB-90)});
}
...
at ({\r*cos(\angB)*\scale},
    {\r*sin(\angB)*\scale})
{$zw=8\operatorname{cis}\!\left(\dfrac{\pi}{6}+\dfrac{2\pi}{3}\right)$};

%--------------------------------------------
% Angle arrow (120 degrees)
%--------------------------------------------
\draw[->,line width=0.9pt]
    (1.4,0.8)
    arc[start angle=30,end angle=150,radius=1.6];
...
{
    \draw
    ({\k*cos(\ang)*\scale+0.08*cos(\ang+90)},
     {\k*sin(\ang)*\scale+0.08*sin(\ang+90)})
    --
    ({\k*cos(\ang)*\scale+0.08*cos(\ang-90)},
     {\k*sin(\ang)*\scale+0.08*sin(\ang-90)});
}
...
% Point z = 2 cis(pi/6)
%--------------------------------------------
\fill ({0.5*\rB*cos(\ang)*\scale},
       {0.5*\rB*sin(\ang)*\scale})
circle (2.8pt);
...
at ({\rA*cos(\ang)*\scale},
    {\rA*sin(\ang)*\scale})
{$(1/|w|)z=2\operatorname{cis}\!\left(\frac{\pi}{6}\right)$};

%--------------------------------------------
% Point z = 4 cis(pi/6)
%--------------------------------------------
\fill ({\rB*cos(\ang)*\scale},
       {\rB*sin(\ang)*\scale})
circle (2.8pt);
...
at ({\rB*cos(\ang)*\scale},
    {\rB*sin(\ang)*\scale})
{$z=4\operatorname{cis}\!\left(\frac{\pi}{6}\right)$};
...
% Tick on the ray
\draw
({cos(\ang)*\scale+0.08*cos(\ang+90)},
 {sin(\ang)*\scale+0.08*sin(\ang+90)})
--
({cos(\ang)*\scale+0.08*cos(\ang-90)},
 {sin(\ang)*\scale+0.08*sin(\ang-90)});
...
at ({\r*cos(\ang)*\scale},
    {\r*sin(\ang)*\scale})
{$\left(\frac1{|w|}\right)z
   =2\operatorname{cis}\!\left(\frac{\pi}{6}\right)$};

%--------------------------------------------
% Dashed extension downward
%--------------------------------------------
\draw[dashed] (0,0)--(0,-2.6*\scale);

% Star point
\node at (0,-2*\scale) {\Large *};

\node[right]
at (0.35,-2*\scale)
{$zw=
2\operatorname{cis}\!\left(
\frac{\pi}{6}
+\frac{2\pi}{3}
\right)$};
...
\draw[->,line width=0.9pt]
    (0.95*\scale,0)
    arc[start angle=0,end angle=-240,radius=0.95*\scale];
...
\axes
\tlabel[cl](6,-0.5){Real Axis}
\tlabel[cl](0.5,6){Imaginary Axis}
\xmarks{-4,-2,2,4}
\ymarks{-4,-2,2,4}
...
{The four fourth roots of $z = -16$ equally spaced $\frac{2\pi}{4} = \frac{\pi}{2}$ around the plane.}
