...
% Axis labels
%------------------------------------------------
\node[above right] at (0,4.8) {Imaginary Axis};
\node[right] at (5.05,-0.45) {Real Axis};
...
%------------------------------------------------
\node[above] at (-4,2.25)
{$(-4,2)\ \longleftrightarrow\ z=-4+2i$};

\node[above] at (3,0.25)
{$(3,0)\ \longleftrightarrow\ z=3$};

\node[right] at (0.25,-3.0)
{$(0,-3)\ \longleftrightarrow\ z=-3i$};
...
\draw[->,line width=1pt] (0,-2.0)--(0,5.0);
...
%------------------------------------------------
\node[above right] at (0,4.8) {Imaginary Axis};
\node[right] at (5.2,-0.40) {Real Axis};

%------------------------------------------------
% Dashed extension to -117
%------------------------------------------------
\draw[dashed] (-6,0)--(-2,0);
...
\item  \textbf{Quotient Rule:}  $\left| \frac{z}{w} \right| = \frac{|z|}{|w|}$, provided $w \neq 0$ \index{quotient rule ! for the modulus of a complex number}
...
|zw| & = & \sqrt{(ac-bd)^2+(ad+bc)^2} & \\
		 & = & \sqrt{a^2c^2 - 2abcd + b^2d^2 + a^2d^2 +2abcd + b^2c^2} & \text{Expand} \\
		 & = & \sqrt{a^2c^2 + a^2d^2 + b^2c^2 + b^2d^2} & \text{Rearrange terms} \\
...
\[\begin{array}{rcll}
\left|z^{k+1}\right| & = & \left|z^{k} z\right| & \text{Properties of Exponents} \\
& = & \left|z^{k}\right| |z| & \text{Product Rule} \\
...
\left| \frac{z}{w} \right| &  = & \left| (z) \left( \frac{1}{w} \right) \right| & \\
	 & = & |z| \left| \frac{1}{w}\right| & \text{Product Rule.} \\ \end{array} \]
...
% Coordinates
\coordinate (O) at (0,0);
\coordinate (Z) at (4.5,3.2);

% Axes
\draw[->,line width=1pt] (0,-0.8)--(0,5.8);
\draw[->,line width=1pt] (-0.2,0)--(5.2,0);

% Tick marks
\draw (-0.10,3.2)--(0.10,3.2);
\node[left] at (-0.10,3.2) {$bi$};

\draw (4.5,-0.10)--(4.5,0.10);
\node[below] at (4.5,-0.10) {$a$};

% Axis labels
\node[above right] at (0,5.55) {Imaginary Axis};
\node[right] at (5.15,-0.35) {Real Axis};
...
% Dashed radius
\draw[dashed,line width=0.9pt] (O)--(Z);
...
\draw[-,line width=0.8pt]
    (1.05,0)
    arc[start angle=0,end angle=35.4,radius=1.05];

\node at (2,0.42) {$\theta\in\arg(z)$};

% Equivalent representations
\node at (4.45,4.25)
{$(a,b)\longleftrightarrow
  z=a+bi\longleftrightarrow
  (r,\theta)$};
...
\item  Last but not least, for $z = -117$, $|z| = 117$ and $\theta = \pi$. We get $z = 117 \text{cis}(\pi)$. As with the previous problem, our answer is easily checked geometrically. 
...
\item  \textbf{Quotient Rule:} $\frac{z}{w} = \frac{|z|}{|w|} \text{cis}(\alpha - \beta)$, provided $|w| \neq 0$ \index{quotient rule ! for complex numbers}
...
zw & = & \left[|z|\text{cis}(\alpha)\right] \left[|w|\text{cis}(\beta)\right]  \\
...
&  & + \, i\left(\sin(\alpha)\cos(\beta)+ \cos(\alpha)\sin(\beta)\right) & \text{Factor out $i$}\\
...
z^{k+1} & = & z^{k} z & \text{Properties of Exponents} \\
				& = & \left(|z|^{k} \text{cis}(k \theta)\right) \left(|z| \text{cis}(\theta)\right) & \text{Induction Hypothesis}\\
				& = & \left(|z|^k |z|\right) \text{cis}(k \theta + \theta) & \text{Product Rule}\\
...
\frac{z}{w} & = & \frac{|z| \text{cis}(\alpha)}{|w| \text{cis}(\beta)}  \\  
& = & \left( \frac{|z|}{|w|}\right) \frac{\cos(\alpha) + i \sin(\alpha)}{\cos(\beta) + i \sin(\beta)}   \\ 		 \end{array}\]
...
\frac{z}{w}	& = & \left( \frac{|z|}{|w|}\right) \frac{\cos(\alpha) + i \sin(\alpha)}{\cos(\beta) + i \sin(\beta)} \cdot \frac{\cos(\beta) - i \sin(\beta)}{\cos(\beta) - i \sin(\beta)} & \\ \end{array}\]
...
N & = & \left[\cos(\alpha) + i \sin(\alpha)\right] \left[\cos(\beta) - i \sin(\beta)\right] & \\
  & = & \cos(\alpha)\cos(\beta)-i\cos(\alpha)\sin(\beta)  + i \sin(\alpha)\cos(\beta) - i^2 \sin(\alpha)\sin(\beta) & \text{Expand} \\
	& = & \left[\cos(\alpha)\cos(\beta)+\sin(\alpha)\sin(\beta)\right] + i\left[\sin(\alpha)\cos(\beta) -\cos(\alpha)\sin(\beta)  \right] & \text{Rearrange and Factor} \\
...
& = & \left( \frac{|z|}{|w|}\right) \frac{\text{cis}(\alpha - \beta)}{1}  &  \\ 	 & = &  \frac{|z|}{|w|} \text{cis}(\alpha - \beta) \\ \end{array}\]
...
\begin{explanation}  In order to use Theorem \ref{prodquotpolarcomplex}, we need to write $z$ and $w$ in polar form.
...
\item  Last, but not least, we have $\frac{z}{w} = \frac{4 \text{cis}\left(\frac{\pi}{6}\right)}{2\text{cis}\left(\frac{2\pi}{3}\right)} = \frac{4}{2} \text{cis}\left(\frac{\pi}{6} - \frac{2\pi}{3}\right) = 2\text{cis}\left(-\frac{\pi}{2}\right)$.  

 

Since $-\frac{\pi}{2}$ is a quadrantal angle, we can `see' the rectangular form by moving out $2$ units along the positive real axis, then rotating $\frac{\pi}{2}$ radians \textit{clockwise} to arrive at the point $2$ units below $0$ on the imaginary axis.  The long and short of it is that $\frac{z}{w} = -2i$. 
...
Focusing on $z$ and $w$ from Example \ref{polararithmeticex}, we can arrive at the product $zw$ by plotting $z$, doubling its distance from $0$ (since $|w| = 2$), and rotating $\frac{2\pi}{3}$ radians counter-clockwise.  Using the GeoGebra inteactive below, we can visualize this two step procedure one step at a time. 

% https://www.geogebra.org/classic/nvpc6xs8
\begin{center}
    \geogebra{nvpc6xs8}{800}{600}
\end{center}
...
% https://www.geogebra.org/classic/etr8vy2s
\begin{center}
    \geogebra{etr8vy2s}{800}{600}
\end{center}
...
\begin{definition} \label{nthrootcomplex} Let $z$ and $w$ be complex numbers.  If there is a natural number $n$ such that $w^{n} = z$, then $w$ is an \textbf{\boldmath $n^{th}$ root} of $z$. \index{$n^{th}$ root ! of a complex number} \index{complex number ! $n^{\textrm{th}}$ root} 
...
\[ \begin{array}{rcll}
w^3 & = & 8 & \\
\left(|w| \text{cis}(\alpha)\right)^3 & = & 8 \text{cis}(0) & \\
|w|^3 \text{cis}(3\alpha) & = & 8 \text{cis}(0) & \text{DeMoivre's Theorem} \\
\end{array}\]
...
The point $(2,0)$ corresponds to the complex number  $w_0 = 2 \text{cis}(0)$,  the point $\left(2, \frac{2\pi}{3}\right)$  corresponds to the complex number $w_1 = 2 \text{cis}\left(\frac{2\pi}{3}\right)$,  and the point $\left(2, \frac{4\pi}{3}\right)$ corresponds to the complex number $w_2 = 2 \text{cis}\left(\frac{4\pi}{3}\right)$.  Converting to rectangular form, we find $w_0 = 2$, $w_1 = -1 + i\sqrt{3}$ and $w_2 = -1-i\sqrt{3}$. 

 

While this process seems a tad more involved than our previous factoring approach, this procedure can be generalized to find, for example, all of the fifth roots of $32$. (Try using Section \ref{RealZeros} techniques on that!) 
...
%% \colorbox{ResultColor}{\bbm
\begin{theorem} \label{nthrootscomplexthm} \textbf{The \boldmath $n^{th}$ roots of a Complex Number:}

 Let $z \neq 0$ be a complex number with polar form $z = r\text{cis}(\theta)$. For each natural number $n$, $z$ has $n$ distinct $n^{\text{th}}$ roots, which we denote by $w_0$, $w_1$, \ldots, $w_{n-1}$, and they are given by the formula \index{$n^{\textrm{th}}$ root ! of a complex number} \index{complex number ! $n^{\textrm{th}}$ root} 
...
The proof of Theorem \ref{nthrootscomplexthm} breaks into to two parts:  first, showing that each $w_k$ is an $n^{\text{th}}$ root, and second, showing that the set  $\left\{ w_k \, | \, k = 0, 1, \ldots, (n-1)\right\}$ consists of $n$ different  complex numbers. 

 

 To show $w_k$ is an  $n^{\text{th}}$ root of $z$, we use DeMoivre's Theorem to show $\left(w_k\right)^n = z$.

\[ \begin{array}{rcll}
\left(w_k\right)^n & = & \left(\sqrt[n]{r}\text{cis}\left(\frac{\theta}{n} + \frac{2\pi}{n} k \right)\right)^n & \\
	 & = & \left(\sqrt[n]{r}\right)^{n} \text{cis}\left( n \cdot \left[\frac{\theta}{n} + \frac{2\pi}{n} k \right] \right) & \text{DeMoivre's Theorem}\\
& = & r \text{cis}\left(\theta + 2\pi k\right) & \\ \end{array} \]
																	 
Since $k$ is a whole number, $\cos(\theta + 2\pi k) = \cos(\theta)$ and $\sin(\theta + 2\pi k) = \sin(\theta)$. Hence, it follows that $\text{cis}(\theta + 2\pi k) = \text{cis}(\theta)$, so $\left(w_k\right)^n = r \text{cis}(\theta) = z$, as required.  

 

To show that the formula in Theorem  \ref{nthrootscomplexthm} generates $n$ distinct numbers, we assume $n \geq 2$ (or else there is nothing to prove) and note that the modulus of each of the  $w_k$  is the same, namely $\sqrt[n]{r}$.  
...
As a result, $w_k$ and $w_j$ are different complex numbers, and we are done.  By Theorem  \ref{complexfactorization}, we know there at most $n$ distinct solutions to $w^{n} = z$, and we have just found all $n$ of them.   
...
\item  We start by writing $z= - 2 + 2i\sqrt{3}$ in polar form as $z = 4 \text{cis}\left(\frac{2\pi}{3}\right)$. Since we are looking for \textit{square} roots, $n=2$.  In keeping with the notation used in Theorem \ref{nthrootscomplexthm}  we will call these roots  $w_0$ and $w_1$, in keeping with the notation suggested there.  

 

 Identifying $r =4$,  $\theta = \frac{2\pi}{3}$, Theorem \ref{nthrootscomplexthm} gives  one root as $w_0 = \sqrt{4} \text{cis}\left(\frac{(2\pi/3)}{2} + \frac{2\pi}{2} (0)\right) = 2\text{cis}\left(\frac{\pi}{3}\right)$ and the other root as $w_1 = \sqrt{4} \text{cis}\left(\frac{(2\pi/3)}{2} + \frac{2\pi}{2} (1)\right) = 2\text{cis}\left(\frac{4\pi}{3}\right)$.   
 
  
 
 Though not asked to do so, we can easily convert each of $w_0$ and $w_1$ to  rectangular form:  $w_0  =  1+i\sqrt{3}$ and $w_1  = -1-i\sqrt{3}$.  We can check our answers by showing $w_0^2= -2 + 2i\sqrt{3}$ and $w_1^2= -2 + 2i\sqrt{3}$.
...
Identifying $r = 16$, $\theta = \pi$ and $n = 4$,  Theorem \ref{nthrootscomplexthm} gives us:  $w_0 = \sqrt[4]{16} \text{cis}\left(\frac{\pi}{4} + \frac{2\pi}{4} (0)\right) = 2\text{cis}\left(\frac{\pi}{4}\right)$, $w_1 = \sqrt[4]{16} \text{cis}\left(\frac{\pi}{4} + \frac{2\pi}{4} (1)\right) = 2\text{cis}\left(\frac{3\pi}{4}\right)$, $w_2 = \sqrt[4]{16} \text{cis}\left(\frac{\pi}{4} + \frac{2\pi}{4} (2)\right) = 2\text{cis}\left(\frac{5\pi}{4}\right)$ and last, but not least,  $w_3 = \sqrt[4]{16} \text{cis}\left(\frac{\pi}{4} + \frac{2\pi}{4} (3)\right) = 2\text{cis}\left(\frac{7\pi}{4}\right)$.  

 

Once agin, we can conveniently convert our answers to rectangular form. We get:  $w_0 = \sqrt{2} + i\sqrt{2}$,  $w_1 = -\sqrt{2} + i\sqrt{2}$,  $w_2 = -\sqrt{2} - i\sqrt{2}$ and  $w_3 = \sqrt{2} - i\sqrt{2}$.  We invite the reader to check  our answers algebraically by showing  $w_0^{4} = w_1 ^{4} = w_2^{4}  = w_3^{4} = -16$.

\item  For $z = \sqrt{2} + i \sqrt{2}$, we have $z = 2\text{cis}\left(\frac{\pi}{4}\right)$.  With $r = 2$, $\theta = \frac{\pi}{4}$ and $n =3$ the usual computations yield $w_0 = \sqrt[3]{2} \text{cis}\left(\frac{\pi}{12}\right)$,  $w_1 = \sqrt[3]{2} \text{cis}\left(\frac{9\pi}{12}\right) = \sqrt[3]{2} \text{cis}\left(\frac{3\pi}{4}\right) $ and  $w_2 = \sqrt[3]{2} \text{cis}\left(\frac{17\pi}{12}\right)$. 

 

 To  convert our answers to rectangular form requires the use of  either the Sum and Difference Identities in Theorem \ref{circularsumdifference} or the Half-Angle Identities in Theorem \ref{halfangle} to evaluate $w_0$ and  $w_2$.  Since we are not explicitly told to do so, we leave this as a good, but messy, exercise.

\item  To find the five fifth roots of $1$, we write $1 = 1 \text{cis}(0)$.  We have $r = 1$, $\theta = 0$ and $n = 5$. Since $\sqrt[5]{1} = 1$, the roots are  $w_0 = \text{cis}(0) = 1$, $w_1 = \text{cis}\left(\frac{2\pi}{5}\right)$, $w_2 = \text{cis}\left(\frac{4\pi}{5}\right)$, $w_3 = \text{cis}\left(\frac{6\pi}{5}\right)$ and $w_4 = \text{cis}\left(\frac{8\pi}{5}\right)$. 

 

 The situation here is even graver than in the previous example, since we have not developed any identities to help us determine the cosine or sine of $\frac{2\pi}{5}$.  At this stage, we could approximate our answers using a calculator, and we leave this as an exercise. 
...
Essentially,  Theorem \ref{nthrootscomplexthm} says that to find the $n^{\text{th}}$ roots of a complex number,  we first take the $n^{\text{th}}$ root of the modulus and divide the argument by $n$.  This gives the first root  $w_0$. 

Each successive root is found by adding  $\frac{2\pi}{n}$ to the argument, which amounts to rotating $w_0$ by $\frac{2\pi}{n}$ radians. The result of these actions  produces $n$ roots, spaced equally around the complex plane.  
...
% Parameters
%--------------------------------------------
\def\rsmall{0.75}      % radius of inner circle
\def\rbig{2.0}         % radius of outer points

%--------------------------------------------
% Axes
%--------------------------------------------
\draw[->,line width=1pt] (-2.8,0)--(2.9,0)
    node[right]{Real Axis};

\draw[->,line width=1pt] (0,-2.8)--(0,2.9)
    node[above]{Imaginary Axis};
...
\foreach \a in {45,135,225,315}
{
    \fill (\a:\rbig) circle (2.8pt);
}

\node[right=5pt] at (45:\rbig) {$w_0$};
\node[left=5pt]  at (135:\rbig) {$w_1$};
\node[left=5pt]  at (225:\rbig) {$w_2$};
\node[right=5pt] at (315:\rbig) {$w_3$};

%--------------------------------------------
% Inner circle
%--------------------------------------------
\draw (0,0) circle (\rsmall);
...
{
    \draw[<-]
      ({\rsmall*cos(\s)},{\rsmall*sin(\s)})
      arc[start angle=\s,end angle=\e,radius=\rsmall];
}
