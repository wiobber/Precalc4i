...
%================================================
% Left panel: Cartesian coordinates
%================================================
...
% --- Point label ---
\node[left] at (-3.12,4.10)
    {$P(-3,4)$};


%================================================
% Right panel: Polar coordinates
%================================================
...
% --- Point P ---
\fill (\thetaP:3.45) circle (2.5pt);

\node[left] at (\thetaP:3.55)
    {$P(r,\theta)$};

% --- Radius label ---
\node[left=8pt] at (\thetaP:1.65)
    {$r$};
...
% Tick marks on terminal ray
\foreach \r in {0.7,1.4,2.1,2.8}
{
    \draw
        ({\r*cos(\ang)-0.06*sin(\ang)},
         {\r*sin(\ang)+0.06*cos(\ang)})
        --
        ({\r*cos(\ang)+0.06*sin(\ang)},
         {\r*sin(\ang)-0.06*cos(\ang)});
}
...
% Short dashed continuation
\draw[dashed]
    (\ang:\raylength) -- (\ang:{\raylength+0.7});
...
% Point P at r=4, theta=5pi/6
\coordinate (P) at (\ang:\pointdist);

% Dotted guide from pole to P
\draw[densely dotted]
    (0,0) -- (P);
...
If $r < 0$, we begin by moving in the opposite direction on the polar axis from the pole.  For example, to plot the point with polar coordinates $Q\left(-3.5, \frac{\pi}{4}\right)$ we would start from the pole, move to the left $3.5$ units, then rotate counter-clockwise $\frac{\pi}{4}$ radians.  Once again, we are fortunate to have a GeoGebra interactive to help us visualize this process.

% https://www.geogebra.org/classic/cpcceeus
\begin{center}
\geogebra{cpcceeus}{800}{600}
\end{center}

If we interpret the angle first, we rotate $\frac{\pi}{4}$ radians, then move back through the pole $3.5$ units.  Here we are locating a point $3.5$ units away from the pole on the terminal side of $\frac{5\pi}{4}$, not $\frac{\pi}{4}$.  This process is diagrammed below.
...
% Dashed ray at theta = pi/4
\draw[dashed]
    (0,0) -- (\ang:\raylength);

% Tick marks on dashed ray
\foreach \r in {0.7,1.4,2.1,2.8}
{
    \draw
        ({\r*cos(\ang)-0.06*sin(\ang)},
...
% Tick marks on dashed positive ray
\foreach \r in {0.7,1.4,2.1,2.8}
{
    \draw
        ({\r*cos(\ang)-0.06*sin(\ang)},
...
% Solid negative-radius ray
\draw[very thick]
    (0,0) -- ({-\negdist*cos(\ang)},{-\negdist*sin(\ang)});

% Tick marks on negative-radius ray
\foreach \r in {0.7,1.4,2.1,2.8}
{
    \draw
        ({-\r*cos(\ang)-0.06*sin(\ang)},
...
% Point with negative radius
\coordinate (Q) at ({-\negdist*cos(\ang)},{-\negdist*sin(\ang)});

% Dotted full line through the pole
\draw[densely dotted]
    (Q) -- (\ang:\raylength);
...
% Radius
\draw[very thick,->] (0,0)--(2.45,0);

\node[above] at (1.25,0.18)
{$r=3.5$};
...
% Pole and point
\fill (0,0) circle (2.5pt);
\fill (\ang:\pointdist) circle (2.5pt);
...
% Point
\coordinate (R) at (\ang:\pointdist);

% Dotted guide
\draw[densely dotted]
    (0,0) -- (R);

% Dotted clockwise angle
\draw[densely dotted]
    (0.75,0)
    arc[
        start angle=0,
...
% Short dashed continuation
\draw[dashed]
    (\ang:\pointdist) -- (\ang:2.6);

% Tick marks on radius
\foreach \r in {0.8,1.6}
{
    \draw
        ({\r*cos(\ang)-0.06*sin(\ang)},
         {\r*sin(\ang)+0.06*cos(\ang)})
...
% Pole
\fill (0,0) circle (2.5pt);
\node[above left] at (0,0) {Pole};

% Point
\coordinate (P) at (-120:\radius);
...
{
    \draw
    ({\r*cos(-120)-0.06*sin(-120)},
     {\r*sin(-120)+0.06*cos(-120)})
    --
    ({\r*cos(-120)+0.06*sin(-120)},
     {\r*sin(-120)-0.06*cos(-120)});
}
...
\draw[->]
    (0.78,0)
    arc[start angle=0,end angle=-120,radius=0.78];

\node[right] at (0.60,-0.70)
{$\theta=-120^\circ$};
...
% Tick marks
\foreach \x in {-2.4,-1.6,-0.8,0.8,1.6,2.4,3.2}
    \draw (\x,-0.08)--(\x,0.08);

% Pole
\fill (0,0) circle (2.5pt);
\node[above] at (0,0.18) {Pole};

% Point
\coordinate (Q) at (240:\radius);





% Solid ray (negative radius)
\draw[very thick]
    (0,0)--(Q);
...
{
    \draw
    ({-\r*cos(60)-0.06*sin(240)},
     {-\r*sin(60)+0.06*cos(240)})
    --
    ({-\r*cos(60)+0.06*sin(240)},
     {-\r*sin(60)-0.06*cos(240)});
}
...
\draw[->]
    (-0.78,0)
    arc[start angle=180,end angle=240,radius=0.78];

\node[left] at (-1.10,-0.80)
{$\theta=60^\circ$};
...
% Dotted negative polar axis
\draw[densely dotted] (-4.8,0) -- (0,0);

% Pole
\fill (0,0) circle (2.5pt);
\node[above left] at (0,0) {Pole};

% Radius corresponding to theta = 7pi/6
\draw[very thick]
    (0,0) -- (\ang:\pointdist);
...
% Short dotted continuation
\draw[densely dotted]
    (\ang:\pointdist) -- (\ang:5.0);
...
% Point P(-4,7pi/6)
% Negative radius reverses the direction by pi,
% so the plotted point lies at angle pi/6.
\coordinate (P) at (\ang:\pointdist);
...
As  noted above, $P$ lies on the terminal side of $\frac{\pi}{6}$, so this, coupled with $r=4$, gives us $\left(4, \frac{\pi}{6}\right)$ as one of our answers as seen below on the left.

 

To find a different representation for $P$ with $r = -4$, we may choose any angle coterminal with the angle $\theta =  \frac{7\pi}{6}$.  We pick $-\frac{5\pi}{6}$ and get $\left(-4, -\frac{5 \pi}{6}\right)$ as our second answer, sketched below on the right.
...
% Pole
\fill (0,0) circle (2.5pt);
\node[below] at (0,-0.20) {Pole};

% Dotted terminal ray
\draw[dashed] (0,0)--(\ang:4.8);
...
\node[right] at (\ang:\pointdist)
{$P\left(4,\frac{\pi}{6}\right)$};
...
% Point (negative radius reverses direction)
\coordinate (P) at (30:\pointdist);

% Dotted guide
\draw[densely dotted] (0,0)--(30:4.8);

% Tick marks on guide
\foreach \r in {1,2,3}
{
    \draw
    ({\r*cos(30)-0.06*sin(30)},
     {\r*sin(30)+0.06*cos(30)})
    --
    ({\r*cos(30)+0.06*sin(30)},
     {\r*sin(30)-0.06*cos(30)});
}
...
\def\panelwidth{7.6}
\def\radius{3.0}      % display length (represents r=117)
%%%%
\def\R{2}
...
\def\rend{1}   % ending radius (grows slightly over the sweep)
\def\amax{-450}    % total sweep in degrees: 360 + 60 = 7*180/3
...
% Arrow at end of radius
\draw[->]
    (270:\radius)--(270:{\radius+0.45});

% Endpoint
\fill (270:\radius) circle (2.5pt);

% Clockwise spiral angle


\node[right] at (1.20,-0.55)
{$\theta=-\frac{5\pi}{2}$};
...
For the $r=117$ case, we can  take $\theta$ to be any angle coterminal with $-\frac{5\pi}{2}$.  In this case, we choose $\theta = \frac{3\pi}{2}$, and get $\left(117, \frac{3\pi}{2}\right)$ as one answer as seen below on the left. 

 

For the $r = -117$ case,  we visualize moving left $117$ units from the pole and then rotating through an angle $\theta$ to reach $P$.  We find that $\theta = \frac{\pi}{2}$ works here, so our second answer is $\left(-117, \frac{\pi}{2}\right)$, sketched below on the right.
...
\def\panelwidth{8.2}
\def\radius{3.0}      % Display length for r=117
%%%%
\def\R{2}
\def\tdeg{60} % theta, in degrees
\def\ttdeg{60} % theta, in degrees
...
\node[left] at (-1.15,-0.65)
{$\theta=\frac{3\pi}{2}$};

% Label
\node[below] at (270:\radius)
{$P\left(117,\frac{3\pi}{2}\right)$};
...
\def\panelwidth{8.0}
\def\ang{135}          % equivalent direction for (-3,-pi/4)
\def\pointdist{3}
...
% Dotted negative polar axis
\draw[densely dotted] (-4.6,0)--(0,0);

% Pole
\fill (0,0) circle (2.5pt);
\node[below] at (0,-0.18) {Pole};
...
Since $P$ lies on the terminal side of $\frac{3\pi}{4}$, one alternative representation for  $P$ is $\left(3, \frac{3\pi}{4}\right)$, sketched below on the left.

 

 To find a different representation for $P$ with $r=-3$, we may choose any angle coterminal with $-\frac{\pi}{4}$.  We choose $\theta = \frac{7\pi}{4}$ for our final answer $\left(-3, \frac{7\pi}{4} \right)$, as seen below on the right.
...
\node[right] at (0.40,0.78)
{$\theta=\frac{3\pi}{4}$};


%================================================
% Right panel : P(-3,7pi/4)
%================================================
...
{
    \draw
    ({\r*cos(135)-0.06*sin(135)},
     {\r*sin(135)+0.06*cos(135)})
    --
    ({\r*cos(135)+0.06*sin(135)},
     {\r*sin(135)-0.06*cos(135)});
}
...
To find $\theta$, know $\tan(\theta) = \frac{y}{x} = \frac{-2\sqrt{3}}{2} = -\sqrt{3}$.  This tells us $\theta$ has a reference angle of $\frac{\pi}{3}$, and  since $P$ lies in Quadrant IV, we know $\theta$ is a Quadrant IV angle.   To satisfy the stipulation that $0 \leq \theta < 2\pi$, we choose $\theta = \frac{5\pi}{3}$.  Hence, our answer is  $\left(4, \frac{5\pi}{3}\right)$, which we sketch below on the left.  
...
We find $\tan(\theta) = \frac{-3}{-3} = 1$, which means $\theta$ has a reference angle of $\frac{\pi}{4}$.  Since $Q$ lies in Quadrant III, we choose $\theta = \frac{5\pi}{4}$, to satisfy the requirement that $0 \leq \theta < 2\pi$.  Our final answer is  $\left(3\sqrt{2}, \frac{5\pi}{4}\right)$, which we sketch below on the right.  
...
{
\draw
({\r*cos(300)-0.06*sin(300)},
 {\r*sin(300)+0.06*cos(300)})
--
({\r*cos(300)+0.06*sin(300)},
 {\r*sin(300)-0.06*cos(300)});
}
...
% Point
\fill (300:3.8) circle (2.5pt);

\node[right=5pt] at (300:3.8)
{$P$};
...
\node[right] at (0.65,0.80)
{$\theta=\frac{5\pi}{3}$};

%================================================
% Right panel : theta = 5pi/4
%================================================
...
{
\draw
({\r*cos(225)-0.06*sin(225)},
 {\r*sin(225)+0.06*cos(225)})
--
({\r*cos(225)+0.06*sin(225)},
 {\r*sin(225)-0.06*cos(225)});
}
...
% Point
\fill (225:3.8) circle (2.5pt);

\node[left=5pt] at (225:3.8)
{$Q$};
...
\item The point $R(0,-3)$ lies along the negative $y$-axis.  While we could go through the usual computations\footnote{Since $x=0$, we would have to determine $\theta$ geometrically.} to find the polar form of $R$, in this case it is much more efficient to find the polar coordinates of $R$ using the definition.
 ...
 Concerning $\theta$, we once again find more or less `by inspection' that $\theta = \frac{3\pi}{2}$ satisfies $0 \leq \theta < 2\pi$ with its terminal side along the negative $y$-axis.  Hence, our answer is $\left(3, \frac{3\pi}{2}\right)$, as seen below on the left.  
...
% Axes
\draw[->] (-\axislength,0) -- (\axislength,0)
    node[right] {$x$};

\draw[->] (0,-\axislength) -- (0,\axislength)
    node[above] {$y$};
...
\node[left] at (-3.15,4.05)
    {$S$};

% Tick marks on the dashed ray
\foreach \r in {1.25,2.5,3.75}
{
    \draw
        ({-\r*3/5-0.07*4/5},
         {\r*4/5-0.07*3/5})
...
r^2\left(\cos^2(\theta) + \sin^{2}(\theta)\right) - 6 r\cos(\theta) & = & 0 & \text{Subtract $9$ from both sides.}\\
r^2 - 6 r\cos(\theta) & = & 0 & \text{Since $\cos^2(\theta) + \sin^2(\theta) = 1$} \\
...
-\left(-(r')^2 - r'\cos(\theta') \right)   & \stackrel{\text{?}}{=}  & (r')^2 - r' \cos(\theta' + \pi) & \text{Since $r' = -(r')^{2} - r'\cos(\theta')$} \\

(r')^2 + r'\cos(\theta') & \stackrel{\text{?}}{=}  & (r')^2 - r' (- \cos(\theta')) & \text{Since $\cos(\theta' + \pi) = -\cos(\theta')$} \\

(r')^2 + r'\cos(\theta') & \stackrel{\text{check}}{=}  & (r')^2 + r' \cos(\theta') & \\
...
Hence,  any point $(r,\theta)$ which satisfies $r^2 = \left(r^2 + r\cos(\theta)\right)^2$ has a representation which satisfies  $r = r^2 + r\cos(\theta)$, so a rectangular representation of $r = 1 - \cos(\theta)$ is $x^2 + y^2 = \left(x^2 + y^2 + x\right)^2$.   
