...
To help identify which process $f$ or $g$ is a function and  why the other is not, we create  \index{diagram ! mapping} \textbf{mapping diagrams} for $f$ and $g$ below.  In each case, we organize the inputs in a column on the left and the outputs in a column on the right.  We draw an arrow connecting each input to its corresponding output(s).  Note that the arrows communicate the grammatical bias: the arrow originates at the input and points to the output.
...
% Column titles
\node at (-3.5,6) {$N$ (inputs)};
\node at (0,6) {$f$};
\node at (3.5,6) {$T$ (outputs)};

% Left nodes
\node (wp) at (-3.5,5) {White Paw};
\node (co) at (-3.5,4) {Cooper};
\node (bi) at (-3.5,3) {Bingo};
\node (ke) at (-3.5,2) {Kennie};

% Right nodes
\node (cat) at (3.5,5) {cat};
\node (liz) at (3.5,4) {lizard};
\node (tur) at (3.5,3) {turtle};
...
% Column titles
\node at (-3.5,6) {$T$ (inputs)};
\node at (0,6) {$g$};
\node at (3.75,6) {$N$ (outputs)};

% Left nodes
\node (cat2) at (-3.5,5) {cat};
\node (liz2) at (-3.5,4) {lizard};
\node (tur2) at (-3.5,3) {turtle};

% Right nodes
\node (wp2) at (3.75,5) {White Paw};
\node (co2) at (3.75,4) {Cooper};
\node (bi2) at (3.75,3) {Bingo};
\node (ke2) at (3.75,2) {Kennie};
...
\end{image}
\begin{center}
    \captionof{figure}{Mapping diagrams for $f$ and $g$}
\xmalt{Mapping diagrams illustrating a function and its reverse. In the top diagram, the function $f$ maps the inputs White Paw, Cooper, Bingo, and Kennie to the outputs cat, cat, lizard, and turtle, respectively. In the bottom diagram, the function $g$ maps cat to White Paw and Cooper, lizard to Bingo, and turtle to Kennie.} 
\end{center}
...
% Left set A (closed curve)
\draw[line width=0.75pt]
  plot[smooth cycle] coordinates {(-6,7) (-12,0) (-6,-9) (-7,0)};
\node at (-9,-10) {$A$};

% Right set B (closed curve)
\draw[line width=0.75pt]
  plot[smooth cycle] coordinates {(6,7) (11,0) (5,-8)};
\node at (7,-10) {$B$};

% Arrow representing the function
\draw[->, line width=0.75pt]
  plot[smooth] coordinates {(-8.75,0.25) (0,5) (6.75,0.25)};

\end{tikzpicture}
\end{image}
\begin{center}
    \captionof{figure}{Diagrammatic representation of a function}
\xmalt{Diagram illustrating the concept of a function. An element labeled $a$ in set $A$ is mapped by the function $f$ to the element labeled $f(a)$ in set $B$. The element $a$ is identified as the input, and $f(a)$ is identified as the output.} 
\end{center}
...
2h(x) & = & 2\left(-x^2 + 3x + 4\right) & \\  
      & = & -2x^2 + 6x + 8. \\ 
      \end{array} \]

\item  To find $h(t + 2)$, we substitute the quantity $t + 2$ in place of $t$: \[ \begin{array}{rclr}  
h(t + 2) & = & -(t + 2)^2 + 3(t + 2) + 4 & \\  
         & = & -\left(t\,^{2} + 4t + 4\right) + (3t + 6) + 4 & \\  
         & = & -t\,^{2} - 4t - 4 + 3t + 6 + 4 &  \\  
         & = & -t\,^{2} - t + 6. & 
       \end{array} \] To find $h(t) + 2$, we add $2$ to the expression for $h(t)$ \[ \begin{array}{rclr}  
h(t) + 2 & = & \left(-t\,^{2} + 3t + 4\right) + 2  & \\  
         & = & -t\,^{2} + 3t + 6. \\ 
         \end{array} \] From our work above, we see that $h(2) = 6$ so \[ \begin{array}{rclr}  
h(t) + h(2) & = & \left(-t\,^{2} + 3t + 4\right) + 6  & \\  
            & = & -t\,^{2} + 3t + 10. \\ 
            \end{array} \]

\end{enumerate}

\item   We know $h(-1) = 0$ from above, so $t=-1$ should be one of the answers to $h(t) = 0$.  In order to see if there are any more, we set  $h(t) = -t^2+3t+4 = 0$. Factoring\footnote{You may need to review Section \ref{AppFactoring}.} gives $-(t+1)(t-4) = 0$, so we get $t=-1$ (as expected) along with $t=4$.     
...
\begin{image}
%---------------- LEFT GRAPH ----------------%
\begin{tikzpicture}[scale=0.9]\Large
...
% Axis labels
\node[below] at (13,0) {\scriptsize $t$};
\node[left]  at (0,10) {\scriptsize $T$};
...
\foreach \y/\lab in {
  ...
  \node[left] at (-0.3,\y) {\scriptsize \lab};

% Points
\foreach \x/\y in {
  0/6.4, 2/6.7, 4/7.5, 6/8.0, 8/8.3, 10/8.3, 12/8.2}
  \fill (\x,\y) circle (2pt);

\end{tikzpicture}
\end{image}
\begin{center}
    \captionof{figure}{The graph of $T=f(t)$}
\xmalt{Scatter plot of temperature $T$ versus time $t$. The plotted points are approximately $(0,64)$, $(2,67)$, $(4,75)$, $(6,80)$, $(8,83)$, $(10,83)$, and $(12,82)$. The temperature generally increases over time, reaches a high value near $83$, and then levels off.} 
\end{center}
...
\foreach \y/\lab in {
  1/64,2/66,3/68,4/72,5/74,6/76,7/78,8/80,9/82}
  \node[left] at (-0.3,\y) {\scriptsize \lab};

% Points
\foreach \x/\y in {
  0/1, 2/2.5, 4/5.5, 6/8, 8/9.5, 10/9.5, 12/9}
  \fill (\x,\y) circle (2pt);
...
\begin{center}
    \captionof{figure}{The graph of $T=f(t)$}
\xmalt{Scatter plot of temperature $T$ versus time $t$. The plotted points are $(0,64)$, $(2,67)$, $(4,75)$, $(6,80)$, $(8,83)$, $(10,83)$, and $(12,82)$. The temperature increases from $64$ to a maximum of $83$, remains near $83$, and then decreases slightly to $82$.} 
\end{center}
...
%% Extra thick graph
\addplot[smooth, line width=3.5pt,domain=-2:4, penColor1] {x^2-4};
...
\begin{center}
%    \captionof{figure}{The graph of $T=f(t)$}
\xmalt{Graph of a smooth U-shaped curve passing through the points $(-2,0)$, $(0,-4)$, and $(2,0)$. The curve reaches its lowest point at $(0,-4)$. An open circle at $(1,-3)$ indicates that this point is excluded from the graph.} 
\end{center}
...
\item  The domain of $F$ is the set of inputs to $F$.  With $v$ as the input here,  we need to describe the set of $v$-values on the graph.  We can accomplish this by  \index{projection ! to an axis}\textbf{projecting} the graph to the $v$-axis and seeing what part of the $v$-axis is covered.   Using the GeoGebra interactive below, adjust the slider to visualize the graph of $F$ being projected to the $v$-axis.  Each point on the graph of $F$ is `pushed' to the $v$-axis, and the portion which is covered by this process is the domain of $F$.


% https://www.geogebra.org/classic/xrjvhdwu

\begin{center}
    \geogebra{xrjvhdwu}{800}{600}
\end{center}


The leftmost point on the graph is $(-2,0)$, so we know that the domain starts at $v=-2$.  The graph continues to the right until we encounter the `hole' labeled at $(1,-3)$.  This indicates one and only one point, namely $(1,-3)$ is missing from the curve which for us means $v = 1$ is not in the domain of $F$.  The graph continues to the right and the arrow on the graph indicates that the graph goes upwards to the right indefinitely.  Hence, our domain is $\{ v \, | \, v \geq -2, \, v \neq 1 \}$ which, in interval notation, is $[-2, 1) \cup (1, \infty)$. 


To find the range of $F$, we need to describe the set of outputs - in this case, the $w$-values on the graph.  Here, we project the graph to the $w$-axis.  Once again, we are fortunate to have a GeoGebra interactive to help us visualize this process.

% https://www.geogebra.org/classic/z2pvgbmr
\begin{center}
\geogebra{z2pvgbmr}{800}{600}
\end{center}



Vertically, the graph starts at $(0,-4)$ so our range starts at $w=-4$.  Note that even  though there is a hole at $(1,-3)$, the $w$-value $-3$ is covered by what \textbf{appears} to be the point $(-1,-3)$ on the graph.\footnote{For all we know, it could be $(-0.992, -3)$.} The arrow indicates that the graph extends upwards indefinitely so the range of $F$ is   $\{ w \, |  \, w \geq -4 \}$  or, in interval notation, $[-4, \infty)$.   Regarding extrema, $F$ has a minimum of $-4$ when $v = 0$, but given that the graph extends upwards indefinitely, $F$ has  no maximum. 
...
\item  Finally, to determine if $v$ is a function of $w$, we look to see if each $w$-value is paired with only one $v$-value on the graph.  We have points on the graph, namely $(-2,0)$ and $(2,0)$, that clearly show us that $w = 0$ is matched with the \textbf{two} $v$-values $v = 2$ and $v = -2$.  Hence, $v$ is not a function of $w$.  
...
coordinates {
  ...
  \draw[gray, line width=1.5pt] (-4.5,-5)--(-4.5,5);
\draw[gray, line width=1.5pt] (-4,-5)--(-4,5);
\draw[gray, line width=1.5pt] (-3.5,-5)--(-3.5,5);
\draw[gray, line width=1.5pt] (-3,-5)--(-3,5);
\draw[gray, line width=1.5pt] (-2.5,-5)--(-2.5,5);
\draw[gray, line width=1.5pt] (-2,-5)--(-2,5);
\draw[gray, line width=1.5pt] (-1.5,-5)--(-1.5,5);
\draw[gray, line width=1.5pt] (-1,-5)--(-1,5);

\draw[gray, line width=1.5pt] (-0.5,-5)--(-0.5,5);
\draw[gray, line width=1.5pt] (0.5,-5)--(0.5,5);
\draw[gray, line width=1.5pt] (1,-5)--(1,5);
\draw[gray, line width=1.5pt] (1.5,-5)--(1.5,5);
\draw[gray, line width=1.5pt] (2,-5)--(2,5);
...
\end{image}
\begin{center}
   \captionof{figure}{Vertical line test}
\xmalt{Graph of the function $F$. The graph forms a U-shaped curve with $v$-intercepts at $(-2,0)$ and $(2,0)$ and a minimum at $(0,-4)$. An open circle at $(1,-3)$ indicates that this point is excluded from the graph.  Several vertical lines are drawn across the graph to illustrate the Vertical Line Test. Each vertical line intersects the graph at most once. } 
\end{center}
...
\begin{tikzpicture}
    ...
    \filldraw[blue, opacity=0.4] (5,0,0)--(5, 0, 3)--(5,3,3)--(5,3,0)--cycle;
    \filldraw[blue, opacity=0.5] (5,0,3)--(0,0,3)--(0,3,3)--(5,3,3)--cycle;
    \filldraw[blue, opacity=0.6] (0,3,3)--(0,3,0)--(5,3,0)--(5,3,3)--cycle;

    \node at (5.2,0,1.5) {$x$};
    \node at (5.2,1.5,0) {$x$};
    \node at (2.5,-0.2,3.2) {Length};
          
\end{tikzpicture}
\end{image}
\begin{center}
\xmalt{A rectangular box whose base has dimensions $x\times x$. } 
\end{center}
...
Moving forward, we start with $x = 5$ and sample $V$ at steps of $5$ in its domain.  Our goal is to graph $y = V(x)$, so we list the ordered pairs $(x, V(x))$.   The right endpoint, $x = 26$, is included in the domain $0 < x \leq 26$, so we end our table with $(26, V(26)) = (26,17, \! 576)$.
\begin{center}
$
...
To graph the points, $(x, V(x))$, we use the domain as a guide to help us set the horizontal bounds (i.e., the bounds on $x$) and the sample values from the range to help us set the vertical bounds (i.e., the bounds on $y$).  With some help from Desmos, we create the graph below.
  



% https://www.desmos.com/calculator/vq29svps5g

\begin{center}
    \desmos{vq29svps5g}{800}{600}
\end{center}



\item  The largest volume in this case refers to the maximum of $V$.  The biggest $y$-value in our table of data is $20, \! 000$ cubic inches which occurs at $x = 20$ inches, but the graph produced by Desmos indicates that there are points on the graph of $V$ with $y$-values (hence $V(x)$ values) greater than $20, \! 000$.  Indeed, the graph continues to rise to the right of $x = 20$ and the graphing utility reports the maximum $y$-value to be $y \approx 20, \! 342.593$ when $x \approx 21.667$. 

% https://www.desmos.com/calculator/eaijwrnfa5
\begin{center}
\desmos{eaijwrnfa5}{800}{600}
\end{center}
