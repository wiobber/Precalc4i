...
The \index{absolute value ! definition of}\index{function ! absolute value}\textbf{absolute value} of a real number $x$, denoted $|x|$, is given by \[ |x| =  \begin{cases} 
      -x &  \text{if $x < 0$} \\
      x & \text{if $x \geq 0$} \\
   \end{cases} \]
...
\addplot[smooth, line width=1.5pt, penColor1, domain=-4:0, samples=2] {-x};
...
%% Main ray: f(x) = |x|, x < 0  (i.e. y = -x for x < 0)
\addplot[smooth, line width=1.5pt, penColor1, domain=0:4, samples=2] {x};
%% Regular points, labeled
\addplot[
  only marks,
  mark=*,
...
\addplot[smooth, line width=3pt, penColor1] {abs(x-3)};
...
Now that we have a complete graph,\footnote{We know it's complete because we did the Math - no trusting technology on this example!}  we see that the vertex is $(3, 0)$ and the range  is $[0, \infty)$.  The minimum of $f$ is $0$ when $x = 3$ and $f$ has no maximum.  Also, $f$  is decreasing over $(-\infty, 3]$ and increasing on $[3, \infty)$. 
...
\addplot[smooth, line width=3pt, penColor1] {abs(2*x-1)-3};
...
\addplot[smooth, line width=3pt, penColor1,domain=1/3:2] {4-2*(3*x-1)};
\addplot[smooth, line width=3pt, penColor1,domain=-1:1/3] {4+2*(3*x-1)};
...
% x-axis (no vertical axis, no numbers)
\draw[-Latex, line width=1pt] (-3,0) -- (3.3,0) node[right] {$x$};

% Main V-shaped curve (minimum at h)
\draw[line width=1.5pt, penColor1] (-2.3,2.6) -- (0,0.3) -- (2.3,2.6);

% Horizontal dashed line connecting F(h-\Delta x) and F(h+\Delta x)
\draw[dashed] (-1,1.3) -- (1,1.3);
% Vertical dashed line through h
\draw[dashed] (0,1.3) -- (0,0);

% Vertical projection arrows from axis up to the curve
\draw[<->, line width=0.6pt] (-1,0) -- (-1,1.2);
\draw[<->, line width=0.6pt] (1,0) -- (1,1.2);

% Marked points
\filldraw (-1,1.3) circle (1.5pt);
\filldraw (1,1.3) circle (1.5pt);

% Point labels
\node[left] at (-1,1.3) {$F(h-\Delta x)$};
\node[right] at (1,1.3) {$F(h+\Delta x)$};

% Axis-position labels
\node at (-1,-0.35) {$h-\Delta x$};
\node at (0,-0.35) {$h$};
\node at (1,-0.35) {$h+\Delta x$};

% Delta x brackets
\draw[<->] (-1,-0.8) -- (0,-0.8) node[midway,below] {$\Delta x$};
\draw[<->] (0,-0.8) -- (1,-0.8) node[midway,below] {$\Delta x$};
\end{tikzpicture}\quad\quad\quad\quad
\begin{tikzpicture}[scale=1]
% x-axis (no vertical axis, no numbers)
\draw[-Latex, line width=1pt] (-3,0) -- (3.3,0) node[right] {$x$};

% Main peak-shaped curve (maximum at h)
\draw[line width=1.5pt, penColor1] (-2.3,0) -- (0,2.3) -- (2.3,0);
...
\addplot[smooth, line width=3pt, penColor1,domain=-2:1] {2*x+1};
\addplot[smooth, line width=3pt, penColor1,domain=1:4] {-2*x+5};
...
\addplot[smooth, line width=3pt, penColor1,domain=-7:1] {abs(x+3)+2};
...
\addplot[smooth, line width=3pt, penColor1,domain=5/3:4] {-1.5*(x-3)};
\addplot[smooth, line width=3pt, penColor1,domain=-1:5/3] {1.5*x-0.5};
...
\addplot[smooth, line width=3pt, penColor1,domain=-1:2] {-2*x+2};
\addplot[smooth, line width=3pt, penColor1,domain=2:5] {-2};
...
Last, for $t \geq 2$, $|t| = t$ and $|t - 2| = t - 2$, so $g(t) = |t-2| - |t| = (t - 2) - (t) = -2$.  Putting all three parts together yields: \[ g(t) = |t - 2| - |t| = \begin{cases} 
      (-t + 2) - (-t) &  \text{if $t < 0$} \\
...
\addplot[smooth, line width=3pt, penColor1,domain=0:2] {-2*x+2};
\addplot[smooth, line width=3pt, penColor1,domain=2:5] {-2};
\addplot[smooth, line width=3pt, penColor1,domain=-2:0] {2};
...
% https://www.geogebra.org/calculator/scjmphnq
\begin{center}
    \geogebra{scjmphnq}{800}{450}
\end{center}
...
% https://www.geogebra.org/calculator/sqwyzgzj
\begin{center}
    \geogebra{sqwyzgzj}{800}{450}
\end{center}
...
% https://www.geogebra.org/calculator/db9r7h2t
\begin{center}
    \geogebra{db9r7h2t}{800}{450}
\end{center}
...
We know from Section \ref{AppLines} that the graph of $y = 0.9x - 3.6$ is a line with slope $0.9$ and $y$-intercept $(0, -3.6)$. To find the $x$-intercept here we solve $0.9x - 3.6 = 0$ and get $x = 4$.  Hence, $(4,0)$ is an $x$-intercept here as well, and we have stumbled upon one solution to $4-|x| = 0.9x - 3.6$, namely $x = 4$.  Our graph is below.

% https://www.geogebra.org/calculator/ydcyxygn

\begin{center}
    \geogebra{ydcyxygn}{800}{500}
\end{center}

The question is if there are any other solutions.  Our graph certainly looks as if there is just one intersection point, but we know from Theorem \ref{linearabsvaluegraphs} that the slopes of the linear parts of $y = 4 - |x|$ are $\pm 1$.  The slope of $y =  0.9x - 3.6$ is $0.9$ and $0.9 \neq 1$ so we know that the left hand side of the`$\wedge$' must meet up with the graph of the line because they are not parallel.\footnote{See Theorem \ref{parallelperpendicularslopetheorem}.}

\medskip

Definition \ref{absolutevaluepiecewise} tells us that when $x<0$, $|x| = -x$, so $4-|x| = 4-(-x) = 4+x$.  Hence we set about solving  $4+x = 0.9x - 3.6$ and get $x = -76$. Both $x = -76$ and $x = 4$  check in our original equation, $4-|x| = 0.9x  - 3.6$, so we have found our two solutions. Returning to the graph above, we invite the reader to adjust the "zoom" slider to see the other intersection point, $(-76, -72)$ which corresponds to our second solution.\footnote{Once again, we see that the technology \textit{suggests} solutions whereas the algebra \textit{proves} them.}
...
% https://www.geogebra.org/calculator/kkt2npqm
\begin{center}
\geogebra{kkt2npqm}{800}{500}
\end{center}

To our surprise and delight, the graphs appear to overlap for $t \leq 0$.  Indeed, for $t \leq 0$, $|t-3| = -(t-3) = -t+3$ and $|t| + 3 = - t+3$.  Since the formulas are \textbf{identical} for these values of $t$, our solutions are all values of $t$ with $t \leq 0$.  Using interval notation, we state our solution as $(-\infty, 0]$. (The other parts of the graphs are non-intersecting parallel lines so we ignored them.)

\item  To solve $|x+1|\geq \frac{x+4}{2}$, we first graph $y = |x+1|$ and $y = \frac{x+4}{2} = \frac{1}{2} x + 2$.  The former is `$\vee$' shaped with a vertex at $(-1,0)$ and a $y$-intercept of $(0,1)$.  The latter is a line with $y$-intercept $(0,2)$, slope $m = \frac{1}{2}$ and $x$-intercept $(-4,0)$.  Our graph of the situation is below.

% https://www.geogebra.org/calculator/hgpajzzf

\begin{center}
    \geogebra{hgpajzzf}{800}{500}
\end{center}
...
\item  Recall that the inequality $2 < |t-1| \leq 5$ is an example of a `compound' inequality in that is two inequalities in one.\footnote{See Example \ref{linearineqreview} for examples of linear compound inequalities.}  The values of $t$ in the solution set need to satisfy $2 < |t-1|$ \textbf{and} $|t-1| \leq 5$.  To help us sort through the cases, we graph the horizontal lines $y =2$ and $y = 5$ along with the `$\vee$' shaped $y= |t-1|$ with vertex $(1,0)$ and $y$-intercept $(0,1)$ below. 

% https://www.geogebra.org/calculator/xpkc3jbj

\begin{center}
    \geogebra{xpkc3jbj}{800}{500}
\end{center}
