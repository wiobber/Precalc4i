...
\begin{equation} \label{falg} f(x) = \frac{x^2-x-6}{x^4+x^2} \tag{1} \end{equation}
...
\begin{equation} \label{fcalc} f(x) = \frac{x+7}{x^2+1} - \frac{1}{x} - \frac{6}{x^2}  \tag{2} \end{equation}
...
\[ \frac{x^2-x-6}{x^4+x^2} =  \frac{x^2-x-6}{x^2 \left(x^2+1\right)} \]
...
\[ \frac{x^2-x-6}{x^4+x^2} =  \frac{x^2-x-6}{x^2 \left(x^2+1\right)} = \frac{?}{x} + \frac{?}{x^2} + \frac{?}{x^2+1} \]
...
\[ \frac{x^2-x-6}{x^4+x^2} =  \frac{x^2-x-6}{x^2 \left(x^2+1\right)} = \frac{A}{x} + \frac{Bx+C}{x^2} + \frac{Dx+E}{x^2+1} \]
...
\[ \frac{x^2-x-6}{x^4+x^2} =  \frac{x^2-x-6}{x^2 \left(x^2+1\right)} = \frac{A}{x} + \frac{B}{x^2} + \frac{Cx+D}{x^2+1} \] 
...
\[ \frac{x^2-x-6}{x^4+x^2} =  \frac{x^2-x-6}{x^2 \left(x^2+1\right)} = -\frac{1}{x} - \frac{6}{x^2} + \frac{x+7}{x^2+1} \] 
...
\begin{theorem}  \label{pfdecomp} Suppose $R(x) = \frac{N(x)}{D(x)}$ is a rational function where the degree of $N(x)$ less than the degree of $D(x)$ and $N(x)$ and $D(x)$ have no common factors.\footnote{In other words, $R(x)$ is a proper rational function which has been fully reduced.}
...
\[ \frac{A_1}{ax+b} + \frac{A_2}{(ax+b)^2} + \ldots + \frac{A_{m}}{(ax+b)^m} \]

for real numbers $A_1$, $A_2$, \ldots $A_{m}$.
...
\[ \frac{B_1x + C_1}{ax^2+bx+c} + \frac{B_2x + C_2}{\left(ax^2+bx+c\right)^2} + \ldots +\frac{B_{m}x + C_{m}}{\left(ax^2+bx+c\right)^m} \]

for real numbers $B_1$, $B_2$, \ldots $B_{m}$ and $C_1$, $C_2$, \ldots $C_{m}$. 
...
The proof of Theorem \ref{pfdecomp} is best left to a course in Abstract Algebra.  Notice that the theorem provides for the general case, so we need to use subscripts, $A_1$, $A_2$, etc.,  to denote different unknown coefficients as opposed to the usual convention of $A$, $B$, etc..  The stress on multiplicities is to help us correctly group factors in the denominator.  For example, consider the rational function

\[\frac{3x-1}{\left(x^2-1\right)\left(2-x-x^2\right)}\]
...
\[\frac{3x-1}{(x+1)(x-1)(1-x)(2+x)} = \frac{3x-1}{-(x-1)^2(x+1)(x+2)} = \frac{1-3x}{(x-1)^2(x+1)(x+2)}\]

from which we proceed with the partial fraction decomposition

\[\frac{1-3x}{(x-1)^2(x+1)(x+2)} = \frac{A}{x-1} + \frac{B}{(x-1)^2} + \frac{C}{x+1} + \frac{D}{x+2}\]
...
\begin{theorem}  \label{polyequality} Suppose \[a_{n} x^{n} + a_{n-1} x^{n-1} + \cdots + a_2 x^2 + a_1 x + a_0 = b_{m} x^{m} + m_{m-1} x^{m-1} + \cdots + b_2 x^2 + b_1 x + b_0\]
...
\item  $R(x) = \frac{x+5}{2x^2-x-1}$

\item  $f(z) = \frac{3}{z^3-2z^2+z}$

\item  $F(s) = \frac{3}{s^3-s^2+s}$
...
\item  $r(x) = \frac{4x^3}{x^2-2}$

\item  $G(z) = \frac{z^3+5z-1}{z^4+6z^2+9}$

\item  $H(s) = \frac{8s^2}{s^4+16}$
...
\[\frac{x+5}{2x^2-x-1} = \frac{x+5}{(2x+1)(x-1)} = \frac{A}{2x+1} + \frac{B}{x-1}\]
...
\[\frac{x+5}{2x^2-x-1} = \frac{2}{x-1} -\frac{3}{2x+1} \]
...
\[ \frac{3}{z^3-2z^2+z} = \frac{3}{z(z-1)^2} = \frac{A}{z} + \frac{B}{z-1} + \frac{C}{(z-1)^2} \]
...
\[ \frac{3}{z^3-2z^2+z} = \frac{3}{z} - \frac{3}{z-1} + \frac{3}{(z-1)^2} \]
...
\[\frac{3}{s^3-s^2+s} = \frac{3}{s\left(s^2-s+1\right)} = \frac{A}{s} + \frac{Bs+C}{s^2-s+1}\]
...
\[\frac{3}{s^3-s^2+s} =  \frac{3}{s} + \frac{3-3s}{s^2-s+1}\]
...
\[ \frac{8x}{x^2-2} = \frac{8x}{ \left(x - \sqrt{2}\right)\left(x + \sqrt{2}\right)} = \frac{A}{x - \sqrt{2}} + \frac{B}{x + \sqrt{2}} \]
...
\[\frac{4x^3}{x^2-2} = 4x + \frac{8x}{x^2-2} = 4x + \frac{4}{x + \sqrt{2}} + \frac{4}{x - \sqrt{2}}\]
...
\[ \frac{z^3+5z-1}{z^4+6z^2+9} =  \frac{z^3+5z-1}{\left(z^2+3\right)^2} = \frac{Az+B}{z^2+3} + \frac{Cz+D}{\left(z^2+3\right)^2}\]
...
\[ \frac{z^3+5z-1}{z^4+6z^2+9} = \frac{z}{z^2+3} + \frac{2z-1}{\left(z^2+3\right)^2}\]
...
\[ \frac{8s^2}{s^4+16} = \frac{8s^2}{\left(s^2 - 2s\sqrt{2} + 4\right)\left(s^2 + 2s\sqrt{2}+4 \right)} = \frac{As+B}{s^2 - 2s\sqrt{2} + 4} + \frac{Cs+D}{s^2 + 2s\sqrt{2} + 4}\]
...
\[ \frac{8s^2}{s^4+16} = \frac{s\sqrt{2}}{s^2 - 2s\sqrt{2} + 4} - \frac{s\sqrt{2}}{s^2 + 2s\sqrt{2} + 4}\]
