...
\begin{problem}
$ \sum_{n = 0}^{\infty}  \frac{(-1)^n \, 3^{n-1}}{4^{n}}$  

$\answer{\frac{4}{21}}$
\end{problem}  

\begin{problem}
$ \sum_{m = 2}^{\infty}   \frac{3}{2^{m-1}}$
...
\begin{question}\label{seriesforfunctionex}  
In this exercise, use Theorem \ref{geoseries}  to represent  $f(x) = \frac{1}{x^2+4}$ as a series.
...
\begin{problem}
    Use the formula  in Theorem \ref{geoseries}: $\frac{a}{1-r} =  {\sum_{k=1}^{\infty} ar^{k-1}}$ to write $f(x)$ as an infinite series.

\begin{solution}
$f(x) = \frac{\frac{1}{4}}{1 - \left( -\frac{x^2}{4} \right)} = \ds{\sum_{k=1}^{\infty} \frac{1}{4} \,\left( -\frac{x^2}{4} \right)^{k-1}} = \ds{\sum_{k=1}^{\infty} \frac{(-1)^{k-1} \, x^{2k-2}}{4^{k}}} $
\end{solution}
\end{problem}
...
\begin{problem}
    $f(x) = 4-x$ over the interval $[0,4]$.   %Ans:  8 $\text{units}^2$
\end{problem}

\begin{problem}
    $f(x) = 3x^2$ over the interval $[1,3]$.    %Ans:  26 $\text{units}^2$
\end{problem}


\begin{problem}
    $f(x) = 12-x-x^2$ over the interval $[0,3]$.   %Ans:  22.5 $\text{units}^2$
\end{problem}
...
\begin{question}\label{CauchyBoundProofExercise}  
...
\begin{problem}
    If $-1 \leq Z \leq 1$, then Cauchy's Bound is immediately satisfied since $Z$ would automatically lie in the interval $\left[-(M+1), M+1\right]$. So we assume $|Z|>1$. 
...
\begin{problem}
    Use the definition of  $M$  along with the Geometric Sum Formula, Equation \ref{arithgeosum} to show:
