...
\begin{theorem} \label{CauchysBound}\index{Cauchy's Bound}\textbf{Cauchy's Bound:}  Suppose $f(x) = a_{n} x^{n} + a_{n-1}x^{n-1} + \ldots + a_{1} x + a_0$ is a polynomial of degree $n$ with $n \geq 1$.  Let $M$ be the largest of the numbers: $\frac{|a_{0}|}{|a_{n}|}$, $\frac{|a_{1}|}{|a_{n}|}$, \ldots, $\frac{|a_{n-1}|}{|a_{n}|}$.  Then all the real zeros of $f$ lie in in the interval $[-(M+1),M+1]$.
\end{theorem}
...
\begin{theorem} \label{RZT}\index{Rational Zeros Theorem}\textbf{Rational Zeros Theorem:}  Suppose $f(x) = a_{n} x^{n} + a_{n-1}x^{n-1} + \ldots + a_{1} x + a_0$ is a polynomial of degree $n$ with $n \geq 1$, and $a_0$, $a_{1}$, \ldots $a_{n}$ are integers.  If $r$ is a rational zero of $f$, then $r$ is of the form $\pm \frac{p}{q}$, where $p$ is a factor of the constant term $a_0$, and $q$ is a factor of the leading coefficient $a_{n}$.  
\end{theorem}
...
\item  In Example \ref{CBex}, we determined all of the real zeros of $f$ lie in the interval $[-4, 4]$, so we use Desmos to graph $f$ restricted to that portion of the $x$-axis.

% https://www.desmos.com/calculator/radpekcxv6
\begin{center}
\desmos{radpekcxv6}{800}{600}
\end{center}


\item  In Example \ref{RZTex}, we learned that any rational zero of $f$ must be in the list $\left\{\pm \, \frac{1}{2}, \pm \, 1, \pm \, \frac{3}{2}, \pm \, 3\right\}$.  From the graph, it looks as if we can rule out any of the positive rational zeros, since the graph seems to cross the $x$-axis at $x \approx 1.225$.  On the negative side, $x=-1$ looks good, while the other negative real root displayed, $x \approx - 1.225$ is, like its positive counterpart,  likely an approximation of an irrational zero. The shape  of the graph near $(-1,0)$ suggests that if $x=-1$ is a zero, it is of multiplicity at least two.  We set about synthetically dividing:


\begin{center}
$
\begin{array}{r}
 -1 \quad |\quad 2 \quad\quad 4 \quad\quad -1 \quad\quad -6 \quad\quad -3\\
 \underline{\downarrow \quad -2 \quad\quad -2 \phantom{11}\quad\quad 3 \phantom{11.}\quad\quad 3}\\
 \quad \quad 2 \quad\quad 2 \quad\quad -3 \quad\quad -3 \quad\quad \fbox{0}
\end{array}
$    
\end{center}
...
\begin{array}{r}
 -1 \quad |\quad 2 \quad\quad 4 \quad\quad -1 \quad\quad -6 \quad\quad -3\\
 \underline{\downarrow \quad -2 \quad\quad -2 \phantom{11}\quad\quad 3 \phantom{11.}\quad\quad 3}\\
 -1 \quad |\quad 2 \quad\quad 2 \quad\quad -3 \quad\quad -3 \quad\quad \fbox{0}\\
 \underline{\downarrow \phantom{.}\quad  -2 \quad\quad\quad 0 \quad\quad\phantom{1.} 3 \phantom{\quad\quad \fbox{0}}}\\
2 \phantom{1.}\quad\quad 0 \phantom{1.}\quad -3 \phantom{1.}\quad \fbox{0} \phantom{\quad\quad \fbox{0}}
\end{array}
$    
\end{center}

 Our quotient polynomial is now $2x^2 - 3$.  Setting this to zero gives $2x^2 - 3 = 0$, or $x^2 = \frac{3}{2}$, which gives us $x = \pm \, \frac{\sqrt{6}}{2}$.  Based on our division work, we know that $-1$ has a multiplicity of \textbf{at least} $2$. The Factor Theorem tells us our remaining zeros, $\pm \, \frac{\sqrt{6}}{2}$, each have multiplicity at least $1$.  However, Theorem \ref{nzerosreal} tells us $f$ can have at most $4$ real zeros, counting multiplicity, and so we conclude that $-1$ is of multiplicity \textbf{exactly} $2$ and $\pm \, \frac{\sqrt{6}}{2} \approx \pm 1.225$ each has multiplicity $1$. If we zoom in near $(-1,0)$ on our Desmos graph, we find the graph of $y = f(x)$ touches and rebounds from the $x$-axis at $(-1,0)$, typical behavior near a zero of multiplicity 2.   Doing the same near the other two $x$-intercepts shows the graph of $f$ crosses through the $x$-axis in a linear fashion.\footnote{Near $\left(\frac{\sqrt{6}}{2}, 0\right)$, the graph looks nearly vertical!  Indeed, using Theorem \ref{polynomialbehaviornearzeros}, we find the graph of $f$ here is closely approximated by $y = \left(12 + 5 \sqrt{6}\right)\left(x-\frac{\sqrt{6}}{2}\right)$, which has a slope of rougly $24.25$. } 
...
$
\begin{array}{r}
 \frac{1}{2} \quad |\quad 2 \quad 4 \quad -1 \quad -6 \phantom{1}\quad -3 \phantom{1}\\
 \underline{\downarrow \quad 1 \quad\phantom{1.} \frac{5}{2} \phantom{11}\quad \frac{3}{4} \quad -\frac{21}{8}} \phantom{.}\\
 \quad \quad 2 \quad\phantom{1} 5 \quad\phantom{1} \frac{3}{2} \phantom{1} -\frac{21}{4} \phantom{1} \fbox{$-\frac{45}{8}$}
\end{array}
$  
...
$
\begin{array}{r}
 1 \quad |\quad 2 \quad 4 \phantom{1} -1 \phantom{1} -6 \phantom{1} -3 \phantom{1}\\
 \underline{\downarrow \phantom{.}\quad 2 \quad\phantom{1} 6 \phantom{1}\quad 5 \quad -1} \phantom{.}\\
 \quad \quad 2 \quad\phantom{1} 6 \quad\phantom{1} 5 \phantom{1} -1 \phantom{1} \fbox{$-4$}
\end{array}
$ 
...
$
\begin{array}{r}
 \frac{3}{2} \quad |\quad 2 \quad 4 \quad -1 \quad -6 \phantom{1}\quad -3 \phantom{1}\\
 \underline{\downarrow \quad 3 \quad\phantom{1} \frac{21}{2} \phantom{1}\quad \frac{57}{4} \quad\phantom{1.} \frac{99}{8}} \phantom{.}\\
 \quad \quad 2 \quad\phantom{1} 7 \quad\phantom{1} \frac{19}{2} \quad\phantom{1.} \frac{33}{4} \quad \fbox{$\frac{75}{8}$}
\end{array}
$ 
\end{tabular}
\end{center}
...
$
\begin{array}{r}
 -\frac{1}{2} \quad |\quad 2 \quad 4 \quad -1 \quad -6 \phantom{11} -3 \phantom{1}\\
 \underline{\downarrow \phantom{.} -1 \phantom{1} -\frac{3}{2} \phantom{1.}\quad \frac{5}{4} \quad\phantom{1.} \frac{19}{8}} \phantom{.}\\
 \quad \quad 2 \quad\phantom{1} 3 \quad -\frac{5}{2}  -\frac{19}{4} \phantom{1} \fbox{$-\frac{5}{8}$}
\end{array}
$  
...
\begin{array}{r}
 -1 \quad |\quad 2 \quad\quad 4 \quad\quad -1 \quad\quad -6 \quad\quad -3\\
 \underline{\downarrow \phantom{.}\quad  -2 \quad\quad -2\quad\quad\quad 3 \quad\quad\quad 3}\\
 -1 \quad |\quad 2 \quad\quad 2 \quad\phantom{11} -3 \quad\quad -3 \quad\quad \fbox{0}\\
 \underline{\downarrow \phantom{1.} -2 \quad\phantom{1111} 0\quad\phantom{1111} 3 \phantom{\quad\quad \fbox{0}}}\\
2 \phantom{1.}\quad\quad 0 \phantom{1.}\quad -3 \phantom{1.}\quad \fbox{0} \phantom{\quad\quad \fbox{0}}
\end{array}
$  
...
% https://www.desmos.com/calculator/mwvyebekgp
\begin{center}
\desmos{mwvyebekgp}{800}{600}
\end{center}
...
$
\begin{array}{r}
 1 \quad |\quad 2 \phantom{11} -3 \quad\quad 6 \phantom{.}\quad -8 \quad\quad 0 \quad\quad 3\\
 \underline{\downarrow \phantom{1}\quad  2 \quad -1\quad\quad 5 \quad -3 \quad -3}\\
 1 \quad |\quad 2 \quad\phantom{1} -1 \quad\phantom{1} 5 \quad -3 \quad -3 \quad \fbox{0}\\
 \underline{\downarrow \phantom{11}\quad  2 \quad\quad 1\quad\quad\phantom{.} 6 \quad\quad\phantom{.} 3 \phantom{\quad \fbox{0}}}\\
 -\frac{1}{2} \quad |\quad 2 \quad\quad 1 \quad\phantom{11} 6 \quad\quad 3 \quad\quad \fbox{0} \phantom{\quad \fbox{0}}\\
 \underline{\downarrow \phantom{1.} -1 \quad\phantom{1} 0 \quad\phantom{1} -3 \phantom{\quad \fbox{0} \quad \fbox{0}}}\\
2 \phantom{1.}\quad 0 \phantom{1.}\quad 6 \phantom{11}\quad \fbox{0} \phantom{\quad\quad \fbox{000}}
\end{array}
$   
\end{center}
...
\node[anchor=north, gray] at (-0.5,1) {$0$};
\node[anchor=north, gray] at (1,1) {$0$};

\draw[->, gray] (0.25,-1.5)--(0.25,-0.5);
\node[anchor=north, gray] at (0.25,-2) {$0$};
...
\item To interpret this solution graphically, we set $f(x) = 2x^5+6x^3+3$ and $g(x) = 3x^4+8x^2$.  Recall from Section \ref{AbsoluteValueFunctions} the solution to $f(x) \leq g(x)$ is the set of $x$ values for which the graph of $f$ is below the graph of $g$ (where $f(x) < g(x)$) along with the $x$ values where the two graphs intersect ($f(x) = g(x)$).  Graphing $f$ and $g$ using Desmos, we see that the graph of $f$ is below the graph of $g$ on $\left(-\infty, -\frac{1}{2}\right)$. However, it is difficult to see what is happening near $x=1$.  Zooming in near $(1,11)$, we see that the graphs of $f$ and $g$ do intersect at $x=1$, but the graph of $g$ remains below the graph of $f$ on either side of $x = 1$.

% https://www.desmos.com/calculator/jt9mkygqqz
\begin{center}
\desmos{jt9mkygqqz}{800}{600}
\end{center} 
...
Next up is an application problem torn from Exercises \ref{lcdmaxprofitexerfirst} - \ref{lcdmaxprofitexerlast} of Section \ref{GraphsofPolynomials}.
...
\begin{tikzpicture}[scale=1]font=\Large
        \begin{axis}[
          domain=0:10.1,
          xmin=-0.13, xmax=10.13,
         ymin=-3, ymax=1.5,
          width=4in,
          height=2in,
          xtick={2.41, 5},
          xticklabels={$1+\sqrt{2}$, $5$},
          tick style={line width=1.2pt},
            axis x line=center,
            axis y line=none,
...
\node[anchor=north, red] at (3.75,1) {$(+)$};
\node[anchor=north, blue] at (9.5,1) {$(-)$};

\draw[->, gray] (0.05,-1.5)--(0.05,-0.5);
\node[anchor=north, gray] at (0.05,-2) {$0$};
\draw[->, gray] (3.75,-1.5)--(3.75,-0.5);
\node[anchor=north, gray] at (3.75,-2) {$3$};
\draw[->, gray] (10,-1.5)--(10,-0.5);
\node[anchor=north, gray] at (9.4,-2) {$10.07$};

\draw[fill=black] (0,0) circle[radius=2.95pt];
\draw[fill=black] (10,0) circle[radius=2.95pt];
\node[anchor=north, gray] at (2.41,1) {$0$};
\node[anchor=north, gray] at (5,1) {$0$};
