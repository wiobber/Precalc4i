        ...
        \begin{axis}[
          ...
          xtick={-4,-3,-2,-1,1,2,3,4},
           ...
           ytick={-4,-3,-2,-1,1,2,3,4},
            yticklabels={$-4i$,$-3i$, $-2i$, $-i$, $i$, $2i$,$3i$, $4i$},  
...
coordinates {
  (-4,2) [$(-4, 2)\leftrightarrow z=-4+2i$]
  (3,0) [$(3, 0)\leftrightarrow z=3$]
};
...
$
\begin{array}{r}
 1+2i \quad |\quad1 \quad\quad\quad 2 \quad\quad\quad 5 \\
 \underline{\downarrow \phantom{1}\quad  1+2i \quad -5}\\
1 \phantom{1}\quad 1-2i \phantom{111} \fbox{0}
\end{array}
$    
\end{center}
...
%% \colorbox{ResultColor}{\bbm
\begin{theorem} \label{complexfactorization} \textbf{Complex Factorization Theorem:} Suppose $f$ is a polynomial function with complex number coefficients.  If the degree of $f$ is $n$ and $n \geq 1$, then  $f$ has exactly $n$ complex zeros, counting multiplicity.  If $z_1$, $z_2$, \ldots, $z_k$ are the distinct zeros of $f$, with multiplicities $m_1$, $m_2$, \ldots, $m_k$, respectively, then $f(x) = a\left(x - z_1  \right)^{m_1}\left(x - z_2  \right)^{m_2} \cdots \left(x - z_k  \right)^{m_k}$. \index{Complex Factorization Theorem}
...
$
\begin{array}{r}
 \frac{1}{2} \quad |\quad 12 \quad -20 \phantom{11}\quad 19 \quad -6 \quad -2 \phantom{11}\quad 1\\
 \underline{\downarrow \phantom{11}\quad 6 \phantom{1}\quad -7 \phantom{111}\quad 6 \quad\phantom{11} 0 \quad -1}\\
 \frac{1}{2} \quad |\quad 12 \quad -14 \phantom{11}\quad 12 \phantom{11}\quad 0 \quad -2 \phantom{1}\quad \fbox{0}\\
 \underline{\downarrow \phantom{11}\quad 6 \phantom{1}\quad -4 \phantom{11}\quad 4 \quad\phantom{11} 2 \phantom{\fbox{.} \quad \fbox{0}}}\\
 -\frac{1}{3} \quad |\quad 12 \phantom{1.}\quad -8 \quad\quad 8 \quad\quad 4 \quad\quad \fbox{0} \phantom{1 \quad \fbox{0}}\\
 \underline{\downarrow \phantom{1}\quad -4 \quad\phantom{11} 4\quad -4 \phantom{\fbox{0} \quad \fbox{0} \quad \fbox{0}}}\\
12 \phantom{1} -12 \phantom{1.}\quad 12 \phantom{1}\quad \fbox{0} \phantom{\fbox{0} \quad \fbox{0} \quad \fbox{0}}
\end{array}
$    
\end{center}
...
$ f(x) = a_{n} x^{n} + a_{n-1} x^{n-1} + \ldots + a_2 x^2 + a_1 x + a_0$ be a polynomial function with real number coefficients.  If $z$ is a zero of $f$, then $f(z) = 0$, which means $a_{n} z^{n} + a_{n-1} z^{n-1} + \ldots + a_2 z^2 + a_1 z + a_0 = 0$.  Next, we consider $f\left(\overline{z}\right)$ and apply Theorem \ref{conjugateprops} below.

\[ \begin{array}{rclr}

 f\left(\overline{z}\right) & = &  a_{n} \left(\overline{z}\right)^{n} + a_{n-1} \left(\overline{z}\right)^{n-1} + \ldots + a_2\left( \overline{z}\right)^2 + a_1 \overline{z} + a_0 & \\ [3pt]
 
 &  = & a_{n}\overline{z^{n}} + a_{n-1}\overline{z^{n-1}} + \ldots + a_2\overline{z^2} + a_1 \overline{z} + a_0 & \mbox{ since $\left(\overline{z}\right)^n = \overline{z^{n}}$}\\ [3pt]
 
 & = & \overline{a_{n}}\overline{z^{n}} + \overline{a_{n-1}}\overline{z^{n-1}} + \ldots +  \overline{a_2}\overline{z^2} + \overline{a_1}\, \overline{z} + \overline{a_0} & \mbox{since the coefficients are real} \\ [3pt]
 
 & = & \overline{a_{n} z^{n}} + \overline{a_{n-1} z^{n-1}} + \ldots +  \overline{a_2 z^2} + \overline{a_1 z} + \overline{a_0} &  \mbox{ since $\overline{z} \, \overline{w}=\overline{zw} $}\\ [3pt]
 
 & = & \overline{a_{n} z^{n} + a_{n-1} z^{n-1} + \ldots + a_2 z^2 + a_1 z + a_0} & \mbox{ since $ \overline{z} + \overline{w} = \overline{z+w} $}\\ [3pt]
...
$
\begin{array}{r}
 2+2i \quad |\quad 1 \phantom{1.}\quad\quad\quad 0 \quad\quad\quad\quad 0 \quad\quad\quad\quad 0 \quad\quad\quad\quad 64\\
 \underline{\downarrow \phantom{1.}\quad\quad  2+2i \quad\phantom{11.} 8i\quad\phantom{1} -16+16i \quad -64}\\
 \quad 1 \phantom{11}\quad\quad 2+2i \quad\phantom{11.} 8i \quad\quad -16+16i \quad \phantom{11}\fbox{0}\\
\end{array}
$    
\end{center}
...
\begin{array}{r}
 2+2i \quad |\quad 1 \phantom{1.}\quad\quad\quad 0 \quad\quad\quad\quad 0 \quad\quad\quad\quad 0 \quad\quad\quad\quad 64\\
 \underline{\downarrow \phantom{1.}\quad\quad  2+2i \quad\phantom{11.} 8i\quad\phantom{1} -16+16i \quad -64}\\
 2-2i \quad |\quad 1 \phantom{11}\quad\quad 2+2i \quad\phantom{11.} 8i \quad\quad -16+16i \quad   \fbox{0}\\
 \underline{\downarrow \phantom{1}\quad  2-2i \quad\phantom{11.} 8-8i \quad\quad 16-16i \phantom{\quad \fbox{0}}}\\
1 \phantom{11}\quad\quad 4 \phantom{11}\quad\quad\quad 8 \phantom{11}\quad\quad \quad \fbox{0}  \phantom{\phantom{1}\quad\quad \fbox{0}}
\end{array}
$    
\end{center}
...
\addplot[smooth, line width=3pt, blue] {(1/3)*(x+3)*(x-1)^2};
