...
\end{center}
 ...
 We now turn our attention to the problem at hand:  finding the zeros of a polynomial function.  Suppose we wish to determine the zeros of  $f(x) = x^3 + 4x^2-5x-14$.  Setting $f(x)=0$ results in the polynomial equation $x^3 + 4x^2-5x-14=0$.   Despite all of the factoring techniques we learned (and forgot!) in Intermediate Algebra, this equation foils\footnote{pun intended} us at every turn. Knowing that the zeros of $f$ correspond to $x$-intercepts on the graph of $y=f(x)$, Desmos to graph $f$ and identify the $x$-intercepts we can see.\footnote{Are these \textbf{all} the $x$-intercepts?  Stay tuned!}
...
x & - & 2 \, \vline& x^3 & + & 4x^2 & - & 5x & - & 14 \\
 ...
 &  &  -& \left(x^3 \right. & - & \left.  2x^2\right) &  &  &  &  \\ \hline 
 &  &  &   &  & 6 x^2 & - & 5x &  &  \\ 
 &  &  &   & - & \left(6 x^2 \right. & - & \left. 12x \right) &  &  \\ \hline 
 &  &  &   &   &  & & 7x  & - & 14 \\
 &  &  &   &   &  & - & \left( 7x \right. & - & \left. 14 \right) \\ \hline 
 &   &  &  &  &  &  &  &  & 0
...
\begin{array}{r}
 \quad x^2 + 6x + 7 \\
 x  - 2 \quad\overline{)\, x^3 +  4x^2  -  5x  -  14} \\
