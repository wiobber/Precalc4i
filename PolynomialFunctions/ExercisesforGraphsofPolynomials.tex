...
\begin{document}
	\author{Stitz-Zeager}
	\xmtitle{Exercises for Graphs of Polynomials}{}
...
$\text{Leading term} = \answer{3x^{5}}$

$\text{Leading coefficient} = \answer{3}$

$\text{Constant term} = \answer{1}$

$\lim_{x \rightarrow - \infty} g(x) = $ \wordChoice{\choice[correct]{$-\infty$}, \choice{$\infty$}}

$\lim_{x \rightarrow  \infty} g(x)  = $ \wordChoice{\choice{$-\infty$}, \choice[correct]{$\infty$}}
...
$\text{Leading term} = \answer{-b^{3}}$

$\text{Leading coefficient} = \answer{-1}$

$\text{Constant term} = \answer{0}$

$\lim_{b \rightarrow - \infty} Z(b) = $ \wordChoice{\choice{$-\infty$}, \choice[correct]{$\infty$}}

$\lim_{b \rightarrow  \infty} Z(b)  = $ \wordChoice{\choice[correct]{$-\infty$}, \choice{$\infty$}}
...
\begin{problem}
Find the degree, the leading term, the leading coefficient, the constant term and the end behavior of the polynomial function $s(t) = -4.9t^{2} + v_0t + s_0$.  



$\text{Degree} = \answer{2}$

$\text{Leading term} = \answer{-4.9t^{2}}$

$\text{Leading coefficient} = \answer{-4.9}$

$\text{Constant term} = s_0$

$\lim_{t \rightarrow - \infty} s(t) = $ \wordChoice{\choice[correct]{$-\infty$}, \choice{$\infty$}}
...
$\text{Leading term} = \answer{5t^{5}}$

$\text{Leading coefficient} = \answer{5}$

$\text{Constant term} = \answer{0}$

$\lim_{t \rightarrow - \infty} p(t) = $ \wordChoice{\choice[correct]{$-\infty$}, \choice{$\infty$}}

$\lim_{t \rightarrow  \infty} p(t)  = $ \wordChoice{\choice{$-\infty$}, \choice[correct]{$\infty$}}
...
$\text{Leading term} = \answer{4t^{3}}$

$\text{Leading coefficient} = \answer{4}$

$\text{Constant term} = \answer{8}$

$\lim_{t \rightarrow - \infty} G(t) = $ \wordChoice{\choice[correct]{$-\infty$}, \choice{$\infty$}}

$\lim_{t \rightarrow  \infty} G(t)  = $ \wordChoice{\choice{$-\infty$}, \choice[correct]{$\infty$}}
...
% problems 21-30 only have solutions for zeroes and multiplicity, graphs are missing
...
\wordChoice{\choice[correct]{even},\choice[correct]{odd}, \choice{neither}}
\end{problem}
...
\begin{solution}
    odd\footnote{You need to first multiply out the expression for $g(x)$ so it is in the form prescribed by Definition \ref{polynomialfunction}.}
\end{solution}
...
\begin{solution}
    For $f(x) = |x|$, $f(-x) = |-x| = |(-1) x|  = |-1| |x| = (1) |x| = |x|$.  Hence, $f(-x) = f(x)$.
\end{solution}
...
\begin{solution}
    $V(x) = x(8.5-2x)(11-2x) = 4x^3-39x^2+93.5x$, $0 < x < 4.25$.  Volume is maximized when $x \approx 1.58$, so we get the dimensions of the box with maximum volume are: height $\approx$ 1.58 inches, width $\approx$ 5.34 inches, and depth $\approx$ 7.84 inches.  The maximum volume is $\approx$ 66.15 cubic inches.
\end{solution}
...
\begin{solution}
As we sample points closer to $x=1$, the slope of the curve approaches the exponent on $x$.
...
\begin{solution}
    To maximize the volume, we assume we start with the maximum Length $+$ Girth of $130$,  so the length is $130 - 4x$.  The volume of a rectangular box is  `length $\times$ width $\times$ height' so we get $V(x) = x^{2}(130 - 4x) = -4x^{3} + 130x^{2}$.  
\end{solution}

\item \label{girthbox2} Find the dimensions of the box of maximum volume that can be shipped via Parcel Post.

\begin{solution}
    Using a graphing utility, we get a (local) maximum of  $y = V(x)$ at $(21.67, 20342.59)$.  Hence, the maximum volume is $20342.59\mbox{in.}^{3}$ using a box with dimensions $21.67\mbox{in.} \times 21.67\mbox{in.} \times 43.32\mbox{in.}$.
\end{solution}
...
\begin{solution}
    The cubic regression model is $p_{\mbox{\tiny $3$}}(x) = 0.0226x^{3} - 0.9508x^{2} + 8.615x - 3.446$.  It has $R^{2} = 0.9377$ which isn't bad.  The graph of $y = p_{\mbox{\tiny $3$}}(x)$ along with the data is shown below on the left.  Note $p_{\mbox{\tiny $3$}}$ hits the $x$-axis at about $x = 12.45$ making this a bad model for future predictions. 
\end{solution}

\begin{solution}
    The quartic regression model is $p_{\mbox{\tiny $4$}}(x) = 0.0144x^{4} - 0.3507x^{3} + 2.259x^{2} - 1.571x + 5.513$.  It has $R^{2} = 0.9859$ which is good.  The graph of $y = p_{\mbox{\tiny $4$}}(x)$  along with data is shown below on the right.  Note  $p_{\mbox{\tiny $4$}}(15)$ is above $24$ making this a bad model as well for future predictions.  
\end{solution}
...
\begin{solution}
    The quadratic model is $P_{\mbox{\tiny $2$}}(x) = -0.021x^{2} + 0.241x + 0.956$, $R^{2} = 0.7771$. \\
...
\begin{solution}
     $\ds{\lim_{x \rightarrow - \infty} p(x)  = -\infty}$  and $\ds{\lim_{x \rightarrow  \infty} p(x)  = -\infty}$
...
\begin{solution}
    The zeros appear to be: $x=-1.5$, even multiplicity - probably $2$ since it doesn't `look like' the graph is very flat near $x = 2$;  $x=0$, odd multiplicity - probably $1$ since the graph seems fairly linear as it passes through the origin;  $x=1$ odd multiplicity - probably $3$ or higher since the graph seems fairly `flat' near $x = 1$.
\end{solution}
...
\begin{solution}
     We only have a \textit{portion} of the graph represented here.
\end{solution}
...
\begin{problem}  Use the graph of $y= p(x) = (2x-1)(x+1)(1-x^4)$ (the graph which directly precedes Example \ref{boxnotopex}) to estimate the largest open interval containing $x = -0.235$ which satisfies the the criteria for `local minimum'  in Definition \ref{localmaxmindefn}.

\begin{solution}
    We are looking for the largest open interval containing $x = -0.235$ for which the graph of $y = p(x)$ is at or above $y=-1.121$.  Since each of the gridlines on the $x$-axis correspond to $0.2$ units, we approximate this interval as  $(-1.25 \, \text{ish}, 1.1 \, \text{ish})$.
\end{solution}
\end{problem}
...
(This is a follow-up to Exercises \ref{LagrangeLinearExercise} in Section \ref{LinearFunctions} and \ref{LagrangeQuadExercise} in Section \ref{QuadraticFunctions}.) The  \href{https://en.wikipedia.org/wiki/Lagrange_polynomial}{\underline{Lagrange Interpolate}} function $L$  for four  points:  $(x_{0}, y_{0})$, $(x_{1}, y_{1})$,  $(x_{2}, y_{2})$,   $(x_{3}, y_{3})$ where $x_{0}$,  $x_{1}$, $x_{2}$, and $x_{3}$ are four distinct real numbers is given by the formula: 
