...
In Sections \ref{ConstantFunctions}, \ref{LinearFunctions}. \ref{AbsoluteValueFunctions} and \ref{QuadraticFunctions}, we studied constant, linear,  absolute value,\footnote{These were introduced, as you may recall, as piecewise-defined linear functions.} and quadratic functions. Constant, linear and quadratic functions were specific examples of polynomial functions, which we studied extensively from Section \ref{GraphsofPolynomials} on through Section \ref{ComplexZeros}.  One of the major results from that last section is Theorem \ref{realfactorization}, which says that all polynomial functions with real coefficients can be thought of as products of linear and quadratic functions.  

Our next step was to enlarge our field\footnote{This is a really bad math pun.} of study to rational functions which were introduced in Section \ref{IntroRational}.  Being quotients of polynomials, we can ultimately view this family of functions as being built up of linear and quadratic functions as well.  So in some sense, those early sections along with chapters on  polynomial and rational functions can be thought of as an exhaustive study of linear and quadratic\footnote{If we broaden our concept of functions to allow for complex valued coefficients, the Complex Factorization Theorem, Theorem \ref{complexfactorization}, tells us every function we have studied thus far is a combination of linear functions.} functions.  We now turn our attention to functions involving radicals which cannot be written in terms of linear functions.  For a more detailed review of the basics of roots and radicals, we refer the reader to Sections \ref{AppRealNumberArithmetic} and  \ref{AppRadEqus}.  
...
% % \input{RootRadicalFunctions_pic2.tex}
% \begin{tikzpicture}
% \begin{axis}[
%   xmin=0, xmax=9,
%   ymin=0, ymax=3,
...
% % \input{RootRadicalFunctions_pic3.tex}
% \begin{tikzpicture}
% \begin{axis}[
%   xmin=0, xmax=9,
%   ymin=0, ymax=3,
...
% % \input{RootRadicalFunctions_pic4.tex}
% \begin{tikzpicture}
% \begin{axis}[
%   xmin=-4, xmax=4,
%   ymin=-3, ymax=3,
...
% % \input{RootRadicalFunctions_pic5.tex}
% \begin{tikzpicture}
% \begin{axis}[
%   xmin=-4, xmax=4,
%   ymin=-3, ymax=3,
...
% % \input{RootRadicalFunctions_pic6.tex}
% \begin{tikzpicture}
% \begin{axis}[
%   xmin=-4, xmax=4,
%   ymin=-3, ymax=3,
...
% https://www.desmos.com/calculator/6cw2sm1l0h

\begin{center} 
\desmos{6cw2sm1l0h}{800}{600} 
\end{center}
...
\item  We begin by rewriting the expression for $f(x)$ in the form prescribed Theorem \ref{linearrootgraphs}:  $f(x) = -2 \sqrt[3]{x+3} + 1$.  We identify $n=3$, $a = -2$, $b = 1$, $h = -3$ and $k = 1$.  GeoGebra helps us visualize the steps prescribed in Theorem \ref{linearrootgraphs}  to transform the graph of $y = \sqrt[3]{x}$ to the graph of $y=f(x)$.  We track the points $(-1,1)$, $(0,0)$ and $(1,1)$ through each step.
...
% % \input{RootRadicalFunctions_pic7.tex}
% \begin{tikzpicture}
% \begin{axis}[
%   xmin=-4, xmax=4,
%   ymin=-3, ymax=3,
...
% % \input{RootRadicalFunctions_pic8.tex}
% \begin{tikzpicture}
% \begin{axis}[
%   xmin=-7, xmax=1,
%   ymin=-3, ymax=3,
...
% % \input{RootRadicalFunctions_pic9.tex}
% \begin{tikzpicture}
% \begin{axis}[
%   xmin=-7, xmax=1,
%   ymin=-3, ymax=3,
...
% % \input{RootRadicalFunctions_pic10.tex}
% \begin{tikzpicture}
% \begin{axis}[
%   xmin=-7, xmax=1,
%   ymin=-6, ymax=6,
...
% % \input{RootRadicalFunctions_pic11.tex}
% \begin{tikzpicture}
% \begin{axis}[
%   xmin=-7, xmax=1,
%   ymin=-6, ymax=6,
...
% % \input{RootRadicalFunctions_pic12.tex}
% \begin{tikzpicture}
% \begin{axis}[
%   xmin=-7, xmax=1,
%   ymin=-5, ymax=7,
...
%  \end{array} \]
 ...
 % https://www.geogebra.org/classic/gfjgcbsp

 \begin{center}
     \geogebra{gfjgcbsp}{800}{600}
 \end{center}
 
 We get the domain and range of $f$ are $(-\infty, \infty)$.
 
 \item  For $g(t) = \dfrac{\sqrt{1-2t}}{4} = \frac{1}{4} \sqrt{-2t+1}$, we identify $n=2$, $a = \frac{1}{4}$, $b = -2$, $h = -1$ and $k =0$.  Once again, GeoGebra helps us visualize the the steps taken to transform the graph of $y = \sqrt{t}$ to the graph of $y = g(t)$. Since we are asked to label \textit{three} points on the graph, we track $(4,2)$ along with $(0,0)$ and $(1,1)$.\footnote{As $\sqrt{4} = 2$, we know $(4,2)$ is on the graph of $y = \sqrt{t}$.}
...
% % \input{RootRadicalFunctions_pic13.tex}
% \begin{tikzpicture}
% \begin{axis}[
%   xmin=0, xmax=9,
%   ymin=0, ymax=3,
...
% % \input{RootRadicalFunctions_pic14.tex}
% \begin{tikzpicture}
% \begin{axis}[
%   xmin=-1, xmax=8,
%   ymin=0, ymax=3,
...
% % \input{RootRadicalFunctions_pic15.tex}
% \begin{tikzpicture}
% \begin{axis}[
%   xmin=-1, xmax=8,
%   ymin=0, ymax=3,
...
% % \input{RootRadicalFunctions_pic16.tex}
% \begin{tikzpicture}
% \begin{axis}[
%   xmin=-4, xmax=2,
%   ymin=0, ymax=3,
...
% https://www.geogebra.org/classic/q5zycxxw

\begin{center}
    \geogebra{q5zycxxw}{800}{600}
\end{center}
...
We graph $y = g(t)$ below using Desmos. The graph confirms our suspicions about the asymptotes $t = -1$ and $y = 2$.  Moreover, the range appears to be $(-\infty, 2) \cup (2, \infty)$.  

% https://www.desmos.com/calculator/utcpj39vk8
\begin{center}
    \desmos{utcpj39vk8}{800}{600}
\end{center}
...
To create a sign diagram for $g(t)$, we note that the function is undefined when $t = -1$ (so we place a `\textinterrobang' above it) and has a zero $t=0$.  When  $t<-1$, $g(t) > 0$ or $(+)$, for $-1<t<0$, $g(t)<0$ or $(-)$, and for $t>0$, $g(t) > 0$ or $(+)$.   The sign diagram for $g(t)$ is below.

\begin{center}
% \input{RootRadicalFunctions_pic20.tex}
\begin{tikzpicture}
% Number line with arrows at both ends
\draw[<->] (-4,0) -- (2,0);
...
Moving on to end behavior, as $x \rightarrow -\infty$ or  $x \rightarrow \infty$, the term $x^2$ is the dominant term in the radicand in the denominator. As such, $h(x) = \frac{3x}{\sqrt{x^2 + 1}} \approx \frac{3x}{\sqrt{x^2}} = \frac{3x}{|x|}$.  As $x \rightarrow -\infty$, $|x| = -x$ (since $x<0$) and hence, $h(x)\approx  \frac{3x}{-x} = -3$, so $\ds{\lim_{x \rightarrow -\infty} h(x) =  -3}$.   As $x \rightarrow \infty$, $|x| = x$ (since $x>0$), so $h(x) \approx \frac{3x}{x} = 3$, so $\ds{\lim_{x \rightarrow \infty} h(x) =  3}$. 
  ...
  This analysis suggests the graph of $y=h(x)$ has not one, but \textit{two} horizontal asymptotes.\footnote{We warned you this was coming \ldots see the discussion following Theorem \ref{hathm} in Section \ref{IntroRational}.} The graph of $h$ below bears this out.

  % https://www.desmos.com/calculator/gkloclketw
\begin{center}
    \desmos{gkloclketw}{800}{600}
\end{center}
...
Since the domain of $h$ is all real number and the only zero of $h$ is $x=0$, the sign diagram for $h(x)$ is fairly straight forward.  For $x<0$, $h(x)<0$ or $(-)$ and for $x>0$, $h(x) >0$ or $(+)$.  The sign diagram for $h(x)$ is below.

\begin{center}

% \input{RootRadicalFunctions_pic21.tex}
\begin{tikzpicture}
% Number line with arrows at both ends
\draw[<->] (-2,0) -- (2,0);
...
We graph $y=r(t)$ below.  We see the range appears to be all real numbers, $(-\infty, \infty)$.  It appears as if $r$ is increasing on $\left(-\infty, -\frac{1}{2} \right]$ and again on $\left[\frac{1}{2}, \infty \right)$.  The graph does appear to be asymptotic to $y = 4t$, and it also appears to be symmetric about the origin.  Sure enough, we find  $r(-t) = \frac{\sqrt{16(-t)^4-1}}{-t}  = - \frac{\sqrt{16t^4-1}}{t} = -r(t)$, proving $r$ is an odd function.

% https://www.desmos.com/calculator/usuiezrrvm
\begin{center}
    \desmos{usuiezrrvm}{800}{600}
\end{center}



To construct the sign diagram for $r(t)$ we note $r$ has two zeros, $t = \pm \frac{1}{2}$.  For $t < \frac{1}{2}$, $r(t) < 0$ or $(-)$ and when $t > \frac{1}{2}$, $r(t) > 0$ or $(+)$.  When $-\frac{1}{2} < t < \frac{1}{2}$, $r$ is undefined so we have removed that segment from the diagram, as seen below.

\begin{center}

 % \input{RootRadicalFunctions_pic22.tex}
\begin{tikzpicture}
% Directed segments
\draw[->] (2,0) -- (0,0);
\draw[->] (3,0) -- (5,0);
...
\begin{center}
% \input{RootRadicalFunctions_pic23.tex}
\begin{tikzpicture}
%\fill[white](-2,-2) rectangle (12,9.5);
% Marks on the x-axis
\draw (0,0.1) -- (0,-0.1);
%\draw (4,0.1) -- (4,-0.1);
...
\draw[<->] (0.25,0) -- (3.75,0);
\draw[-, red, line width=1.5pt] (4.,0) -- (9.75,0);

% Bottom route line with arrows
\draw[<->] (0,-1) -- (10,-1);

% Diagonal
\draw[-, red, line width=1.5pt] (4,0) -- (0.25,5.75);

% Junction Box Point
%\fill (0,6) circle (5pt);
\fill (10,0) circle (5pt);
\fill (0.12,6) circle (5pt);

% Labels
\node at (0.75,6.25){\scriptsize Outpost};
%\node[inner sep=0pt, anchor=south west] (cabin) at (-1.75,5.75)
%  {\includegraphics[width=40mm]{./RootRadicalFunctionsGraphics/observationStation.png}};
\node at (11,0){\scriptsize Junction Box};
\node at (7,0.25){$x$};
\node at (2,0.25){$y$};
\node at (2.5,2.7){$z$};
\node at (5,-.5){\scriptsize Route $117$};
\node at (5,-1.5){\scriptsize $50$ miles};
\node[rotate=90] at (-0.5,0){\hspace{2in} \scriptsize $30$ miles};
\end{tikzpicture}
...
\item  As we adjust the slider for $x$ on the  GeoGebra interactive below,  we can not only see the path and cost breakdown for the internet installation, we also see the corresponding point on the graph of $y = C(x)$: 


% https://www.geogebra.org/classic/usjkbqza

\begin{center}
\geogebra{usjkbqza}{800}{600}
\end{center}
...
We find our (local) minimum to be at the point $(15.98, 1146.86)$.  Here the $x$-coordinate tells us that in order to minimize cost, we should run $15.98$ miles of cable along Route 117 and then turn off of the road and head towards the outpost. The $y$-coordinate tells us that the minimum cost, in dollars, to do so is $\$1146.86$.  The ability to stream live SasquatchCasts?  Priceless.
