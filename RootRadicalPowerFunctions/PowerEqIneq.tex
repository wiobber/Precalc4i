...
Geometrically, we are looking for where the graph of $f(x) = (7-x)^{\frac{3}{2}}$ intersects the graph of $g(x) = 8$.  While we could sketch both curves by hand and gauge the reasonableness of the result,\footnote{consider this an exercise!} we are instructed to use a graphing utility.  Using desmos, we see the graphs intersect at one point, $(3,8)$, thereby checking our solution $x = 3$.

% https://www.desmos.com/calculator/dj9jqftbzt

\begin{center}
    \desmos{dj9jqftbzt}{800}{600}
\end{center}
...
(\sqrt[3]{2t-1})^2 - 4 & = & 0 & \text{rewrite using the defintion of rational exponents} \\
...
% https://www.desmos.com/calculator/ewrprzbizv
\begin{center}
    \desmos{ewrprzbizv}{800}{600}
\end{center} 
...
% https://www.desmos.com/calculator/8p6vpfcgug
\begin{center}
    \desmos{8p6vpfcgug}{800}{600}
\end{center}
...
% https://www.desmos.com/calculator/pvhx6w0fep
\begin{center}
    \desmos{pvhx6w0fep}{800}{600}
\end{center} 
...
\frac{2}{(3x-1)^{0.5}} & = & \frac{3x}{(3x-1)^{1.5}} & \\

\left[ \frac{2}{(3x-1)^{0.5}} \right] \cdot (3x-1)^{1.5} & = & \left[  \frac{3x}{(3x-1)^{1.5}} \right ] \cdot (3x-1)^{1.5} & \\

2 \cdot \frac{(3x-1)^{1.5}}{(3x-1)^{0.5}} & = & 3x & \\

2 (3x-1)^{1.5-0.5} & = & 3x & \text{we can subtract exponents since $3x-1 > 0$.} \\
...
% https://www.desmos.com/calculator/ltnevjtebq
\begin{center}
    \desmos{ltnevjtebq}{800}{600}
\end{center}
...
6(9-t^2)^{\frac{1}{3} + \frac{2}{3}}& = & 4t^2 &  \text{we can add exponents since the root here $3$ is odd.} \\
...
% https://www.desmos.com/calculator/3li5pqrdxu
\begin{center}
    \desmos{3li5pqrdxu}{800}{600}
\end{center} 
...
(7-x)^{\frac{3}{2}} & = & 8 & \\
 ...
 (7-x)^{\frac{3}{2} \cdot \frac{2}{3}} & = & 4 & \text{we can multiply exponents since the root here, 3, is odd} \\
...
\frac{231}{1.01 L^{0.75}} & = &  \frac{1.01 L^{0.75} K^{0.25}}{1.01 L^{0.75}} & \text{$L>0$, hence $L^{0.75} \neq 0$.}\\

K^{0.25} & = & 228.\overline{7128}L^{-0.75} & \text{rewrite} \\

\left(K^{0.25}\right)^{\frac{1}{0.25}} & = & \left(228.\overline{7128}L^{-0.75}\right)^{\frac{1}{0.25}} & \\

K^{\frac{0.25}{0.25}} & = & (228.\overline{7128})^{\frac{1}{0.25}} L^{-\frac{0.75}{0.25}} & \text{for a power of a power, multiply exponents} \\
...
\item The function $f(L)$ is a Laurent Monomial (see Section \ref{IntroRational}) with $n = 3$ and $a = (228.\overline{7128})^{4}$ and is graphed below on the applied domain, $L>0$.
...
\addplot[smooth,domain=0.2:2.2, line width=1.5pt, blue] {1/(x^3)};
...
\item  Place any values excluded from the domain of  $f$ on the number line.  You may wish to use a dashed line for a vertical asymptote and an open circle for a hole in the graph.
...
\item \label{fourth} $(t-4)^{\frac{2}{3}} \geq -\frac{2t}{3(t-4)^{\frac{1}{3}}}$  
...
% https://www.desmos.com/calculator/tdsuunihkt
\begin{center}
    \desmos{tdsuunihkt}{600}{600}
\end{center}
...
We find $r(t) = t^{\frac{4}{3}} -t^{\frac{2}{3}} - 6 > 0$  on $\left(-\infty, -3 \sqrt{3}\right)\cup \left(3 \sqrt{3}, \infty\right)$.  To check our answer graphically, we set $f(t) = t^{\frac{2}{3}}$ and $g(t) = t^{\frac{4}{3}}-6$.  The solution to  $t^{\frac{2}{3}} < t^{\frac{4}{3}} - 6$ corresponds to the inequality $f(t) < g(t)$, which means we are looking for the $t$ values for which the graph of $f$ is \textit{below} the graph of $g$.  Using desmos, we see the graph of $y = f(t) = t^{\frac{2}{3}}$  is below the graph of $y = g(t) = t^{\frac{4}{3}}-6$ for $t < - 5.196$ and again for $t > 5.196$, which are desmos's approximations to $\pm 3 \sqrt{3}$.



% https://www.desmos.com/calculator/n6gohzoqb0

\begin{center}
    \desmos{n6gohzoqb0}{600}{600}
\end{center}
...
Rewriting $r(x)$ with positive exponents, we obtain \[r(x) =  3 (2-x)^{\frac{1}{3}} - \frac{x}{(2-x)^{\frac{2}{3}}}\]  Setting the denominator equal to zero we get $(2-x)^{\frac{2}{3}} = 0$, which reduces to  $2-x=0$, or $x=2$.  Hence, the domain of $r$ is $(-\infty, 2) \cup (2, \infty)$. 
...
r(x)  & = & 3 (2-x)^{\frac{1}{3}} - x (2-x)^{-\frac{2}{3}} & \\  
      & = & (2-x)^{-\frac{2}{3}} \left[ 3 (2-x)^{\frac{1}{3} - \left(-\frac{2}{3}\right)} - x\right] & \\  
      & = & (2-x)^{-\frac{2}{3}}\left[3(2-x)^{\frac{3}{3}} - x\right] & \\  
      & = & (2-x)^{-\frac{2}{3}}\left[3(2-x)^{1} - x\right] &  \\  
...
r(x)  & = & 3 (2-x)^{\frac{1}{3}} - x (2-x)^{-\frac{2}{3}} & \\  
      & = & 3 (2-x)^{\frac{1}{3}} - \frac{x}{(2-x)^{\frac{2}{3}}} & \\  
      & = & \frac{3 (2-x)^{\frac{1}{3}}(2-x)^{\frac{2}{3}}}{(2-x)^{\frac{2}{3}}} - \frac{x}{(2-x)^{\frac{2}{3}}} & \text{common denominator} \\  
      & = & \frac{3 (2-x)^{\frac{1}{3} + \frac{2}{3}}}{(2-x)^{\frac{2}{3}}} - \frac{x}{(2-x)^{\frac{2}{3}}} & \text{root here, 3, is odd, so add exponents}  \\  
      & = & \frac{3 (2-x)^{\frac{3}{3}}}{(2-x)^{\frac{2}{3}}} - \frac{x}{(2-x)^{\frac{2}{3}}} & \\  
      & = & \frac{3 (2-x)^1}{(2-x)^{\frac{2}{3}}} - \frac{x}{(2-x)^{\frac{2}{3}}} &  \\  
      & = & \frac{3 (2-x) - x}{(2-x)^{\frac{2}{3}}} & \\  
      & = & \frac{6-4x}{(2-x)^{\frac{2}{3}}} & \\
...
% https://www.desmos.com/calculator/k9art9xq0i
\begin{center}
    \desmos{k9art9xq0i}{600}{600}
\end{center}
...
(t-4)^{\frac{2}{3}} & \geq &  - \frac{2t}{3(t-4)^{\frac{1}{3}}} & \\


(t-4)^{\frac{2}{3}} + \frac{2t}{3(t-4)^{\frac{1}{3}}}  & \geq & 0 \\

\frac{ (t-4)^{\frac{2}{3}} \cdot 3(t-4)^{\frac{1}{3}} }{3(t-4)^{\frac{1}{3}}}  + \frac{2t}{3(t-4)^{\frac{1}{3}}} & \geq & 0 & \text{common denominator} \\

\frac{3(t-4)^{\frac{2}{3}+\frac{1}{3}}}{3(t-4)^{\frac{1}{3}}}  + \frac{2t}{3(t-4)^{\frac{1}{3}}} & \geq & 0 & \text{\text{root is odd so add exponents}} \\ 

\frac{3(t-4)^{1}}{3(t-4)^{\frac{1}{3}}}  + \frac{2t}{3(t-4)^{\frac{1}{3}}} & \geq & 0 & \\ 

\frac{3(t-4) + 2t}{3(t-4)^{\frac{1}{3}}}   & \geq & 0 & \\ 

\frac{5t-12}{3(t-4)^{\frac{1}{3}}}   & \geq & 0 & \\ 
...
\xmalt{Sign diagram showing the intervals on which the function $r(x)$ is positive or negative. Dashed vertical line at $x=2$ indicates a vertical asymptote. The function is positive on $(-\infty,\frac{12}{5})$, negative on $(\frac{12}{5},4)$, and positive on $(4,\infty)$. The function has a zero at $x=\frac{12}{5}$.} 
\end{center}
\begin{image}
\begin{tikzpicture}[scale=1]font=\Large
        \begin{axis}[
         ...
         ymin=-3, ymax=1.5,
          ...
          height=2in,
          xtick={3/2, 2},
          xticklabels={$\frac{12}{5}$, $4$},
          tick style={line width=1.2pt},
            axis x line=center,
            axis y line=none,
            x axis line style={{Latex[length=5pt,width=3pt]}-{Latex[length=5pt,width=3pt]}, line width=1pt}
          ]
\node[anchor=north, red] at (1.25,1) {$(+)$};
\node[anchor=north, blue] at (1.75,1) {$(-)$};
\node[anchor=north, red] at (2.25,1) {$(+)$};
...
% https://www.desmos.com/calculator/ipnyncdfbb
\begin{center}
    \desmos{ipnyncdfbb}{600}{600}
\end{center}
...
3 (2-x)^{\frac{1}{3}} & \leq & \frac{x}{(2-x)^{\frac{2}{3}}} & \\

\left[3 (2-x)^{\frac{1}{3}} \right] \left[  (2-x)^{\frac{2}{3}}\right]& \leq & \frac{x}{(2-x)^{\frac{2}{3}}}  \left[  (2-x)^{\frac{2}{3}}\right]&  \text{provided $x \neq 2$} \\
