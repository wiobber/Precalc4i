...
Suppose we take a \textit{portion} of a circle and compare some arc measuring $s$ units in length along the circumference to the radius, $r$.  Let $\theta$ be the \textbf{central angle}\index{angle ! central angle}\index{central angle} subtended by this arc, that is, an angle whose vertex is the center of the circle and whose determining rays pass through the endpoints of the arc.  

Using the GeoGebra interactive below, click and drag the point $P$ to change the length of the arc, $s$  (and hence the angle, $\theta$.) If we keep $\theta$ the same and adjust the radius of the circle, $r$, notice that the ratio $\frac{s}{r}$ remains constant.\footnote{Also note that as we stretch $s$ to cover one circumference (hence one complete revolution for $\theta$) the ratio approaches $2\pi$.} 

% https://www.geogebra.org/classic/nkfhevs7
\begin{center}
\geogebra{nkfhevs7}{800}{600}
\end{center}

The same proportionality (similarity) arguments used to show $\pi$ is a constant shows the ratio $\frac{s}{r}$ is also constant among all circles.  It is this ratio, $\frac{s}{r}$, which defines the \textbf{radian measure} of an angle.

To get a better feel for radian measure, consider the GeoGebra interactive below. Note there are two sliders: one to adjust the radian measure of the central angle, $\theta$, and one to adjust the radius, $r$.  The length of the subtended arc, $s$, is marked off into increments of $r$.  Notice, for instance, that for an angle $\theta$ with radian measure $2$, the length of $s$ is exactly double the radius, $r$.  That is, $s$ sweeps out two `radius lengths' worth of circumference.  Likewise, if $\theta = 3$ radians, every subtended arc sweeps out three radius lengths.

% https://www.geogebra.org/classic/brnjyatt
\begin{center}
\geogebra{brnjyatt}{800}{600}
\end{center}

In general,  the radian measure of an angle $\theta$ tells us how many `radius lengths' we need to sweep out along the circle to subtend the angle $\theta$.  
...
\item  $\alpha = \frac{\pi}{6}$

\item  $\beta = -\frac{4\pi}{3}$

\item  $\gamma = \frac{9 \pi}{4}$

\item  $\phi = - \frac{5 \pi}{2}$
...
Using the interactive below, we can visualize sweeping out $\alpha$ and identifying coterminal angles.  The circle is divided into $12$ equal sectors, each corresponding to a $\frac{\pi}{6}$'s worth of radian measure.  Starting with the slider at $n = 0$, we move the slider to $n = (+) 1$ which sweeps out $\alpha = (+1) \frac{\pi}{6} = \frac{\pi}{6}$ radians from the positive $x$-axis.  

Now that we've found the terminal side for $\alpha$,  we can look for a coterminal angle by moving the slider more to the right (increasing $n$ and continuing to rotate counter-clockwise) until we reach the same terminal side as $\theta$. Our first such instance occurs when $n=13$.  Hence,  $\theta = 13 \frac{\pi}{6} = \frac{13 \pi}{6}$, is a coterminal angle. Notice that we moved $n = 12$ additional $\frac{\pi}{6}$'s, or $12 \frac{\pi}{6} = \frac{12\pi}{6} = 2\pi$ radians, counter-clockwise to reach this coterminal angle.

Continuing to move the slider to the right sweeps out additional $\frac{\pi}{6}$ sectors in the counter-clockwise direction until we find a second (positive) coterminal angle when $n = 25$.  Hence, $\theta = \frac{25\pi}{6}$ is a second positive coterminal angle. Notice that here,  we moved $n = 24$ additional $\frac{\pi}{6}$'s, that is $24 \frac{\pi}{6} = \frac{24\pi}{6} = 4 \pi$ radians, counter-clockwise to reach this coterminal angle.

Next, we reset the slider to $n = 1$, so we're back to $\alpha = \frac{\pi}{6}$, and begin moving the slider to the \textit{left}.  This corresponds to rotating \textit{clockwise} in increments of $\frac{\pi}{6}$.  Eventually we find another coterminal angle when $n = -11$. This value of $n$ corresponds to the coterminal angle $\theta = -\frac{11\pi}{6}$. Notice that in this case, we moved $n = 12$ $\frac{\pi}{6}$'s, in other words $12 \frac{\pi}{6} = \frac{12\pi}{6} = 2\pi$ radians, \textit{clockwise} to reach this coterminal angle.

% https://www.geogebra.org/classic/v42gefvn
\begin{center}
    \geogebra{v42gefvn}{800}{600}
\end{center}
...
As above, the GeoGebra interactive below allows us to view the above process geometrically.

% https://www.geogebra.org/classic/psvy7avh
\begin{center}
\geogebra{psvy7avh}{800}{600}
\end{center}
...
We can confirm our answers using the GeoGebra interactive below.

% https://www.geogebra.org/classic/qc3vqpvj
\begin{center}
    \geogebra{qc3vqpvj}{800}{600}
\end{center}
...
We can visualize and confirm our answers using the interactive below.

% https://www.geogebra.org/classic/hexty8b3
\begin{center}
\geogebra{hexty8b3}{800}{600}
\end{center}
...
It is worth mentioning that we could have plotted the angles in Example \ref{orientedcoterminalradian} by first converting them to degree measure and following the procedure set forth in Example \ref{orientedcoterminaldegree}.  While converting back and forth from degrees and radians is certainly a good skill to have, it is best that you learn to `think in radians' as well as you can `think in degrees'.  The authors would, however, be derelict in our duties if we ignored the basic conversion between these systems altogether.  
...
\item  To convert degree measure to radian measure, multiply by $\frac{\pi \, \text{radians}}{180^{\circ}}$

\item  To convert radian measure to degree measure, multiply by $\frac{180^{\circ}}{\pi \, \text{radians}}$
...
Consider the Unit Circle, $x^2 + y^2 = 1$, as drawn below, the angle $\theta$ in standard position and the corresponding arc measuring $s$ units in length.  By definition, and the fact that the Unit Circle has radius 1, the radian measure of $\theta$ is $\frac{s}{r}=\frac{s}{1} = s$ so that, once again blurring the distinction between an angle and its measure, we have $\theta = s$.  

In order to identify real numbers with oriented angles, we essentially  `wrap' \index{wrapping function} the real number line around the Unit Circle and associating to each real number $t$ an \textit{oriented} arc \index{oriented arc} on the Unit Circle with initial point $(1,0)$.  

Viewing the vertical line $x=1$ as another real number line demarcated like the $y$-axis, given a real number $t>0$, we `wrap' the (vertical) interval $[0,t]$ around the Unit Circle in a counter-clockwise fashion.

This process is best seen in the GeoGebra interactive below on the left. By adjusting the slider upwards from $0$, we see as we move up the vertical line $x = 1$ from $(1,0)$ to the point $(1,t)$,  an arc of length $t$ units is traced out counter-clockwise along the Unit Circle. Since this arc has a length of $t$ units, the corresponding angle has radian measure equal to $t$. 


% https://www.geogebra.org/classic/aapdqza4
\begin{center}
    \geogebra{aapdqza4}{800}{600}
\end{center}


If $t<0$, we wrap the interval $[t,0]$ \textit{clockwise} around the Unit Circle.  This is seen above using the interactive on the right.  Adjusting the slider and moving downwards, we once again trace out an interval on the line $x = 1$ from $(1,0)$ to $(1,t)$. Here, the arc wraps \textit{clockwise} around the Unit Circle.  Since we have defined clockwise rotation as having negative radian measure, the angle determined by this arc has radian measure equal to $t$ in this case as well.  

Finally, note that if $t=0$, we are at the point $(1,0)$ on the $x$-axis which corresponds to an angle with radian measure $0$. In this way, we identify each real number $t$ with the corresponding angle with radian measure $t$.
...
% --- Parameters ---
\def\R{2}
\def\tdeg{135} % t = 3*pi/4 in degrees

% --- Full circle, light gray ---
\draw[gray!50] (0,0) circle (\R);

% --- Axes ---
\draw[->] (-\R-0.6,0) -- (\R+0.6,0) node[right] {$x$};
\draw[->] (0,-\R-0.6) -- (0,\R+0.6) node[above] {$y$};

% --- Bold arc from angle 0 to t, arrowhead at the terminal end ---
\draw[very thick, blue, domain=0:\tdeg, samples=60, smooth, variable=\a,
      -{Latex[length=3mm]}]
    plot ({\R*cos(\a)}, {\R*sin(\a)});

% --- Radius line to the terminal point, dotted, no arrowhead ---
\draw[dotted, thick] (0,0) -- ({\R*cos(\tdeg)},{\R*sin(\tdeg)});

% --- Unit labels ---
\node[above left] at (0,\R) {$1$};
\node[below right] at (\R,0) {$1$};

% --- Caption ---
\node at (0,-\R-1.1) {$t = \frac{3\pi}{4}$};
...
% --- Parameters ---
\def\R{2}
\def\tdeg{-360} % t = 3*pi/4 in degrees
...
% --- Parameters ---
\def\R{2}
\def\tdeg{-114.7} % t = 3*pi/4 in degrees
...
% --- Parameters ---
\def\R{2}
\def\tdeg{583.2} % t = 3*pi/4 in degrees
...
% --- Parameters ---
\def\R{2}
\def\tdeg{60} % angle theta, in degrees

% --- Full circle, light gray ---
\draw[gray!50] (0,0) circle (\R);

% --- Ray through P (angle 0), extending past the circle ---
\draw[->] (0,0) -- (\R+0.8,0);

% --- Ray through Q (angle theta), extending past the circle ---
\draw[->] (0,0) -- ({(\R+0.8)*cos(\tdeg)},{(\R+0.8)*sin(\tdeg)});

% --- Bold arc from P to Q ---
\draw[very thick, domain=0:\tdeg, samples=60, smooth, variable=\a]
    plot ({\R*cos(\a)}, {\R*sin(\a)});

% --- Center point ---
\fill (0,0) circle (2.5pt);

% --- Point P (on the circle, angle 0) ---
\fill (\R,0) circle (2.5pt);
\node[below right] at (\R+0.05,-0.05) {$P$};

% --- Point Q (on the circle, angle theta) ---
\fill ({\R*cos(\tdeg)},{\R*sin(\tdeg)}) circle (2.5pt);
\node[above left] at ({\R*cos(\tdeg)+0.05},{\R*sin(\tdeg)+0.05}) {$Q$};

% --- Angle arc for theta, near the center ---
\draw (0.55,0) arc [start angle=0, end angle=\tdeg, radius=0.55];
\node at ({0.85*cos(\tdeg/2)},{0.85*sin(\tdeg/2)}) {$\theta$};

% --- Radius label r, along the horizontal ray ---
\node[below] at (\R/2,-0.05) {$r$};

% --- Arc length label s, near the arc ---
\node at ({1.18*\R*cos(\tdeg/2.6)},{1.18*\R*sin(\tdeg/2.6)}) {$s$};
...
\begin{explanation} To use the formula $v = r \omega$, we first need to compute the angular velocity $\omega$.  The earth makes one revolution in 24 hours, and one revolution is $2 \pi$ radians, so $\omega = \frac{2 \pi \, \text{radians}}{24 \, \text{hours}} = \frac{\pi}{12 \, \text{hours}}$. Note that once again, we are identifying angles in radian measure as real numbers so we can drop the `radian' units as they are dimensionless.  Also note that  for simplicity's sake, we assume that we are viewing the rotation of the earth as counter-clockwise so $\omega > 0$.  Hence, the linear velocity is \[ v = 2960 \, \text{miles} \cdot \frac{\pi}{12 \, \text{hours}} \approx 775 \, \frac{\text{miles}}{\text{hour}}\] 
