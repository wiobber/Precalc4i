...
\begin{tabular}{ll}
 ...
 $\bullet \, $ The function $f(t) = \sin(t)$ &  $\bullet \, $ The function $g(t) = \cos(t)$  \\
 
\qquad -- has domain $(-\infty, \infty)$ & \qquad -- has domain $(-\infty, \infty)$  \\
...
\frac{3\pi}{2}  & -1 & \left(\frac{3\pi}{2}, -1 \right) \\ \hline 
...
% https://www.desmos.com/calculator/m6sshcvnod
\begin{center}
    \desmos{m6sshcvnod}{800}{600}
\end{center}
...
% https://www.desmos.com/calculator/i8zvkblwhp

\begin{center}
\desmos{i8zvkblwhp}{800}{600}
\end{center}
...
% https://www.desmos.com/calculator/0xabgsnluo
\begin{center}
    \desmos{0xabgsnluo}{800}{600}
\end{center}
...
% https://www.desmos.com/calculator/4xh96xwznz
\begin{center}
\desmos{4xh96xwznz}{800}{600}
\end{center}
...
\qquad -- has range $[-1,1]$ & \qquad  -- has range $[-1,1]$ \\
\qquad  -- is continuous and smooth &  \qquad -- is continuous and smooth \\ 
\qquad -- is odd &  \qquad  -- is  even \\ 
\qquad  -- has period $2\pi$ & \qquad -- has period $2\pi$ \\ 

\end{tabular}



\qquad $\bullet \, $ Conversion formulas:  $\sin\left(t + \frac{\pi}{2}\right) = \cos(t)$ and   $\cos\left(t - \frac{\pi}{2} \right) = \sin(t)$
...
% --- Sine curve ---
\draw[very thick,domain=0:6.283,samples=200,smooth]
    plot (\x,{sin(\x r)});

% --- Key points ---
\fill (0,0) circle (2.5pt);
\fill ({pi/2},1) circle (2.5pt);
\fill ({pi},0) circle (2.5pt);
\fill ({3*pi/2},-1) circle (2.5pt);
\fill ({2*pi},0) circle (2.5pt);

% --- x-axis ticks ---
\foreach \x in {pi/2,pi,3*pi/2,2*pi}
{
    \draw (\x,-0.06) -- (\x,0.06);
}
...
% --- Tick labels ---
\node[below] at ({pi/2},-0.08) {$\pi/2$};
\node[below] at ({pi},-0.08) {$\pi$};
\node[below] at ({3*pi/2},-0.08) {$3\pi/2$};
\node[below] at ({2*pi},-0.08) {$2\pi$};
...
\node[above right] at ({pi},0)
    {$\left(\pi,0\right)$};

\node[below] at ({3*pi/2},-1)
...
\fill ({pi/2},0) circle (2.5pt);
\fill ({pi},-1) circle (2.5pt);
\fill ({3*pi/2},0) circle (2.5pt);
\fill ({2*pi},1) circle (2.5pt);
...
\node[above right] at ({pi/2},0)
    {$\left(\frac{\pi}{2},0\right)$};

\node[below] at ({pi},-1)
    {$\left(\pi,-1\right)$};

\node[above left] at ({3*pi/2},0)
...
Using the desmos interactive below, we can select each transformation, in sequence, to see the progression from the graph of $y = \sin(t)$ to $y = f(t) = 3 \sin(2t)$.

% https://www.desmos.com/calculator/0y2nwkw9yo

\begin{center}
    \desmos{0y2nwkw9yo}{800}{600}
\end{center}
...
Using the desmos interactive below, we can select each transformation, in sequence, to see the progression from the graph of $y = \cos(t)$ to $y = g(t) = 2 \cos\left(t +\frac{\pi}{2} \right) +1$.


% https://www.desmos.com/calculator/pwzvj3bqjl
\begin{center}
    \desmos{pwzvj3bqjl}{800}{600}
\end{center}
...
% --- Sinusoidal curve ---
\draw[very thick,domain=0:\period,samples=200,smooth]
    plot (\x,{\A*sin(360*\x/\period)});
...
% --- Baseline label ---
\node[right,fill=white,inner sep=2pt]
    at ({0.63*\period},0)
    {baseline};
...
\item  have period\index{sinusoid ! period}\index{period ! of a sinusoid}  $T = \frac{2\pi}{\omega}$

\item  have amplitude \index{sinusoid ! amplitude}\index{amplitude}$|A|$

\item  have phase shift\index{sinusoid ! phase shift}\index{phase shift} $-\frac{\phi}{\omega}$
...
We connect the dots in a `wavelike' fashion to produce the graph below.
...
% --- Curve ---
\draw[very thick, blue,domain=1:5,samples=200,smooth]
    plot (\x,{3*cos(90*(\x-1))+1});
...
We identify $A = -\frac{1}{2}$, $\omega = 2$, $\phi = -\pi$ and $B = \frac{3}{2}$.  The frequency is $\omega = 2$, the period is  $T=\frac{2\pi}{2} = \pi$,  the phase shift is $-\frac{-\pi}{2} = \frac{\pi}{2}$ (indicating a shift \textit{right} $\frac{\pi}{2}$ units), the amplitude is $\left| - \frac{1}{2} \right| = \frac{1}{2}$, and, finally,  the vertical shift is \textit{up} $\frac{3}{2}$. 


Proceeding as before, we know one cycle of $g$ starts at $t = \frac{\pi}{2}$ and ends at $t = \frac{\pi}{2} + \pi = \frac{3\pi}{2}$.  Dividing the interval $\left[ \frac{\pi}{2}, \frac{3 \pi}{2} \right]$ into four equal pieces  gives pieces of length  $\frac{\pi}{4}$ units.  Hence, to obtain our new quarter marks, we start at $t = \frac{\pi}{2}$ and add $\frac{\pi}{4}$ until we reach $t=\frac{3\pi}{2}$.   Our new quarter marks are:  $t = \frac{\pi}{2}$, $t = \frac{3\pi}{4}$, $t = \pi$, $t = \frac{5\pi}{4}$, $t = \frac{3\pi}{2}$.  Substituting these values into $g$ gives us the table below. 



\[ \begin{array}{|r||r|r|} \hline
t & g(t) & (t,g(t)) \\  \hline
\frac{\pi}{2} & \frac{3}{2} & \left(\frac{\pi}{2}, \frac{3}{2}\right)  \\    \hline
\frac{3\pi}{4} & 1 & \left(\frac{3\pi}{4} , 1 \right) \\  \hline 
\pi & \frac{3}{2} & \left(\pi , \frac{3}{2} \right)  \\  \hline 
\frac{5\pi}{4}  & 2 &  \left(\frac{5\pi}{4} , 2 \right) \\  \hline 
\frac{3\pi}{2} & \frac{3}{2} & \left(\frac{3\pi}{2}, \frac{3}{2} \right) \\   \hline
\end{array} \]


From this table, we produce the graph below.
...
\node[below] at (1,-0.12) {$\frac{\pi}{4}$};
\node[below] at (2,-0.12) {$\frac{\pi}{2}$};
\node[below] at (3,-0.12) {$\frac{3\pi}{4}$};
\node[below] at (4,-0.12) {$\pi$};
\node[below] at (5,-0.12) {$\frac{5\pi}{4}$};
\node[below] at (6,-0.12) {$\frac{3\pi}{2}$};
...
\draw[very thick,penColor1, domain=0.5:1.5, samples=80, smooth, variable=\u]
    plot ({\u*\piunit}, {1.5 +0.5*sin(360*(-\u+0.5))});
...
t & g(t) & (t,g(t)) \\  \hline
\frac{\pi}{2} & \frac{3}{2} & \left(\frac{\pi}{2}, \frac{3}{2}\right)  \\ \hline
\frac{\pi}{4} & 2 & \left(\frac{\pi}{4}, 2\right) \\  \hline 
0 & \frac{3}{2} & \left(0, \frac{3}{2} \right)  \\  \hline 
...
Comparing this result (in green) with the one obtained in Example \ref{cosinesinegraphex2} (in purple) side by side, we see that one cycle ends right where the other starts.  The cause of this discrepancy goes back to using the odd property of sine. 
...
\draw[very thick,penColor1, domain=0.5:1.5, samples=80, smooth, variable=\u]
    plot ({\u*\piunit}, {1.5 - 0.5*sin(360*(\u-0.5))});
    \draw[very thick,penColor2, domain=-0.5:0.5, samples=80, smooth, variable=\u]
    plot ({\u*\piunit}, {1.5 - 0.5*sin(360*(\u-0.5))});
...
\fill (3.5,0.5) circle (2.5pt);
\fill (5,2.5) circle (2.5pt);
...
\fill (3.5,0.5) circle (2.5pt);
\fill (5,2.5) circle (2.5pt);
\fill (6.5,0.5) circle (2.5pt);
\fill (8,-1.5) circle (2.5pt);
...
\node[above left] at (3.6,0.45)
    {$\left(\frac72,\frac12\right)$};

\node[above] at (5,2.5)
    {$\left(5,\frac52\right)$};

\node[above right] at (6.75,0.45)
    {$\left(\frac{13}2,\frac12\right)$};

\node[below] at (8,-1.5)
    {$\left(8,-\frac32\right)$};

\node[above right] at (9.75,0.45)
    {$\left(\frac{19}2,\frac12\right)$};
...
\begin{explanation}  We sketch the problem situation below and assume a counter-clockwise rotation.\footnote{Otherwise, we could just observe the motion of the wheel from the other side.} 
...
\coordinate (Q) at (\R,0);
\coordinate (P) at (0,-\R);
\coordinate (O) at (0,\groundy);

% Point used for the height measurement
\coordinate (H) at (210:\R);

% ============================================================
% Ground
% ============================================================

\draw (-3.75,\groundy) -- (3.75,\groundy);
...
% ============================================================
% Rotation arrows along the wheel
% ============================================================
...
% ============================================================
% Vertical radius from P to O
% ============================================================

\draw (P) -- (O);

% ============================================================
% Dashed horizontal continuation through Q
% ============================================================
...
We know from the Equation \ref{equationsforcircularmotion} in Section \ref{cosinesinebeyond} that the $y$-coordinate for counter-clockwise motion on a circle of radius $r$ centered at the origin with constant angular velocity (frequency) $\omega$ is given by $y = r\sin(\omega t)$.  Here,  $t=0$ corresponds to the point $(r,0)$ so that $\theta$, the angle measuring the amount of rotation, is in standard position. 
...
Putting these two pieces of information together, we have that  $y = 64 \sin\left(\frac{4 \pi}{127} t\right)$ describes the $y$-coordinate on the Giant Wheel after $t$ seconds, assuming it is centered at $(0,0)$ with $t=0$ corresponding to point $Q$. 
 ...
 From the discussion motivating Equation \ref{equationsforcircularmotion},  we know $\theta = \omega t = \frac{4 \pi}{127} t$, so we (temporarily) write the height in terms of $\theta$ as  $h =64 \sin\left(\theta\right) + 72$.    Subtracting $\frac{\pi}{2}$ from $\theta$ gives $h(t) = 64 \sin\left(\theta - \frac{\pi}{2}\right) + 72 = 64\sin\left(\frac{4 \pi}{127} t -\frac{\pi}{2} \right) + 72$. 
...
% --- Scale factors: t goes up to ~63.5, y goes up to 136 ---
\def\xs{0.095}
\def\ys{0.037}
% Macro to convert data coordinates (t,y) to drawing coordinates
\newcommand{\pt}[2]{({#1*\xs},{#2*\ys})}

% --- Axes ---
\draw[->] (-0.3,0) -- \pt{69}{0} node[right] {$t$};
\draw[->] (0,-0.3) -- (0,{136*\ys+0.6}) node[above] {$y$};

% --- Tick marks and labels on y-axis ---
\foreach \v in {8,72,136}{
    \draw ({-0.06},{\v*\ys}) -- ({0.06},{\v*\ys});
    \node[left] at ({-0.1},{\v*\ys}) {$\v$};
}

% --- Tick mark and label on t-axis at 127/2 ---
\draw \pt{63.5}{-0.5} -- \pt{63.5}{0.5};
\node[below] at \pt{63.5}{-0.6} {$\frac{127}{2}$};

% --- Curve: y = 72 - 64*cos(4*pi*t/127), t in [0, 127/2] ---
\draw[very thick,blue, domain=0:63.5, samples=100, smooth, variable=\t]
    plot ({\t*\xs}, {(72 - 64*cos((720/127)*\t))*\ys});

% --- Data points ---
\fill \pt{0}{8} circle (2.5pt);
\fill \pt{15.875}{72} circle (2.5pt);
\fill \pt{31.75}{136} circle (2.5pt);
\fill \pt{47.625}{72} circle (2.5pt);
\fill \pt{63.5}{8} circle (2.5pt);
...
We've already stated as fact (but not proven) that $f(t) = \sin(t)$ and $g(t) = \cos(t)$ are continuous.  Per Definition \ref{continuousdefn}, this means that $\lim_{t \rightarrow a} \sin(t) = \sin(a)$ and $lim_{t \rightarrow a} \cos(t) = \cos(a)$ for all real numbers, $a$.  This means, for instance, we can compute  $\lim_{t \rightarrow \pi} \cos(t) = \cos(\pi) = -1$.  Per the discussion following Definition \ref{continuousdefn}, so long as we avoid the usual domain pitfalls, we can also compute:
 ...
 \item $ \lim_{t \rightarrow 0} \sin\left( \frac{\pi}{t} \right)$ 
  \item $\lim_{t \rightarrow 0} \sin\left( 10^{23} \, t \right)$
   \item $\lim_{t \rightarrow 0} \frac{\sin(t)}{t}$
...
\item   To analyze $\lim_{t \rightarrow 0} \sin\left( \frac{\pi}{t} \right)$ numerically, we construct a table of values `near' $t = 0$ for $F(t) = \sin\left( \frac{\pi}{t} \right)$.  Choosing values such as $t = \pm 0.001$,  $t = \pm 0.01$, etc., we get  a list of `$0$'s. This result shouldn't be too surprising since in each of these cases, $\frac{\pi}{t}$ results in an integer multiple of $\pi$. However, the graph near $t=0$ (literally) paints a different picture.  The graph appears to wildly oscillate near $t=0$, suggesting that  $\lim_{t \rightarrow 0}\sin\left( \frac{\pi}{t} \right)$ does not exist.

% https://www.desmos.com/calculator/ag6olhwkkb
\begin{center}
\desmos{ag6olhwkkb}{800}{600}
\end{center}
 
 Indeed, as $t \rightarrow 0^{-}$, $\frac{\pi}{t} \rightarrow -\infty$ and as $t \rightarrow 0^{+}$, $\frac{\pi}{t} \rightarrow \infty$.  In other words, all of the infinite oscillations experienced by the standard sine function, $f(t) = \sin(t)$ as $t \rightarrow -\infty$ and as $t \rightarrow \infty$ are being brought  back to near  $t = 0$ in $F(t) = \sin\left( \frac{\pi}{t} \right)$.  Hence, $\lim_{t \rightarrow 0} \sin\left( \frac{\pi}{t} \right)$ does not exist.


\item  We proceed as above to investigate $ \lim_{t \rightarrow 0} \sin\left( 10^{23} \, t \right)$.    A table and  a graph of  $F(t) = \sin \left( 10^{23} \, t\right)$  `near' $t = 0$ produces numerical and graphical noise.  Neither the table nor the graph suggest $\lim_{t \rightarrow 0} F(t)$ exists.  However, we know  $F(t) = \sin \left( 10^{23} \, t\right)$ is a sinusoid with frequency $\omega = 10^{23}$.  Hence, resetting the window on the desmos interactive below to $-\frac{\pi}{10^{23}} \leq x \leq \frac{\pi}{10^{23}}$ and $-1 \leq y \leq 1$, we see one cycle of $F$ which suggests the limit is, in fact, $0$.

% https://www.desmos.com/calculator/jue9qbhzmb
\begin{center}
\desmos{jue9qbhzmb}{800}{600}
\end{center}



Indeed, we know  $F(t) = \sin \left( 10^{23} \, t\right)$ is continuous, so $\lim_{t \rightarrow 0} \sin\left( 10^{23} \, t \right) = \sin\left(10^{23} (0)) \right) = \sin(0) = 0$.   

\item  Next we investigate  $F(t) =  \frac{\sin(t)}{t}$ near $t=0$.   The table and graph both suggest   $\lim_{t \rightarrow 0} \frac{\sin(t)}{t} = 1$. 


% https://www.desmos.com/calculator/ohffbwnwsf
\begin{center}
\desmos{ohffbwnwsf}{800}{600}
 \end{center}
 
 Attempting to analyze this limit analytically, we see that  $\lim_{t \rightarrow 0} \frac{\sin(t)}{t}$ produces the indeterminate form `$\frac{0}{0}$'.  At this stage, we do not have the tools to either prove or disprove our conjecture that  $\lim_{t \rightarrow 0} \frac{\sin(t)}{t} = 1$,  but we will soon, \footnote{See Exercises \ref{sintovertexercise1} through \ref{sintovertexercise3} in Section \ref{TheOtherCircularFunctions}.} so stay tuned! 
