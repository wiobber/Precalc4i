...
% Labels
\node[below=2pt] at (-5,0.05) {$-\frac{5\pi}{2}$};
\node[below=2pt] at (-3,0.05) {$-\frac{3\pi}{2}$};
\node[below=2pt] at (-1,0.05) {$-\frac{\pi}{2}$};
...
0  & 1 & 1 & (0,1) \\ \hline
\frac{\pi}{4}  & \frac{\sqrt{2}}{2} & \sqrt{2} & \left(\frac{\pi}{4}, \sqrt{2} \right) \\ \hline 
\frac{\pi}{2}  & 0 & \text{undefined} &  \\ \hline 
\frac{3\pi}{4}  & -\frac{\sqrt{2}}{2} & -\sqrt{2} & \left(\frac{3\pi}{4}, -\sqrt{2} \right) \\ \hline 
\pi & -1 & -1 &  (\pi, -1) \\ \hline 
\frac{5\pi}{4}  & -\frac{\sqrt{2}}{2} & -\sqrt{2} & \left(\frac{5\pi}{4}, -\sqrt{2} \right) \\ \hline 
\frac{3\pi}{2}  & 0 & \text{undefined} & \\ \hline 
\frac{7\pi}{4}  & \frac{\sqrt{2}}{2} & \sqrt{2} & \left(\frac{7\pi}{4}, \sqrt{2} \right) \\ \hline 
2\pi  & 1 &  1& (2\pi, 1) \\ \hline
\end{array} \] 

Using the techniques developed in Section \ref{IntroRational}, we can more closely analyze the behavior of $F$ near the values excluded from its domain.  We find  as $t \rightarrow \frac{\pi}{2}^{-}$, $\cos(t) \rightarrow 0^{+}$, so $\lim_{t \rightarrow \frac{\pi}{2}^{-}}\sec(t) = \infty$.  Similarly, we get $\lim_{t \rightarrow \frac{\pi}{2}^{+}}\sec(t) = -\infty$, $\lim_{t \rightarrow \frac{3\pi}{2}^{-}}\sec(t) = -\infty$, and $\lim_{t \rightarrow \frac{3\pi}{2}^{+}}\sec(t)  = \infty$.  This means the lines $t = \frac{\pi}{2}$ and $t = \frac{3\pi}{2}$ are vertical asymptotes to the graph of $y = \sec(t)$.


With help from desmos, we graph a fundamental cycle of $y = \sec(t)$ \index{secant ! graph of} with the graph of the fundamental cycle of $y = \cos(t)$ dotted for reference.  


% https://www.desmos.com/calculator/z7krkvyf36
\begin{center}
    \desmos{z7krkvyf36}{800}{600}
\end{center}
...
Proceeding as above,  we can generate a table of values for the cosecant function by taking reciprocals of known sine values.

\[ \begin{array}{|r||r|r|r|}  \hline
x & \sin(x) & \csc(x) & (x,\csc(x)) \\ \hline
0  & 0 & \text{undefined} &  \\ \hline
\frac{\pi}{4}  & \frac{\sqrt{2}}{2} & \sqrt{2} & \left(\frac{\pi}{4}, \sqrt{2} \right) \\ \hline 
\frac{\pi}{2}  & 1 & 1 & \left(\frac{\pi}{2}, 1 \right) \\ \hline 
\frac{3\pi}{4}  & \frac{\sqrt{2}}{2} & \sqrt{2} & \left(\frac{3\pi}{4}, \sqrt{2} \right) \\  \hline 
\pi & 0 & \text{undefined} &   \\  \hline 
\frac{5\pi}{4}  & -\frac{\sqrt{2}}{2} & -\sqrt{2} & \left(\frac{5\pi}{4}, -\sqrt{2} \right) \\  \hline 
\frac{3\pi}{2}  & -1 & -1 & \left(\frac{3\pi}{2},-1 \right)\\ \hline 
\frac{7\pi}{4}  & -\frac{\sqrt{2}}{2} & -\sqrt{2} & \left(\frac{7\pi}{4}, -\sqrt{2} \right) \\ \hline 
2\pi  & 0 & \text{undefined} &  \\ \hline
\end{array} \] 

Plotting these points and performing a similar analysis to the gaps in the domain, we can construct the graph of one cycle of the cosecant function (with a little help from desmos) below.

% https://www.desmos.com/calculator/gbgxaulwmy
\begin{center}
    \desmos{gbgxaulwmy}{800}{600}
\end{center}
...
Substituting these $t$ values $f(t)$, we get the table below.  Note that if $f(t)$ exists, we have a point on the graph;  otherwise, we have found a vertical asymptote.\footnote{As with the examples in Section \ref{GraphsofSineandCosine}, note that we can partially check our answer since the argument of the secant function should simplify to the `original' quarter marks - the quadrantal angles.}

\[ \begin{array}{|r||r|r|}  \hline
t & f(t) & (t,f(t))  \\ \hline
0  & - 1 & (0,-1)  \\ \hline
\frac{\pi}{4}  & \text{undefined} &  \\\hline 
\frac{\pi}{2}  & 3 & \left(\frac{\pi}{2}, 3 \right)  \\ \hline 
\frac{3\pi}{4}  & \text{undefined} &  \\ \hline 
\pi & -1 &   (\pi, -1) \\ \hline 
\end{array} \] 

With the help of desmos, we graph one cycle of  $f(t) = 1 - 2 \sec(2t)$ below along with the associated cosine curve,  $y = 1 - 2 \cos(2t)$ which is dotted, and confirm the period is $\pi - 0 = \pi$.

% https://www.desmos.com/calculator/gyzog4c7gj
\begin{center}
    \desmos{gyzog4c7gj}{800}{600}
\end{center}
...
Substituting these $t$-values into $g(t)$, we get the table of values below.  

\[ \begin{array}{|r||r|r|}  \hline
t & g(t) & (t,g(t))  \\ \hline
-1  & \text{undefined} &   \\ \hline
-\frac{1}{2}  & -2 & \left(-\frac{1}{2}, -2\right)  \\ \hline 
0 & \text{undefined} &   \\ \hline 
\frac{1}{2} & -\frac{4}{3} &  \left(\frac{1}{2}, -\frac{4}{3} \right)  \\  \hline 
1 & \text{undefined} &    \\  \hline 
\end{array} \] 

With help from desmos, we generate the graph below and confirm the period is $1 - (-1) = 2$.   The associated sine curve, $y = \frac{\sin(- \pi t- \pi) - 5}{3}$, is dotted in as a reference.

% https://www.desmos.com/calculator/btir8ep9i1
\begin{center}
    \desmos{btir8ep9i1}{800}{600}
\end{center}
...
\item  have period  $T = \frac{2\pi}{\omega}$

\item  have phase shift $-\frac{\phi}{\omega}$
...
% --- Parameters ---
% x-axis: 1 unit = pi/4 (so pi/2 -> x=1, pi -> x=2, 3pi/2 -> x=3, etc.)
\def\piunit{6}
% Curve: y = 1.5 - 0.5*sin(2(t - pi/2)), sampled over t in [pi/2, 3pi/2]
% In terms of u = t/pi (so x = u*\piunit): y = 1.5 - 0.5*sin(360*(u-0.5))

% --- Axes ---
\draw[->] (-1.5,0) -- (6,0) node[right] {$t$};
\draw[->] (0,-5) -- (0,3) node[above] {$y$};

% --- Tick marks and labels on t-axis, in units of pi/4 ---
\node[below] at (-1,-0.12) {$-\frac{\pi}{6}$};
\node[below] at (1,-0.12) {$\frac{\pi}{6}$};
\node[below] at (0.5,-0.12) {$\frac{\pi}{12}$};
\node[below] at (2,-0.12) {$\frac{\pi}{3}$};
\node[below] at (3.5,-0.12) {$\frac{7\pi}{12}$};
\node[below] at (5,-0.12) {$\frac{5\pi}{6}$};

\foreach \x in {-1, 0.5,1, 2, 3.5,5}
    \draw (\x,-0.06) -- (\x,0.06);
...
% --- Curve over t in [pi/2, 3pi/2], i.e. u in [0.5, 1.5] ---
\begin{scope}
\clip (-2,-4.5) rectangle (5,3);

\draw[very thick,penColor1,
      domain=-0.1567:0.3233,
      samples=80,smooth,variable=\u]
plot ({\u*\piunit},
      {1/cos(360*(\u-7/12))-1});

\draw[very thick,penColor1,
      domain=0.3433:0.8233,
      samples=80,smooth,variable=\u]
plot ({\u*\piunit},
      {1/cos(360*(\u-7/12))-1});

\draw[dashed,penColor1](-1,3)--(-1,-5);    
\draw[dashed,penColor1](2,3)--(2,-5);   
\draw[dashed,penColor1](5,3)--(5,-5);    
...
To find the phase $\phi$, we need to first determine the phase shift.  Recall that what is graphed here is only one \text{cycle} of the function, so by copying and pasting one more cycle, we identify what looks like a fundamental cycle of the secant function to us\footnote{Assuming $A>0$, that is.}
...
% https://www.desmos.com/calculator/wpxkqo7afu
\begin{center}
    \desmos{wpxkqo7afu}{800}{600}
\end{center}
...
Working `from scratch,' we would find $T = \pi$, $\omega = 2$, $B=-1$, and $A=1$ the same as above.\footnote{Again, assuming we want $A > 0$.}  To determine the phase shift, we refer to the figure given to us in the problem above.


Since the phase shift is $\frac{\pi}{3}$, we solve  $-\frac{\phi}{2} = \frac{\pi}{3}$ to get $\phi = -\frac{2\pi}{3}$. Putting all our work together, we get our final answer: $f(t) = \csc\left(2t - \frac{2\pi}{3} \right) - 1$.  Again, our best check here is to graph.
...
Next, we turn our attention to the tangent and cotangent functions.  Viewing $J(t) = \tan(t) = \frac{\sin(t)}{\cos(t)}$, we find the domain of $J$ excludes all values where $\cos(t) = 0$.  Hence, the domain of $J$ is $\{ t \, | \, t \neq  \frac{\pi}{2} + \pi k, \text{for integers $k$} \}$.  Using this information along with the common values we derived in Section \ref{TheOtherCircularFunctions}, we create the table of values below.

\[ \begin{array}{|r||r|r|} \hline
t & \tan(t) & (t,\tan(t)) \\ \hline
0  & 0 & (0, 0) \\ \hline
\frac{\pi}{4}  & 1 & \left(\frac{\pi}{4},1 \right) \\ \hline 
\frac{\pi}{2}  & \text{undefined} &  \\ \hline 
\frac{3\pi}{4}  & -1 & \left(\frac{3\pi}{4}, -1\right) \\ \hline 
\pi & 0 & (\pi, 0) \\ \hline 
\frac{5\pi}{4}  & 1 & \left(\frac{5\pi}{4}, 1 \right) \\ \hline 
\frac{3\pi}{2}  & \text{undefined} &  \\ \hline 
\frac{7\pi}{4}  & -1 & \left(\frac{7\pi}{4}, -1 \right) \\ \hline 
2\pi  & 0 & (2\pi, 0) \\ \hline
\end{array} \] 



Investigating the behavior near the values excluded from the domain, we find as $t \rightarrow \frac{\pi}{2}^{-}$, $\sin(t) \rightarrow 1^{-}$ and $\cos(t) \rightarrow 0^{+}$. Hence,   $\lim_{t \rightarrow \frac{\pi}{2}^{-}}\tan(t) = \infty$ producing a vertical asymptote to the graph at $t = \frac{\pi}{2}$.  Similarly, we get that as $\lim_{t \rightarrow \frac{\pi}{2}^{+}} \tan(t) = -\infty$,   $\lim_{t \rightarrow \frac{3\pi}{2}^{-}} \tan(t) = \infty$, and  $\lim_{t \rightarrow \frac{3\pi}{2}^{+}}\tan(t) = -\infty$.



Putting all of this information together, we graph $y = \tan(t)$ with help from desmos over the interval $[0, 2\pi]$ below.


% https://www.desmos.com/calculator/vlk0j42adn
\begin{center}
    \desmos{vlk0j42adn}{800}{600}
\end{center}

After the usual `copy and paste' procedure, we create the graph of $y = \tan(t)$ below: \index{tangent ! graph of}

% https://www.desmos.com/calculator/hcvtzhuwnq
\begin{center}
    \desmos{hcvtzhuwnq}{800}{600}
\end{center}
...
\frac{3\pi}{4}  & -1 & \left(\frac{3\pi}{4}, -1\right) \\ \hline 
\pi & \text{undefined} &  \\ \hline 
\frac{5\pi}{4}  & 1 & \left(\frac{5\pi}{4}, 1 \right) \\ \hline 
\frac{3\pi}{2}  & 0 & \left(\frac{3\pi}{2}, 0 \right) \\ \hline 
\frac{7\pi}{4}  & -1 & \left(\frac{7\pi}{4}, -1 \right) \\ \hline 
2\pi  & \text{undefined} &  \\ \hline
\end{array} \] 


After analyzing the behavior of $K$ near the values excluded from its domain, we use desmos to help with plotting points and graph $y = \cot(t)$ over the interval $[0,2\pi]$ below. \index{cotangent ! graph of}


% https://www.desmos.com/calculator/rlkr14n3t2
\begin{center}
    \desmos{rlkr14n3t2}{800}{600}
\end{center}

As usual, pasting copies end to end produces the graph of $K(t) = \cot(t)$ below.  \index{cotangent ! graph of}

% https://www.desmos.com/calculator/hevswagsfm
\begin{center}
\desmos{hevswagsfm}{800}{600}
\end{center}
...
\item  have period  $T = \frac{\pi}{\omega}$
...
\item  The phase shift for $y = J(t)$ is $-\frac{\phi}{\omega} - \frac{\pi}{2 \omega}$.

\item  The phase shift for $y = K(t)$ is $-\frac{\phi}{\omega}$.
...
t & f(t) & (t,f(t))  \\ \hline
\pi  & \text{undefined} &   \\ \hline
\frac{3\pi}{2}  &  2 &  \left(\frac{3\pi}{2}, 2 \right) \\ \hline 
2 \pi & 1 &  (2\pi,1)  \\ \hline 
\frac{5\pi}{2}  & 0 &  \left(\frac{5\pi}{2}, 0 \right) \\ \hline 
3 \pi & \text{undefined} &   \\ \hline 
\end{array} \] 

We use desmos to help us produce the graph below

% https://www.desmos.com/calculator/it4qlh5vkg
\begin{center}
    \desmos{it4qlh5vkg}{800}{600}
\end{center}
...
Our quarter marks are $\frac{1}{4}$ units apart and are $t = 2$, $t = \frac{9}{4}$, $t = \frac{5}{2}$, $t = \frac{11}{4}$, and $t = 3$.  We get the following table of values:
...
\[ \begin{array}{|r||r|r|} \hline
t & g(t) & (t,g(t))  \\ \hline
2  & \text{undefined} &   \\ \hline
\frac{9}{4}  &  -3 &  \left(\frac{9}{4}, -3 \right) \\ \hline 
\frac{5}{2} & -1 &  \left( \frac{5}{2}, -1 \right)  \\ \hline 
\frac{11}{4}  & 1 &  \left(\frac{11}{4}, 1 \right) \\ \hline 
3 & \text{undefined} &  \\ \hline 
\end{array} \]

Desmos once again helps with the graph:

% https://www.desmos.com/calculator/vwhrak3xer
\begin{center}
    \desmos{vwhrak3xer}{800}{600}
\end{center}

We confirm the period is $3-2 = 1$.

\end{enumerate}
\end{explanation}
\end{example}

Our next example flips the script and we reverse engineer tangent and cotangent functions from a graph.
...
% --- Axes ---
\draw[->] (-3,0) -- (11,0) node[right] {$t$};
\draw[->] (0,-10) -- (0,10) node[above] {$y$};

% --- Tick marks and labels on t-axis, in units of pi/4 ---
\node[below left] at (-2,-0.12) {$-2$};
\node[below] at (1,-0.12) {$1$};
\node[below] at (2,-0.12) {$2$};
\node[below] at (4,-0.12) {$4$};
\node[below] at (6,-0.12) {$6$};
\node[below] at (7,-0.12) {$7$};
\node[below] at (8,-0.12) {$8$};
\node[below right] at (10,-0.12) {$10$};

\foreach \x in {-2,1,2,4,6,7,8,10}
    \draw (\x,-0.06) -- (\x,0.06);

% --- Tick marks and labels on y-axis ---
\foreach \y in {-5,-3,3,5}{
    \draw (-0.06,\y) -- (0.06,\y);
    \node[left] at (-0.1,\y) {$\y$};
}
...
% --- Curve over t in [pi/2, 3pi/2], i.e. u in [0.5, 1.5] ---
\begin{scope}
\clip (-2.1,-10) rectangle (10.1,10);

\draw[very thick,penColor1,
      domain=-1.9:9.9,
      samples=80,smooth,variable=\u]
plot ({\u},
      {-3*tan(360*((1/24)*\u-1/6)});


\draw[dashed,penColor1](-2,10)--(-2,-10);    
\draw[dashed,penColor1](10,10)--(10,-10);   
...
As above, the vertical stretch is by a factor of $3$, so we take $A = 3$ for our final answer:  $f(t) = 3 \cot \left( \frac{\pi}{12} t  + \frac{\pi}{6}\right)$.  As always, we check our answer by graphing. 
...
\item  Find and interpret $\lim_{\theta \rightarrow \frac{\pi}{2}^{-}} f(\theta)$ and $\lim_{\theta \rightarrow \frac{\pi}{2}^{-}} g(\theta)$.
\end{enumerate}
...
% Endpoint markers
\node[font=\Large] at (A) {$\ast$};
\node[font=\Large] at (C) {$\ast$};

% Labels
\node[left=7pt] at (A) {observation point};
\node[above right=1pt] at (C) {rocket};

\node[left=10pt] at (2.6,3.2) {$g(\theta)\ \text{ft.}$};
\node[right=7pt] at (4.2,3.2) {$f(\theta)\ \text{ft.}$};

\node[below=8pt] at ($(A)!0.5!(B)$) {$42\ \text{ft.}$};
...
\item We find both $\lim_{\theta \rightarrow \frac{\pi}{2}^{-}} f(\theta) = \infty$ and $\lim_{\theta \rightarrow \frac{\pi}{2}^{-}} g(\theta) = \infty$, which we can verify graphically.  This means as the angle of inclination approaches $\frac{\pi}{2}$ or $90^{\circ}$,  the distances from the rocket to the ground and from to the rocket to the observation point increase without bound.   Barring the effects of drift or the curvature of space, this matches our intuition. 
...
In order to write this set in a more compact way, we let $t_{k}$ denote the  the $k$th real number excluded from the domain.  That is,  $t_{k} =  \frac{\pi}{2} + \pi k$.  (This is sequence notation from Section \ref{Sequences}.)



Getting a common denominator and factoring out the $\pi$ in the numerator, we get  $t_{k} = \frac{(2k+1)\pi}{2}$.  The set we're after consists of the union of intervals determined by the successive points $t_{k}$:   $\left(t_{k}, t_{k+1}\right) = \left( \frac{(2k+1)\pi}{2},  \frac{(2k+3)\pi}{2}\right)$ where $k$ ranges through the integers.  We denote this union as:
...
$\bullet \, $ The function $f(t) = \cos(t)$ & $\bullet \, $ The function $g(t) = \sin(t)$ \\

\qquad -- has domain $(-\infty, \infty)$ & \qquad -- has domain $(-\infty, \infty)$ \\ [4pt]
\qquad  -- has range $[-1,1]$ & \qquad -- has range $[-1,1]$ \\
...
$\bullet \, $ The function $F(t) = \sec(t)$ &   $\bullet \, $ The function $G(t) = \csc(t)$ \\
\qquad -- has domain $\bigcup_{k = -\infty}^{\infty} \left( \frac{(2k+1)\pi}{2}, \frac{(2k+3) \pi}{2} \right)$  &  \qquad -- has domain $\bigcup_{k = -\infty}^{\infty} \left(k \pi ,(k+1) \pi \right)$  \\ 
\qquad -- has range $(-\infty, -1] \cup [1, \infty) $ & \qquad  -- has range $(-\infty, -1] \cup [1, \infty) $ \\
...
$\bullet \, $ The function $J(t) = \tan(t)$ &   $\bullet \, $ The function $K(t) = \cot(t)$ \\
\qquad -- has domain $\bigcup_{k = -\infty}^{\infty} \left( \frac{(2k+1)\pi}{2}, \frac{(2k+3) \pi}{2} \right)$  &  \qquad -- has domain $\bigcup_{k = -\infty}^{\infty} \left(k \pi ,(k+1) \pi \right)$   \\ 
\qquad -- has range $(-\infty, \infty) $ &  \qquad -- has range $(-\infty, \infty) $ \\ 
