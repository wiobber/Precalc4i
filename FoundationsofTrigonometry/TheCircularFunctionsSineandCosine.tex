...
\def\panelwidth{6.5}
\def\R{2}
\def\tdeg{55} % theta, in degrees

% ===================== PANEL 1: angle theta only =====================
\begin{scope}
    ...
    % Terminal ray at angle theta
    \draw[very thick, -] (0,0) -- ({1.5*\R*cos(\tdeg)},{1.5*\R*sin(\tdeg)});

    % Center point
    \fill (0,0) circle (2.5pt);

    % Unit labels
    \node[above left] at (-0.12,\R) {$1$};
    \node[below right] at (\R,-0.12) {$1$};

    % Angle arc for theta
    \draw[->] (0.7,0) arc [start angle=0, end angle=\tdeg, radius=0.7];
    \node at ({1.15*cos(\tdeg/2)},{1.15*sin(\tdeg/2)}) {$\theta$};
\end{scope}

% ===================== PANEL 2: with point P(cos theta, sin theta) =====================
\begin{scope}[xshift=\panelwidth cm]
    ...
    \fill ({\R*cos(\tdeg)},{\R*sin(\tdeg)}) circle (2.5pt);
    \node[right] at ({\R*cos(\tdeg)+0.1},{\R*sin(\tdeg)+0.05}) {$P(\cos(\theta),\sin(\theta))$};
...
% --- Parameters ---
\def\R{2}
\def\tdeg{270} % theta, in degrees
\pgfmathsetmacro{\px}{\R*cos(\tdeg)}
\pgfmathsetmacro{\py}{\R*sin(\tdeg)}
...
% --- Radius segment (thick, solid) from center to P ---
\draw[very thick] (0,0) -- (\px,\py);
\draw[very thick] (0,0) -- (2,0);
% --- Ray continuing past P (thin, with arrowhead) ---
\draw[-] (\px,\py) -- ({1.5*\R*cos(\tdeg)},{1.5*\R*sin(\tdeg)});

% --- Dashed vertical from P down to the x-axis ---
%\draw[dashed] (\px,\py) -- (\px,0);

% --- Right-angle mark at (px,0) ---
%\draw (\px-0.18,0) -- (\px-0.18,0.18) -- (\px,0.18);

% --- Center and P points ---
\fill (0,0) circle (2.5pt);
\fill (\px,\py) circle (2.5pt);
\node[below right] at (\px+0.05,\py-0.1) {$P(0,-1)$};

% --- Angle arc for theta, with label ---
\draw[->] (0.55,0) arc [start angle=0, end angle=\tdeg, radius=0.55];
\node[right] at (-1.3,0.4) {$270^\circ$};

% --- Leg labels: "1" along the radius, "x" along the base, "y" along the vertical ---
%\node[above left] at (0.5*\px,0.5*\py) {$1$};


% --- Unit labels on the axes ---
\node[above left] at (-0.12,\R) {$1$};
\node[below right] at (\R,-0.12) {$1$};
...
\fill (0,0) circle (2.5pt);
\fill (\px,\py) circle (2.5pt);
\node[below left] at (\px-0.05,\py-0.1) {$P(0,-1)$};

% --- Angle arc for theta, with label ---
\draw[->] (0.55,0) arc [start angle=0, end angle=\tdeg, radius=0.55];
\node[right] at (0.5,-0.5) {$\theta=-\pi$};
...
\fill (0,0) circle (2.5pt);
\fill (\px,\py) circle (2.5pt);
\node[right] at (\px+0.05,\py+0.1) {$P(x,y)$};

% --- Angle arc for theta, with label ---
\draw[->] (0.55,0) arc [start angle=0, end angle=\tdeg, radius=0.55];
\node[right] at (0.6,0.28) {$45^\circ$};

% --- Leg labels: "1" along the radius, "x" along the base, "y" along the vertical ---
\node[above left] at (0.5*\px,0.5*\py) {$1$};
\node[below] at (0.5*\px,-0.05) {$x$};
\node[right] at (\px+0.08,0.5*\py) {$y$};
...
\def\R{2}
\def\tdeg{30} % theta, in degrees
\def\ttdeg{150} % theta, in degrees
\pgfmathsetmacro{\px}{\R*cos(\tdeg)}
\pgfmathsetmacro{\py}{\R*sin(\tdeg)}

\pgfmathsetmacro{\pxx}{\R*cos(\ttdeg)}
\pgfmathsetmacro{\pyy}{\R*sin(\ttdeg)}
...
% --- Radius segment (thick, solid) from center to P ---
\draw[very thick] (0,0) -- (\pxx,\pyy);
\draw[very thick] (0,0) -- (2,0);
% --- Ray continuing past P (thin, with arrowhead) ---
\draw[-] (\pxx,\pyy) -- ({1.5*\R*cos(\ttdeg)},{1.5*\R*sin(\ttdeg)});


% --- Center and P points ---
\fill (0,0) circle (2.5pt);
\fill (\pxx,\pyy) circle (2.5pt);
\node[left] at (\pxx-0.05,\pyy-0.1) {$P(x,y)$};

% --- Angle arc for theta, with label ---
\draw[<->] (-0.87,0.5) arc [start angle=\ttdeg, end angle=180, radius=1];
\node[right] at (-1.9,0.3) {$\pi/6$};

\draw[->] (0.55,0) arc [start angle=0, end angle=\ttdeg, radius=0.55];
\node[right] at (0.3,0.7) {$\theta=5\pi/6$};
...
\fill (0,0) circle (2.5pt);
\fill (\pxx,\pyy) circle (2.5pt);
\node[left] at (\pxx-0.05,\pyy-0.1) {$P(-\sqrt{3}/2,1/2)$};
...
\draw[->] (0.55,0) arc [start angle=0, end angle=\ttdeg, radius=0.55];
\node[right] at (-0.5,0.7) {$\theta=5\pi/6$};

% --- Radius segment (thick, solid) from center to P ---
\draw[dashed] (0,0) -- (\px,\py);
\draw[dashed] (\pxx,\pyy) -- (\px,\py);


% --- Center and P points ---
\fill (0,0) circle (2.5pt);
\fill (\px,\py) circle (2.5pt);
\node[right] at (\px+0.05,\py-0.1) {$Q(\sqrt{3}/2,1/2)$};

% --- Angle arc for theta, with label ---
\draw[->] (1,0) arc [start angle=0, end angle=\tdeg, radius=1];
\node[right] at (1,0.28) {$\pi/6$};
...
\fill[blue!35](\px,\py)--(\px,0)--(0,0)--cycle;
% --- Radius segment (thick, solid) from center to P ---
\draw[-] (0,0) -- (\px,\py);
\draw[-] (0,0)--(\px,0);

% --- vertical from P down to the x-axis ---
\draw[-] (\px,\py) -- (\px,0);
...
\fill (0,0) circle (2.5pt);
\fill (\px,\py) circle (2.5pt);
\node[right] at (\px+0.05,\py+0.1) {$P=Q$};

% --- Angle arc for theta, with label ---
\draw[->] (0.55,0) arc [start angle=0, end angle=\tdeg, radius=0.55];
\node[right] at (0.6,0.28) {$\alpha$};
...
% --- Ray continuing past P (thin, with arrowhead) ---
\draw[-] (\px,-\py) -- ({1.5*\R*cos(\tdeg)},{-1.5*\R*sin(\tdeg)});
\draw[dashed] (\px,\py) -- (0,0);

% --- Center and P points ---
\fill (0,0) circle (2.5pt);
\fill (\px,\py) circle (2.5pt);
\node[right] at (\px+0.05,\py+0.1) {$Q$};
\fill (\px,-\py) circle (2.5pt);
\node[right] at (\px+0.05,-\py+0.1) {$P$};
...
\fill (0,0) circle (2.5pt);
\fill (\px,\py) circle (2.5pt);
\node[right] at (\px,\py+0.3) {$P$};
\fill (-\px,\py) circle (2.5pt);
\node[right] at (-\px+0.03,\py+0.3) {$Q$};

% --- Angle arc for theta, with label ---
\draw[->] (0.55,0) arc [start angle=0, end angle=60, radius=0.55];
\node[right] at (0.6,0.28) {$\alpha$};
\draw[<->] (-0.25,0.43) arc [start angle=120, end angle=180, radius=0.55];
\node[right] at (-1,0.28) {$\alpha$};
...
\draw[->] (0.55,0) arc [start angle=0, end angle=60, radius=0.55];
\node[right] at (0.6,0.28) {$\alpha$};
\draw[<->] (-0.55,0) arc [start angle=180, end angle=240, radius=0.55];
\node[right] at (-1,-0.4) {$\alpha$};
...
In light of Theorem \ref{refanglethm}, it pays to know the sine and cosine values for certain common Quadrant I angles\footnote{We'll provide an efficient method to recall these values in Exercise \ref{CosineSineFacts}.} as well as to keep in mind the signs of the coordinates of points in the given quadrants. 


\[ \begin{array}{|c|c||c|c|} \hline
 \theta (\mbox{degrees}) &  \theta (\mbox{radians}) & \cos(\theta) & \sin(\theta) \\ \hline
0^{\circ} & 0 & 1 & 0 \\ \hline
30^{\circ} & \frac{\pi}{6} & \frac{\sqrt{3}}{2} & \frac{1}{2} \\  \hline
45^{\circ} & \frac{\pi}{4} & \frac{\sqrt{2}}{2} & \frac{\sqrt{2}}{2} \\ \hline
60^{\circ} & \frac{\pi}{3} & \frac{1}{2} & \frac{\sqrt{3}}{2} \\  \hline
90^{\circ} & \frac{\pi}{2} & 0 & 1 \\  \hline
\end{array} \]
...
\def\panelwidth{6.5}
\def\R{2}
\def\tdeg{225} % theta, in degrees
\def\ttdeg{225} % theta, in degrees
\pgfmathsetmacro{\px}{\R*cos(\tdeg)}
\pgfmathsetmacro{\py}{\R*sin(\tdeg)}
...
% --- Angle arc for theta, with label ---
\draw[<->] (-1,0) arc [start angle=180, end angle=\ttdeg, radius=1];
\node[right] at (-1.7,-0.5) {$45^o$};

\draw[->] (0.55,0) arc [start angle=0, end angle=\ttdeg, radius=0.55];
\node[right] at (0.3,0.7) {$\theta=225^o$};
...
\def\panelwidth{6.5}
\def\R{2}
\def\tdeg{330} % theta, in degrees
\def\ttdeg{330} % theta, in degrees
\pgfmathsetmacro{\px}{\R*cos(\tdeg)}
\pgfmathsetmacro{\py}{\R*sin(\tdeg)}
...
% --- Angle arc for theta, with label ---
\draw[<->] (0.84,-0.5) arc [start angle=\ttdeg, end angle=360, radius=1];
\node[right] at (1.2,-0.4) {$\frac{\pi}{6}$};

\draw[->] (0.55,0) arc [start angle=0, end angle=\ttdeg, radius=0.55];
\node[right] at (-1.6,0.7) {$\theta=\frac{11\pi}{6}$};
...
% --- Angle arc for theta, with label ---
\draw[<->] (-1,0) arc [start angle=180, end angle=135, radius=1];
\node[right] at (-1.5,0.5) {$\frac{\pi}{4}$};

\draw[->] (0.55,0) arc [start angle=0, end angle=\ttdeg, radius=0.55];
\node[right] at (-1.6,-1) {$\theta=-\frac{5\pi}{4}$};
...
\def\panelwidth{6.5}
\def\R{2}
\def\tdeg{60} % theta, in degrees
\def\ttdeg{60} % theta, in degrees
\pgfmathsetmacro{\px}{\R*cos(\tdeg)}
\pgfmathsetmacro{\py}{\R*sin(\tdeg)}
\def\rstart{0.35} % starting radius (near the center)
\def\rend{0.75}   % ending radius (grows slightly over the sweep)
\def\amax{420}    % total sweep in degrees: 360 + 60 = 7*180/3
...
\node[right] at (1.3,1) {$\frac{\pi}{3}$};

\draw[<->] (1.5,0) arc [start angle=0, end angle=60, radius=1.5];
\node[right] at (-1.3,-1) {$\theta=\frac{7\pi}{3}$};
...
\draw[->,
      domain=0:\amax, samples=120, smooth, variable=\a]
    plot ({(\rstart + (\rend-\rstart)*\a/\amax)*cos(\a)},
          {(\rstart + (\rend-\rstart)*\a/\amax)*sin(\a)});
...
\begin{tikzpicture}[
    ...
    >=Latex,
    axis/.style={->,thin},
    radius/.style={densely dotted,thin},
    point/.style={circle,fill=black,inner sep=1.35pt},
    anglelabel/.style={font=\small},
    coordlabel/.style={font=\small}
]

% Radius of the unit circle in the drawing
\def\R{3.35}

% Axes
\draw[axis] (-4.15,0) -- (4.25,0)
    node[right] {$x$};

\draw[axis] (0,-3.75) -- (0,4.05)
    node[above] {$y$};
...
\node[anglelabel,above left] at (-2,0) {$\pi$};
\node[anglelabel,above right] at (2,0) {$0,\ 2\pi$};

\node[anglelabel] at (30:2) {$\frac{\pi}{6}$};
\node[anglelabel] at (45:2) {$\frac{\pi}{4}$};
\node[anglelabel] at (60:2) {$\frac{\pi}{3}$};

\node[anglelabel,right] at (90:2) {$\frac{\pi}{2}$};

\node[anglelabel] at (120:2) {$\frac{2\pi}{3}$};
\node[anglelabel] at (135:2) {$\frac{3\pi}{4}$};
\node[anglelabel] at (150:2) {$\frac{5\pi}{6}$};

\node[anglelabel] at (210:2) {$\frac{7\pi}{6}$};
\node[anglelabel] at (225:2) {$\frac{5\pi}{4}$};
\node[anglelabel] at (240:2) {$\frac{4\pi}{3}$};

\node[anglelabel,right] at (270:2) {$\frac{3\pi}{2}$};

\node[anglelabel] at (300:2) {$\frac{5\pi}{3}$};
\node[anglelabel] at (315:2) {$\frac{7\pi}{4}$};
\node[anglelabel] at (330:2) {$\frac{11\pi}{6}$};

% --------------------------------------------------------------------
% Coordinate labels on the axes
% --------------------------------------------------------------------

\node[coordlabel,right] at (\R+0.08,-0.18) {$(1,0)$};
\node[coordlabel,left]  at (-\R-0.08,-0.18) {$(-1,0)$};

\node[coordlabel,above right] at (0.05,\R+0.02) {$(0,1)$};
\node[coordlabel,below right] at (0.05,-\R-0.02) {$(0,-1)$};

% --------------------------------------------------------------------
% First quadrant coordinate labels
% --------------------------------------------------------------------

\node[coordlabel,right] at (30:\R+0.2)
    {$\left(\frac{\sqrt3}{2},\frac{1}{2}\right)$};

\node[coordlabel,above right] at (45:\R+0.1)
    {$\left(\frac{\sqrt2}{2},\frac{\sqrt2}{2}\right)$};

\node[coordlabel,above] at (60:\R+0.2)
    {$\left(\frac{1}{2},\frac{\sqrt3}{2}\right)$};

% --------------------------------------------------------------------
% Second quadrant coordinate labels
% --------------------------------------------------------------------

\node[coordlabel,above] at (120:\R+0.2)
    {$\left(-\frac{1}{2},\frac{\sqrt3}{2}\right)$};

\node[coordlabel,above left] at (135:\R+0.1)
    {$\left(-\frac{\sqrt2}{2},\frac{\sqrt2}{2}\right)$};

\node[coordlabel,left] at (150:\R+0.2)
    {$\left(-\frac{\sqrt3}{2},\frac{1}{2}\right)$};

% --------------------------------------------------------------------
% Third quadrant coordinate labels
% --------------------------------------------------------------------

\node[coordlabel,left] at (210:\R+0.2)
    {$\left(-\frac{\sqrt3}{2},-\frac{1}{2}\right)$};

\node[coordlabel,below left] at (225:\R+0.1)
    {$\left(-\frac{\sqrt2}{2},-\frac{\sqrt2}{2}\right)$};

\node[coordlabel,below] at (240:\R+0.2)
    {$\left(-\frac{1}{2},-\frac{\sqrt3}{2}\right)$};

% --------------------------------------------------------------------
% Fourth quadrant coordinate labels
% --------------------------------------------------------------------

\node[coordlabel,below] at (300:\R+0.2)
    {$\left(\frac{1}{2},-\frac{\sqrt3}{2}\right)$};

\node[coordlabel,below right] at (315:\R+0.1)
    {$\left(\frac{\sqrt2}{2},-\frac{\sqrt2}{2}\right)$};

\node[coordlabel,right] at (330:\R+0.2)
    {$\left(\frac{\sqrt3}{2},-\frac{1}{2}\right)$};
...
\begin{scope}
    ...
    % Point P on the circle
    \fill ({\R*cos(\tdeg)},{\R*sin(\tdeg)}) circle (2.5pt);
    \node[right] at ({\R*cos(\tdeg)+0.1},{\R*sin(\tdeg)+0.05}) {$\left(\frac{5}{13},y\right)$};
    ...
    % Angle arc for theta
    \draw[->] (0.7,0) arc [start angle=0, end angle=\tdeg, radius=0.7];
    \node at ({1.15*cos(\tdeg/2)},{1.15*sin(\tdeg/2)}) {$\alpha$};
\end{scope}
...
% --- Leg labels: "1" along the radius, "x" along the base, "y" along the vertical ---
\node[above left] at (0.5*\px,0.5*\py) {$1$};
\node[below] at (0.5*\px,-0.05) {$\frac{5}{13}$};
\node[right] at (\px+0.08,0.5*\py) {$\frac{12}{13}$};
\end{scope}
...
% --- Angle arc for theta, with label ---
\draw[->] (1.8,0) arc [start angle=0, end angle=\tdeg, radius=1.8];
\node[right] at (-1.8,1.8) {$\theta$};
\draw[->] (-0.55,0) arc [start angle=180, end angle=\tdeg, radius=0.55];
\node[right] at (-1,-0.28) {$\alpha$};
\draw[->] (0.55,0) arc [start angle=0, end angle=180, radius=0.55];
\node[right] at (-0.6,0.6) {$\pi$};
...
\fill (0,0) circle (2.5pt);
\fill (-\px,-\py) circle (2.5pt);
\node[above right] at (-\px+0.05,-\py+0.1) {$\left(\frac{5}{13},\frac{12}{13}\right)$};
\fill (\px,\py) circle (2.5pt);
\node[below left] at (\px-0.05,\py-0.1) {$\left(-\frac{5}{13},-\frac{12}{13}\right)$};
...
\draw[->] (1,0) arc [start angle=360, end angle=\tdeg, radius=1];
\node[right] at (1,-0.5) {$-\alpha$};

\draw[->] (0.55,0) arc [start angle=0, end angle=360, radius=0.55];
\node[right] at (-0.7,-0.6) {$\pi$};
...
\fill (\px,\py) circle (2.5pt);
\node[below right] at (\px-0.05,\py-0.1) {$\left(\frac{5}{13},-\frac{12}{13}\right)$};

% --- Angle arc for theta, with label ---
\draw[<->] (0.55,0) arc [start angle=0, end angle=60, radius=0.55];
\node[right] at (0.6,0.4) {$\alpha$};
\draw[<->] (0.55,0) arc [start angle=360, end angle=\tdeg, radius=0.55];
\node[right] at (0.6,-0.4) {$\alpha$};
\draw[->] (1.8,0) arc [start angle=0, end angle=\tdeg, radius=1.8];
\node[right] at (-1.8,1.8) {$\theta$};
...
\def\rstart{0.35} % starting radius (near the center)
\def\rend{0.75}   % ending radius (grows slightly over the sweep)
\def\amax{540}    % total sweep in degrees: 360 + 60 = 7*180/3
...
\draw[->] (-1,0) arc [start angle=180, end angle=\tdeg, radius=1];
\node[right] at (-1.7,0.5) {$-\alpha$};

\node[right] at (0.6,0.6) {$3\pi$};
...
\draw[<->] (-1,0) arc [start angle=180, end angle=\tdeg, radius=1];
\node[right] at (-1.5,0.6) {$\alpha$};

\draw[<->] (1,0) arc [start angle=0, end angle=60, radius=1];
\node[right] at (1,0.6) {$\alpha$};

\node[right] at (0.6,-0.6) {$\theta$};
...
% --- Spiral: radius grows linearly with angle from 0 to amax ---
\draw[->,
      domain=0:\tdeg+360, samples=120, smooth, variable=\a]
    plot ({(\rstart + (\rend-\rstart)*\a/\amax)*cos(\a)},
          {(\rstart + (\rend-\rstart)*\a/\amax)*sin(\a)});
...
\fill (0,0) circle (2.5pt);
\fill (\px,\py) circle (2.5pt);
\node[above left] at (\px+0.05,\py+0.1) {$\left(-\frac{5}{13},\frac{12}{13}\right)$};
...
% --- Angle arc for theta, with label ---
\draw[->] (1.8,0) arc [start angle=0, end angle=\tdeg, radius=1.8];
\node[right] at (1.4,1.6) {$\theta$};

\draw[->] (0,1) arc [start angle=90, end angle=\tdeg, radius=1];
\node[right] at (-0.8,1.1) {$\alpha$};

\draw[->] (0.55,0) arc [start angle=0, end angle=90, radius=0.55];
\node[right] at (0.6,0.6) {$\frac{\pi}{2}$};

\node[below left] at (\px-0.05,\py) {$Q(x,y)$};
...
\draw[dashed] (\py,-\px) -- (0,0);
\draw[dotted] (\py,-\px) -- (\py,0);
\draw[dotted] (\px,\py) -- (0,\py);

% --- Center and P points ---
\fill (0,0) circle (2.5pt);
\fill (\px,\py) circle (2.5pt);
\node[above left] at (\px+0.05,\py+0.1) {$\left(-\frac{12}{13},\frac{5}{13}\right)$};

\fill (\py,-\px) circle (2.5pt);
\node[right] at (\py+0.05,-\px+0.1) {$\left(\frac{5}{13},-\frac{12}{13}\right)$};

% --- Angle arc for theta, with label ---
\draw[<->] (0.55,0) arc [start angle=0, end angle=60, radius=0.55];
\node[right] at (0.5,0.4) {$\alpha$};

\draw[<->] (0,0.55) arc [start angle=90, end angle=\tdeg, radius=0.55];
\node[right] at (-0.6,0.7) {$\alpha$};
...
Our next example turns the tables and makes good use of the Unit Circle values as well as Theorem \ref{coterminalsamcosinesinethm} in a different way:  instead of giving information about the angle and asking for sine or cosine values, we are given sine or cosine values and asked to produce the corresponding angles.  In other words, we solve some rudimentary equations involving sine and cosine.\footnote{We will study equations in more detail in Section \ref{TrigonometricEquationsandInequalities}.}
...
\item  \label{cosineishalf} $\cos(\theta) = \frac{1}{2}$ \vphantom{$\frac{3}{2}$}

\item \label{sineisnegativehalf} $\sin(\alpha) = -\frac{1}{2}$ \vphantom{$\frac{3}{2}$}

\item  \label{cosineiszero} $\cos(\beta) = 0$. \vphantom{$\frac{3}{2}$}

\item  \label{sineisover1} $\sin(\gamma) = \frac{3}{2}$
...
\item If $\cos(\theta) = \frac{1}{2}$, then we know the terminal side of $\theta$, when plotted in standard position, intersects the Unit Circle at $x = \frac{1}{2}$.  This means $\theta$ is a Quadrant I or IV angle.  Since $\cos(\theta) = \frac{1}{2}$, we know that that the reference angle is $\frac{\pi}{3}$.
...
\node at (0.82,0.58) {$\frac{\pi}{3}$};

% --- Tick at x = 1/2 ---
\draw (1,-0.06) -- (1,0.06);
\node[below] at (1,-0.10) {$\frac{1}{2}$};
...
\pgfmathsetmacro{\px}{-\R*cos(\refang)}
\pgfmathsetmacro{\py}{-\R*sin(\refang)}
\pgfmathsetmacro{\qx}{ \R*cos(\refang)}
\pgfmathsetmacro{\qy}{-\R*sin(\refang)}
...
\node at (-1.3,-0.38) {$\frac{\pi}{6}$};

% --- Tick and label at y=-1/2 ---
\draw (-0.06,-1) -- (0.06,-1);
\node[right] at (0.10,-1) {$-\frac{1}{2}$};
...
% --- Positive x-axis and positive y-axis emphasized ---
\draw[very thick] (0,0) -- (\R,0);
...
% --- Positive x-axis and negative y-axis emphasized ---
\draw[very thick] (0,0) -- (\R,0);
\draw[very thick] (0,0) -- (0,-\R);
...
\item  We are asked to solve $\sin(\gamma) = \frac{3}{2}$.   Since sine values are $y$-coordinates on the Unit Circle, $\sin(\gamma)$ can't be any larger than $1$.  Hence, $\sin(t) = \frac{3}{2}$ has no solutions. 
...
One of the key items to take from Example \ref{solveforangle} is that, in general, solutions to trigonometric equations consist of infinitely many answers.  To get a feel for these answers, the reader is encouraged to follow our mantra from Section \ref{Sequences} - that is, `When in doubt, write it out!'   This is especially important when checking answers to the exercises.  
...
% Axes
\draw[->] (-\R-0.35,0) -- (\R+0.70,0)
    node[right] {$x$};

\draw[->] (0,-\R-0.55) -- (0,\R+0.55)
    node[above] {$y$};
...
% Extension of radius
\draw[-]
    (\px,\py)--({1.35*\R*cos(\ang)},{1.35*\R*sin(\ang)});
...
\draw[very thick, blue, ->]
    (2,0)
    arc[start angle=0,end angle=\ang,radius=\R];

% Angle
\draw[<->]
    (0.75,0)
    arc[start angle=0,end angle=\ang,radius=.75];

\node at (1.1,0.45) {$\theta=t$};


% Labels
\node[right] at (2,\py/2) {$t$};
...
% --- Concentric circles ---
\draw[gray!50] (0,0) circle (\smallR);
\draw[gray!50] (0,0) circle (\R);

% --- Axes ---
\draw[->] (-\R-0.55,0) -- (\R+0.65,0)
    node[right] {$x$};

\draw[->] (0,-\R-0.55) -- (0,\R+0.65)
    node[above] {$y$};

% --- Positive x-axis emphasized ---
\draw[very thick] (0,0) -- (\R+0.45,0);

% --- Radial line through P and Q ---
\draw[very thick] (0,0) -- (\qx,\qy);
...
\node at (0.70,0.45) {$\theta$};

% --- Labels for points ---
\node[right] at (\px+0.10,\py-0.02)
    {$P(x',y')$};

\node[right] at (\qx+0.10,\qy+0.04)
    {$Q(x,y)$};

% --- Radius labels ---
\node[right] at (0,\smallR+0.2) {$1$};
\node[right] at (0,\R+0.2) {$r$};

\node[below right] at (\smallR,0) {$1$};
...
\[\begin{array}{ccc} \cos(\theta)= \frac{x}{r}  = \frac{x}{\sqrt{x^2+y^2}} & \text{and} &  \sin(\theta) = \frac{y}{r} = \frac{y}{\sqrt{x^2+y^2}} \\ \end{array} \]
...
% --- Circle of radius sqrt(20), scaled so radius is 2.2 ---
\def\R{2.2}
...
\foreach \x in {-2,-1.5,-1,-0.5,0.5,1,1.5,2,2.5,3}
{
    \draw (\x,-0.07) -- (\x,0.07);
}
...
% --- Point Q(4,-2), scaled by factor 1/2 ---
\coordinate (Q) at (2,-1);

% --- Radius segment ---
\draw[very thick] (0,0) -- (Q);

% --- Continuation beyond Q ---
\draw (Q) -- (3,-1.5);
...
% --- Labels ---
\node[right] at (2.15,-0.95) {$Q(4,-2)$};
\node at (0.95,-1.05) {$r=2\sqrt{5}$};
...
\draw (-0.07,1.1) -- (0.07,1.1);
\draw (-0.07,2.2) -- (0.07,2.2);
\draw (-0.07,-1.1) -- (0.07,-1.1);

% --- Labels for 8 and 17 ---
\node[below] at (1.1,0) {$8$};
\node[below] at (2.2,0) {$17$};

\node[left] at (0,1.1) {$8$};
\node[left] at (0,2.2) {$17$};

% --- Point with y-coordinate 8 ---
...
\node at (0.67,0.80) {$\theta$};

% --- Labels ---
\node[left] at (-2.05,1.08) {$(x,8)$};
\node at (-0.95,1.18) {$r=17$};
...
\item  Recall the diagram below indicating the circles which are the parallels of latitude.\footnote{Diagram credit:  \href{https://commons.wikimedia.org/wiki/File:Latitude_(PSF).png}{\underline{Pearson Scott Foresman [Public domain], via Wikimedia Commons}}.}   
...
Assuming the Earth is a sphere of radius $3960$ miles, a cross-section through the poles produces a circle of radius $3960$ miles. Viewing the Equator as the $x$-axis, the value we seek is the $x$-coordinate of the point $Q(x,y)$ indicated in the figure below.
...
\draw[->] (-\R-0.55,0) -- (\R+1.15,0)
    node[right] {$x$};

\draw[->] (0,-\R-0.55) -- (0,\R+0.95)
    node[above] {$y$};

% Emphasize positive x-axis
...
% Continue the ray
\draw[very thick] (\px,\py) --
    ({1.42*\R*cos(\ang)},{1.42*\R*sin(\ang)});
...
\node at (1.3,0.20) {$41.628^\circ$};

%------------------------------------------------
% Labels
%------------------------------------------------

\node[right] at (\px+0.12,\py)
    {$Q(x,y)$};

\node[above right] at (0,\R)
    {$3960$};

\node[below right] at (\R,0)
    {$3960$};

\end{tikzpicture}
\end{image}

Using Theorem \ref{cosinesinecircle}, we get $x = 3960 \cos\left(41.628^{\circ}\right) \approx 2960$.  Hence, the radius of the Earth at North Latitude $41.628^{\circ}$ is approximately $2960$ miles. 
...
% --- Axes ---
\draw[->] (-\R-0.55,0) -- (\R+0.55,0)
    node[right] {$x$};

\draw[->] (0,-\R-0.55) -- (0,\R+0.55)
    node[above] {$y$};

% --- Thick positive x-axis ---
\draw[very thick] (0,0) -- (\R,0);

% --- Radius to Q ---
\draw[very thick] (0,0) -- (\px,\py);

% --- Circular arc of length r omega t ---
\draw[very thick]
    (\R,0)
    arc[
        start angle=0,
...
\node at (1,0.38) {$\theta=\omega t$};

% --- Coordinate label ---
\node[right] at (\px+0.10,\py+0.05)
    {$Q(x,y)=\bigl(r\cos(\omega t),r\sin(\omega t)\bigr)$};
...
\begin{explanation} From Example \ref{EarthRotationEx}, we take $r = 2960$ miles and and $\omega = \frac{\pi}{12 \, \text{hours}}$.  Hence, the equations of motion are $x =  r \cos(\omega t) = 2960 \cos\left(\frac{\pi}{12} t\right)$ and  $y =  r \sin(\omega t) = 2960 \sin\left(\frac{\pi}{12} t\right)$, where $x$ and $y$ are measured in miles and $t$ is measured in hours.\footnote{We will revisit this concept of associating points with times in Section \ref{ParametricEquations}.} 
\end{explanation}
\end{example}
