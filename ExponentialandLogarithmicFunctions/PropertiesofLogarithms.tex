...
\item  \textbf{Quotient Rule:} \index{quotient rule ! for logarithms} $g\left(\frac{u}{w} \right) = g(u) - g(w)$.  In other words, $\log_{b} \left( \frac{u}{w} \right) = \log_{b}(u) - \log_{b}(w)$
...
\item $\log_{2}\left(\frac{8}{x}\right)$ 

\item $\log_{0.1} \left(10 x^2 \right)$ 

\item  $\ln \left(\frac{3}{et}\right)^2$

\item  $\log \sqrt[3]{\frac{100 x^2}{yz^5}}$

\item \label{factorlogpropex} $\log_{117}\left(x^2 - 4\right)$
...
\log_{2}\left(\frac{8}{x}\right) & = &  \log_{2}(8) - \log_{2}(x) & \text{Quotient Rule} \\

& = &  3 - \log_{2}(x) & \text{Since $2^{3} = 8$} \\
...
\[ \begin{array}{rclr}
\log_{0.1} \left(10 x^2 \right) & = &  \log_{0.1} (10) +  \log_{0.1} \left(x^2 \right) & \text{Product Rule} \\
& = &  \log_{0.1} (10)+ 2 \log_{0.1} (x) & \text{Power Rule} \\
& = &  -1 + 2 \log_{0.1} (x) & \text{Since $(0.1)^{-1} = 10$} \\
 & = &  2 \log_{0.1} (x) - 1 & \\
 \end{array}\]
...
\ln \left(\frac{3}{et}\right)^2 & = & 2 \ln \left(\frac{3}{et}\right) & \text{Power Rule} \\
& = & 2 \left[ \ln(3) - \ln(et) \right] & \text{Quotient Rule} \\
& = & 2 \ln(3) - 2\ln(et) & \\
& = & 2 \ln(3) - 2\left[\ln(e) + \ln(t)\right] & \text{Product Rule} \\
& = & 2 \ln(3) - 2\ln(e) - 2 \ln(t) & \\
& = & 2\ln(3) - 2 - 2 \ln(t) & \text{Since $e^{1} = e$} \\
& = & - 2 \ln(t) + 2\ln(3) - 2 & \\
\end{array}\]
...
\log \sqrt[3]{\frac{100 x^2}{yz^5}} & = & \log \left(\frac{100 x^2}{yz^5}\right)^{1/3} & \\  
& = & \frac{1}{3} \log\left(\frac{100 x^2}{yz^5}\right) & \text{Power Rule} \\ 
& = & \frac{1}{3} \left[ \log\left(100x^2\right) - \log\left(yz^5\right) \right] & \text{Quotient Rule} \\ 
& = & \frac{1}{3}\log\left(100x^2\right) - \frac{1}{3}\log\left(yz^5\right) & \\
& = & \frac{1}{3}\left[ \log(100) + \log\left(x^2\right)\right] - \frac{1}{3} \left[ \log(y) + \log\left(z^5\right) \right] & \text{Product Rule} \\
& = & \frac{1}{3} \log(100) + \frac{1}{3} \log\left(x^2\right) - \frac{1}{3} \log(y) - \frac{1}{3} \log\left(z^5\right) \\
& = & \frac{1}{3} \log(100) + \frac{2}{3} \log(x) - \frac{1}{3} \log(y) - \frac{5}{3} \log(z) & \text{Power Rule} \\
& = & \frac{2}{3} + \frac{2}{3} \log(x) - \frac{1}{3} \log(y) - \frac{5}{3} \log(z) & \text{Since $10^2=100$} \\
...
\log_{117}\left(x^2-4\right) & = & \log_{117} \left[(x+2)(x-2)\right] & \text{Factor} \\
& = & \log_{117}(x+2) + \log_{117}(x-2) & \text{Product Rule} \\
\end{array}\]
...
\begin{explanation} Whereas in Example \ref{expandlogex} we read the properties in Theorem \ref{algproplogfcns} from left to right to expand logarithms, in this example we read them from right to left.
...
\log(x) + 2\log(y) - \log(z) & = & \log(x) + \log\left(y^2\right) - \log(z) & \text{Power Rule} \\   
 & = & \log\left(xy^2\right) - \log(z) & \text{Product Rule} \\  
 & = & \log\left( \frac{xy^2}{z}\right) & \text{Quotient Rule} \\
\end{array}\]
...
4\log_{2}(x) + 3 & = & \log_{2}\left(x^4\right) + 3  & \text{Power Rule} \\ 
& = & \log_{2}\left(x^4\right) + \log_{2}\left(2^3\right)& \text{Since $3 = \log_{2}\left(2^3\right)$} \\
& = & \log_{2}\left(x^4\right) + \log_{2}(8)& \\
& = & \log_{2}\left( 8x^4\right) & \text{Product Rule} \\ \end{array}\]
...
& = & \ln\left(t^{-1}\right) - \frac{1}{2} & \text{Power Rule} \\
& = & \ln\left(t^{-1}\right) - \ln\left(e^{1/2}\right)& \text{Since $\frac{1}{2} = \ln\left(e^{1/2}\right)$} \\
& = & \ln\left(t^{-1}\right) - \ln\left(\sqrt{e} \right)& \\   
& = & \ln\left(\frac{t^{-1}}{\sqrt{e}}\right) & \text{Quotient Rule} \\  
& = & \ln\left(\frac{1}{t\sqrt{e}}\right) &
\end{array}\]
...
\item  We apply the Change of Base formula with $a=3$ and $b=10$ to obtain $3^2 = 10^{2 \log(3)}$. The reader is invited to check this answer using your favorite graphing utility.

\item  Here, $a=2$ and $b = e$ so we have $2^{x} = e^{x \ln(2)}$.  Using desmos, we find the graphs of $f(x) = 2^x$ and $g(x) = e^{x \ln(2)}$ appear to overlap perfectly.

% https://www.desmos.com/calculator/kvyjuvno10
\begin{center}
\desmos{kvyjuvno10}{800}{600}
\end{center}
...
\item  We write $\ln(x) = \log_{e}(x) = \frac{\log(x)}{\log(e)}$.  We graph both $f(x) = \ln(x)$ and $g(x) = \frac{\log(x)}{\log(e)}$ using desmos and find both graphs appear to be identical.

% https://www.desmos.com/calculator/ymumlqyhxg
\begin{center}
\desmos{ymumlqyhxg}{800}{600}
\end{center}
...
\item $b^{x} = e^{x \ln(b)}$ for all real numbers $x$. 

\item  $\log_{b}(x) = \frac{\ln(x)}{\ln(b)}$ for all real numbers $x > 0$.
