\documentclass{ximera}

\begin{document}
	\author{Stitz-Zeager}
	\xmtitle{Exercises for Properties of Logarithms}{}

\mfpicnumber{1} \opengraphsfile{ExercisesforPropertiesofLogarithms} % mfpic settings added 


\label{ExercisesforPropertiesofLogarithms}

\begin{question}
In Exercises \ref{expandlogfirst} - \ref{expandloglast}, expand the given logarithm and simplify.  Assume when necessary that all quantities represent positive real numbers.

\begin{problem}\label{expandlogfirst}
$\ln(x^{3}y^{2})$

\begin{solution}
$3\ln(x) + 2\ln(y)$
\end{solution}
\end{problem}

\begin{problem}
$\log_{2}\left(\frac{128}{x^{2} + 4}\right)$

\begin{solution}
    $7 - \log_{2}(x^{2} + 4)$
\end{solution}
\end{problem}

\begin{problem}
$\log_{5}\left(\frac{z}{25}\right)^{3}$ 

\begin{solution}
$3\log_{5}(z) - 6$
\end{solution}
\end{problem}
 

\begin{problem}
    $\log(1.23 \times 10^{37})$

    \begin{solution}
        $\log(1.23) + 37$
    \end{solution}
    
\end{problem}

\begin{problem}
    $\ln\left(\frac{\sqrt{z}}{xy}\right)$

\begin{solution}
$\frac{1}{2}\ln(z) - \ln(x) - \ln(y)$
\end{solution}
\end{problem}

\begin{problem}
    $\log_{5} \left(x^2 - 25 \right)$

    \begin{solution}
        $\log_{5}(x-5) + \log_{5}(x+5)$
    \end{solution}
\end{problem}

\begin{problem}
    $\log_{\sqrt{2}} \left(4x^3\right)$

\begin{solution}
$3\log_{\sqrt{2}}(x) + 4$
\end{solution}
\end{problem}

\begin{problem}
    $\log_{\frac{1}{3}}(9x(y^{3} - 8))$

    \begin{solution}
      $-2 + \log_{\frac{1}{3}}(x) + \log_{\frac{1}{3}}(y - 2) + \log_{\frac{1}{3}}(y^{2} + 2y + 4)$
    \end{solution}
\end{problem}

\begin{problem}
    $\log\left(1000x^3y^5\right)$

\begin{solution}
$3 + 3\log(x) + 5 \log(y)$
\end{solution}
\end{problem}

\begin{problem}
    $\log_{3} \left(\frac{x^2}{81y^4}\right)$

    \begin{solution}
         $2\log_{3}(x) - 4 - 4\log_{3}(y)$
    \end{solution}
\end{problem}

\begin{problem}
    $\ln\left(\sqrt[4]{\frac{xy}{ez}}\right)$

\begin{solution}
$\frac{1}{4} \ln(x) + \frac{1}{4} \ln(y) - \frac{1}{4} - \frac{1}{4} \ln(z)$
\end{solution}
\end{problem}

\begin{problem}
    $\log_{6} \left(\frac{216}{x^3y}\right)^4$

    \begin{solution}
        $12-12\log_{6}(x) - 4\log_{6}(y)$
    \end{solution}
\end{problem}

\begin{problem}
    $\log\left(\frac{100x\sqrt{y}}{\sqrt[3]{10}}\right)$

\begin{solution}
$\frac{5}{3}+\log(x)+\frac{1}{2}\log(y)$
\end{solution}
\end{problem} 

\begin{problem}
    $\log_{\frac{1}{2}}\left(\frac{4\sqrt[3]{x^2}}{y\sqrt{z}}\right)$

    \begin{solution}
        $-2+\frac{2}{3}\log_{\frac{1}{2}}(x)-\log_{\frac{1}{2}}(y)-\frac{1}{2}\log_{\frac{1}{2}}(z)$
    \end{solution}
\end{problem}

\begin{problem}\label{expandloglast}
    $\ln \left(\frac{\sqrt[3]{x}}{10 \sqrt{yz}}\right)$

\begin{solution}
$\frac{1}{3} \ln(x) - \ln(10) - \frac{1}{2}\ln(y)-\frac{1}{2}\ln(z)$
\end{solution}
\end{problem}

\end{question}


\begin{question}
    
In Exercises \ref{combinelogfirst} - \ref{combineloglast}, use the properties of logarithms to write the expression as a single logarithm.

\begin{problem}\label{combinelogfirst}
     $4\ln(x) + 2\ln(y)$ 

     \begin{solution}
         $\ln(x^{4}y^{2})$
     \end{solution}
\end{problem}

\begin{problem}
    $\log_{2}(x) + \log_{2}(y) - \log_{2}(z)$

\begin{solution}
$\log_{2}\left(\frac{xy}{z}\right)$
\end{solution}
\end{problem}

\begin{problem}
    $\log_{3}(x) - 2 \log_{3}(y)$

    \begin{solution}
        $\log_{3} \left( \frac{x}{y^2} \right)$
    \end{solution}
\end{problem}

\begin{problem}
    $\frac{1}{2}\log_{3}(x) - 2\log_{3}(y) - \log_{3}(z)$

\begin{solution}
$\log_{3}\left(\frac{\sqrt{x}}{y^{2}z}\right)$
\end{solution}
\end{problem}

\begin{problem}
    $2 \ln(x) -3 \ln(y) - 4\ln(z)$

    \begin{solution}
        $\ln\left( \frac{x^2}{y^3z^4} \right)$
    \end{solution}
\end{problem}

\begin{problem}
    $\log(x) - \frac{1}{3} \log(z) + \frac{1}{2} \log(y)$

\begin{solution}
$\log\left(\frac{x \sqrt{y}}{\sqrt[3]{z}}  \right)$
\end{solution}
\end{problem}

\begin{problem}
     $-\frac{1}{3} \ln(x) - \frac{1}{3}\ln(y) + \frac{1}{3} \ln(z)$

     \begin{solution}
         $\ln\left(\sqrt[3]{\frac{z}{xy}}   \right)$
     \end{solution}
\end{problem}

\begin{problem}
    $\log_{5}(x) - 3$

\begin{solution}
$\log_{5}\left(\frac{x}{125}\right)$
\end{solution}
\end{problem}

\begin{problem}
    $3 - \log(x)$

    \begin{solution}
        $\log\left(\frac{1000}{x}\right)$
    \end{solution}
\end{problem}

\begin{problem}
     $\log_{7}(x) + \log_{7}(x - 3) - 2$

\begin{solution}
$\log_{7}\left(\frac{x(x - 3)}{49}\right)$
\end{solution}
\end{problem}

\begin{problem}
    $\ln(x) + \frac{1}{2}$ 

    \begin{solution}
        $\ln \left(x \sqrt{e} \right)$
    \end{solution}
\end{problem}

\begin{problem}
    $\log_{2}(x) + \log_{4}(x)$

\begin{solution}
$\log_{2}\left(x^{3/2}\right)$
\end{solution}
\end{problem}

\begin{problem}
    $\log_{2}(x) + \log_{4}(x-1)$

    \begin{solution}
        $\log_{2}\left(x \sqrt{x-1}\right)$
    \end{solution}
\end{problem}

\begin{problem}\label{combineloglast}
    $\log_{2}(x) + \log_{\frac{1}{2}}(x - 1)$ 

\begin{solution}
$\log_{2}\left(\frac{x}{x - 1}\right)$
\end{solution}
\end{problem}

\end{question}

\begin{question}
    
In Exercises \ref{changeofbasefirst} - \ref{changeofbaselast}, use the appropriate change of base formula to convert the given expression to an expression with the indicated base. 

\begin{problem}\label{changeofbasefirst}
    $7^{x - 1}$ to base $e$

    \begin{solution}
        $7^{x - 1} = e^{(x - 1)\ln(7)}$
    \end{solution}
\end{problem}

\begin{problem}
    $\log_{3}(x + 2)$ to base 10

\begin{solution}
$\log_{3}(x + 2) = \frac{\log(x + 2)}{\log(3)}$
\end{solution}
\end{problem}

\begin{problem}
    $\left(\frac{2}{3}\right)^{x}$ to base $e$

    \begin{solution}
        $\left(\frac{2}{3}\right)^{x} = e^{x\ln(\frac{2}{3})}$
    \end{solution}
\end{problem}

\begin{problem}\label{changeofbaselast}
    $\log(x^{2} + 1)$ to base $e$

\begin{solution}
$\log(x^{2} + 1) = \frac{\ln(x^{2} + 1)}{\ln(10)}$
\end{solution}
\end{problem}

\end{question}

\begin{question}
    
In Exercises \ref{changeofbaseapproxfirst} - \ref{changeofbaseapproxlast}, use the appropriate change of base formula to approximate the logarithm.

\begin{problem}\label{changeofbaseapproxfirst}
    $\log_{3}(12)$

    \begin{solution}
        $\log_{3}(12) \approx 2.26186$
    \end{solution}
\end{problem}

\begin{problem}
    $\log_{5}(80)$

\begin{solution}
$\log_{5}(80) \approx 2.72271$
\end{solution}
\end{problem}

\begin{problem}
    $\log_{6}(72)$

    \begin{solution}
        $\log_{6}(72) \approx 2.38685$
    \end{solution}
\end{problem}

\begin{problem}
    $\log_{4}\left(\frac{1}{10}\right)$

\begin{solution}
$\log_{4}\left(\frac{1}{10}\right) \approx -1.66096$
\end{solution}
\end{problem}

\begin{problem}
     $\log_{\frac{3}{5}}(1000)$ 

     \begin{solution}
          $\log_{\frac{3}{5}}(1000) \approx -13.52273$
     \end{solution}
\end{problem}

\begin{problem}\label{changeofbaseapproxlast}
    $\log_{\frac{2}{3}}(50)$

\begin{solution}
$\log_{\frac{2}{3}}(50) \approx -9.64824$
\end{solution}
\end{problem}

\end{question}

\begin{problem}\label{morethanoneforlogexercise}
    
In Example \ref{intrologex} number \ref{findformulaforlogexample} in Section \ref{LogarithmicFunctions}, we obtained the solution  $F(x) = \log_{2}(-x+4)-3$ as one formula for the given graph by making a simplifying assumption that $b = -1$.  This exercises explores if there are any other solutions for different choices of $b$.

\begin{enumerate}

\item  Show  $G(x) =\log_{2}(-2x+8) - 4$ also fits the data for the given graph.

\item  Use properties of logarithms to show $G(x) = \log_{2}(-2x+8) -4  = \log_{2}(-x+4)-3 = F(x)$.

\item  With help from your classmates, find solutions to Example \ref{intrologex} number \ref{findformulaforlogexample} in Section \ref{LogarithmicFunctions} by assuming $b = -4$ and $b = -8$.  In each case, use properties of logarithms to show the solutions reduce to $F(x) = \log_{2}(-x+4)-3$.

\item  Using properties of logarithms and the fact that the range of $\log_{2}(x)$ is all real numbers, show that any function of the form $f(x) = a \log_{2}(bx-h) + k$ where $a \neq 0$ can be rewritten as: 

\[ f(x) = a \left( \log_{2}(bx-h) +  \frac{k}{a}\right) = a ( \log_{2}(bx -h) + \log_{2}(p)) = a \log_{2}(p(bx-h)) = a \log_{2}(pbx - ph),\]

where $\frac{k}{a} = \log_{2}(p)$ for some positive real number $p$. Relabeling, we get every function of the form $f(x) = a \log_{2}(bx-h) + k$ with four parameters ($a$, $b$, $h$, and $k$) can be rewritten as $f(x) = a \log_{2}(Bx - H)$, a formula with just three parameters: $a$, $B$, and $H$.

\smallskip

Show \textit{every} solution to Example \ref{intrologex} number \ref{findformulaforlogexample} in Section \ref{LogarithmicFunctions} can be written in the form  $f(x) = \log_{2}\left( -\frac{1}{8}  x + \frac{1}{2} \right)$ and that, in particular,  $F(x) = \log_{2}(-x+4) -3 = \log_{2}\left( -\frac{1}{8}  x + \frac{1}{2} \right) = f(x)$.  Hence, there is really just one solution to Example \ref{intrologex} number \ref{findformulaforlogexample} in Section \ref{LogarithmicFunctions}.

\end{enumerate}   

\end{problem}

\begin{problem}\label{HendersonHasselbalch}
    
 \index{Henderson-Hasselbalch Equation} The Henderson-Hasselbalch Equation:  Suppose $HA$ represents a weak acid. Then we have a reversible chemical reaction 
\[HA \rightleftharpoons H^{+} + A^{-}.\]  
The acid disassociation constant, $K_{a}$, is given by 
\[K_{a} = \frac{[H^{+}][A^{-}]}{[HA]} = [H^{+}]\frac{[A^{-}]}{[HA]},\]
where the square brackets denote the concentrations just as they did in Exercise \ref{pHexercise} in Section \ref{LogarithmicFunctions}.  The symbol p$K_{a}$ is defined similarly to pH in that p$K_{a} = -\log(K_{a})$.  Using the definition of pH from Exercise \ref{pHexercise} and the properties of logarithms, derive the Henderson-Hasselbalch Equation:

\[\mbox{pH} = \mbox{p}K_{a} + \log\frac{[A^{-}]}{[HA]}\]
\end{problem}

\begin{problem}
    Compare and contrast the graphs of $y = \ln(x^{2})$ and $y = 2\ln(x)$.
\end{problem}

\begin{problem}
    Prove the Quotient Rule and Power Rule for Logarithms.
\end{problem}

\begin{problem}
    Give numerical examples to show that, in general,

\begin{multicols}{2}
\begin{enumerate}

\item $\log_{b}(x + y) \neq \log_{b}(x) + \log_{b}(y)$
\item $\log_{b}(x - y) \neq \log_{b}(x) - \log_{b}(y)$
\setcounter{HWindent}{\value{enumii}}
\end{enumerate}
\end{multicols}

\begin{enumerate}
\setcounter{enumii}{\value{HWindent}}

\item $\log_{b}\left(\frac{x}{y}\right) \neq \frac{\log_{b}(x)}{\log_{b}(y)}$

\end{enumerate}
\end{problem}

\begin{problem}
    Research the history of logarithms including the origin of the word `logarithm' itself.  Why is the abbreviation of natural log `ln' and not `nl'?
\end{problem}

\begin{problem}
    There is a scene in the movie `Apollo 13' in which several people at Mission Control use slide rules to verify a computation.  Was that scene accurate?  Look for other pop culture references to logarithms and slide rules.
\end{problem}

\begin{problem}
    
\begin{enumerate}
\setcounter{enumi}{\value{HW}}



\item\label{derivativeofln} \begin{enumerate}  \item\label{diffquotln}  Use properties of logarithm functions to show that if $f(x) = \ln(x)$, then \[ f'(x) = \lim_{h \rightarrow 0} \frac{f(x+h) - f(x)}{h} = \lim_{h \rightarrow 0} \frac{\ln\left( 1 + \frac{h}{x} \right)}{h} \]

\item\label{speciallnlimit}    Numerically and graphically investigate the limit: $\ds{\lim_{h \rightarrow 0}}$ $\frac{\ln\left( 1 + \frac{h}{x} \right)}{h}$ for various positive numbers $x$ to convince yourself that   $\ds{\lim_{h \rightarrow 0}}$ $\frac{\ln\left( 1 + \frac{h}{x} \right)}{h} = \frac{1}{x}$

\item  Use parts \ref{diffquotln} and \ref{speciallnlimit} to show to show that the derivative of $\ln(x)$ is \ldots  $\frac{1}{x}$.

\item  Write the equations of the tangent lines to $y = \ln(x) $ at the following points.   Check your answers graphically.  Compare your answers the tangent lines at the corresponding points in Exercise \ref{derivativeofex} in Section \ref{ExponentialFunctions}.



\begin{multicols}{3}
\begin{enumerate}

\item  $(1,0)$

\item  $(e, 1)$

\item $\left(e^{-1}, -1 \right)$

\end{enumerate}
\end{multicols}

\end{enumerate}

\setcounter{HW}{\value{enumi}}
\end{enumerate}

\end{problem}




\end{document}
