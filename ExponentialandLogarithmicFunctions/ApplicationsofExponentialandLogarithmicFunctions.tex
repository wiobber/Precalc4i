...
\begin{formula}[Simple Interest] \index{interest ! simple} \index{simple interest} \label{simpleinterest} 
...
\begin{formula}[Compounded Interest] \index{interest ! compound} \index{compound interest} \label{compoundinterest}   
...
If we take $P = 100$, $r = 0.05$, and $n = 4$, Formula \ref{compoundinterest} becomes $A(t) = 100\left(1+ \frac{0.05}{4}\right)^{4t}$ which reduces to $A(t) = 100(1.0125)^{4t}$.  To check this new formula against our previous calculations, we find $A\left(\frac{1}{4}\right) = 100(1.0125)^{4 \left(\frac{1}{4}\right)} = 101.25$, $A\left(\frac{1}{2}\right) \approx \$102.51$, $A\left(\frac{3}{4}\right) \approx \$103.79$, and $A(1) \approx \$105.08$.
...
\item  Find and interpret the average rate of change\footnote{See Definition \ref{arc} in Section \ref{LinearFunctions}.} of the amount in the account:
...
\item  Substituting $P = 2000$, $r = 0.07125$, and $n = 12$ (since interest is compounded \textit{monthly}) into Formula \ref{compoundinterest} yields $A(t) = 2000\left(1 + \frac{0.07125}{12}\right)^{12t}=2000 (1.0059375)^{12t}$.
...
Note that the interest rate quoted to us at the beginning of this problem is $7.125 \%$ per year.  The rate $7.362 \%$ is called the `\textit{effective}' interest rate which factors in the effect of the compounding on the growth of the investment.
...
Consider an investment of $\$ 1$ invested at $100 \%$ interest for $1$ year compounded $n$ times a year.  Formula \ref{compoundinterest} tells us that the amount of money in the account after $1$ year is $A = \left(1+\frac{1}{n}\right)^{n}$.  Below is a table of values relating $n$ and $A$.
...
Using the limit definition of $e$ along with some limit properties, we can derive a general formula for continuously compounded interest. 
 ...
 Consider the limit $\lim_{n \rightarrow \infty} P\left(1 + \frac{r}{n}\right)^{nt}$.   In order to use the limit definition of `$e$,' we need to make a substitution to make `$\left(1 + \frac{r}{n}\right)$' look like `$\left(1 + \frac{1}{\text{something}}\right)$.'  
...
\[ \begin{array}{rclr} 
\lim_{n \rightarrow \infty} P\left(1 + \frac{r}{n}\right)^{nt} & = &  \lim_{u \rightarrow \infty} P\left(1 + \frac{1}{u}\right)^{urt} & \\
& = & P \, \lim_{u \rightarrow \infty} \left(1 + \frac{1}{u}\right)^{urt} & \text{Scalar Multiple Rule} \\
& = & P \, \lim_{u \rightarrow \infty} \left[\left(1 + \frac{1}{u}\right)^{u}\right]^{rt} & \text{Properties of Exponents}\\
& = & P \, \left[  \lim_{u \rightarrow \infty} \left(1 + \frac{1}{u}\right)^{u} \right]^{rt} & \text{Real Number Powers}\\
...
Second, when we use the limit definition of `$e$' in the above argument, we used  $\lim_{u \rightarrow \infty} \left(1+\frac{1}{u}\right)^{u} = e$.  Like $n$, the variable $u$ is a discrete variable (not necessarily natural numbers, but discrete nonetheless).  Since both $n$ and $u$ are tending to infinity, this change in  dummy variable doesn't affect the limit.\footnote{Both $n$ and $u$ are taking discrete steps to infinity - one is doing so at just a slower pace.}
...
\begin{formula}[Continuously Compounded Interest] \index{interest ! compounded continuously} \index{continuously compounded interest} \label{contcompoundinterest} 
...
\begin{formula}[Uninhibited Growth] \index{growth model ! uninhibited} \index{uninhibited growth} \label{lawofuninhibitedgrowth} 

If a population increases according to The Law of Uninhibited Growth, the number of organisms at time $t$, $N(t)$  is given by the formula  \[N(t) = N_0e^{kt},\] where $N(0) = N_0$ (read `$N$ nought') is the initial number of organisms and $k>0$ is the constant of proportionality which satisfies the equation:

\[  \frac{dN}{dt} = k \, N(t),\]

where $\frac{dN}{dt}$ is the Leibniz notation\footnote{See the remark following Definition \ref{derivativefcndefn} on Section \ref{IntroductiontoDerivatives}.} notation for the derivative of $N$ with respect to $t$.
...
%% \ebm}
 ...
 It is worth taking some time to compare Formulas \ref{contcompoundinterest} and \ref{lawofuninhibitedgrowth}.  In  Formula \ref{contcompoundinterest}, we use $P$ to denote the initial investment;  in Formula \ref{lawofuninhibitedgrowth}, we use $N_0$ to denote the initial population.  In  Formula \ref{contcompoundinterest}, $r$ denotes the annual interest rate,  and so it shouldn't be too surprising that the $k$ in Formula \ref{lawofuninhibitedgrowth} corresponds to a growth rate as well.   While Formulas \ref{contcompoundinterest} and \ref{lawofuninhibitedgrowth} look entirely different, they both represent the same mathematical concept.
...
\begin{explanation} We begin with $N(t) = N_0e^{kt}$.  Since $N(t)$ is to give the number of cells \textit{in thousands}, we have $N_0 = 12$, so $N(t) = 12e^{kt}$. 
...
Whereas Formulas \ref{contcompoundinterest} and \ref{lawofuninhibitedgrowth} model the growth of quantities, we can use equations like them to describe the decline of quantities.  
...
This is precisely the same kind of hypothesis which drives The Law of Uninhibited Growth, and as such, the equation governing radioactive decay is hauntingly similar to Formula \ref{lawofuninhibitedgrowth} with the exception that the rate constant $k$ is negative.

 

%% \colorbox{ResultColor}{\bbm

\begin{formula}[Radioactive Decay] \index{radioactive decay} \label{radioactivedecay}

 The amount of a radioactive element  at time $t$, $A(t)$ is given by the formula  \[A(t) = A_0e^{kt},\] where $A(0) = A_0$ is the initial amount of the element and  $k<0$ is the constant of proportionality which satisfies the equation
...
\begin{example}  Iodine-131 is a commonly used radioactive isotope used to help detect how well the thyroid is functioning.  Suppose the decay of Iodine-131 follows the model given in Formula \ref{radioactivedecay}, and that the  half-life\footnote{The time it takes for half of the substance to decay.} of Iodine-131 is approximately $8$ days.  If $5$ grams of Iodine-131 is present initially, find a function which gives the amount of Iodine-131, $A$, in grams, $t$ days later.

 

\begin{explanation} Since we start with $5$ grams initially, Formula \ref{radioactivedecay} gives $A(t) = 5e^{kt}$.  
...
Hence, our final answer is $A(t) = 5 e^{-\frac{t\ln(2)}{8}} \approx 5 e^{-0.08664t}$. 
...
In that example we had a cup of coffee cooling from $160^{\circ}\text{F}$ to room temperature $70^{\circ}\text{F}$ according to the formula $T(t) = 70 + 90 e^{-0.1 t}$, where $t$ was measured in minutes.  In that situation, we knew the physical limit of the temperature of the coffee was room temperature,\footnote{The Second Law of Thermodynamics states that heat can spontaneously flow from a hotter object to a colder one, but not the other way around.  Thus, the coffee could not continue to release heat into the air so as to cool below room temperature.} and the differential equation which gives rise to our formula for $T(t)$ takes this into account.  
...
\begin{formula}[Newton's Law of Cooling (Warming)] \index{Newton's Law of Cooling} \label{newtonslawofcooling} 

The temperature of an object  at time $t$, $T(t)$ is given by the formula \[T(t) = T_{a} + \left(T_0 - T_{a}\right) e^{-kt},\] where $T(0) = T_0$ is the initial temperature of the object, $T_{a}$ is the ambient temperature\footnote{That is, the temperature of the surroundings.} and $k>0$ is the constant of proportionality which satisfies the equation

\[ \frac{dT}{dt} = k \, \left(T(t) - T_{a}\right)\]
...
If we re-examine the situation in Example \ref{exptempex} with $T_0 = 160$, $T_{a} = 70$, and $k = 0.1$, we get, according to Formula \ref{newtonslawofcooling}, $T(t) = 70 + (160 - 70)e^{-0.1t}$ which reduces to the original formula given in that example.  The rate constant $k = 0.1$ in this case indicates the coffee is cooling at a rate equal to $10 \%$ of the difference between the temperature of the coffee and its surroundings.  


Note in Formula \ref{newtonslawofcooling} that the constant $k$ is positive for both the cooling and warming scenarios.  What determines if the function $T(t)$ is increasing or decreasing is if $T_0$ (the initial temperature of the object) is greater than $T_{a}$ (the ambient temperature) or vice-versa, as we see in our next example.


\begin{example} \label{exptempex2} A roast initially at  $40^{\circ}\text{F}$ cooked in a $350^{\circ}\text{F}$ oven.  After $2$ hours, the temperature of the roast is $125^{\circ}\text{F}$.
...
\item  The roast is done when the internal temperature reaches $165^{\circ}\text{F}$.  When will the roast be done?
...
\item  The initial temperature of the roast is $40^{\circ}\text{F}$, so $T_0 = 40$.  The environment in which we are placing the roast is the $350^{\circ}\text{F}$ oven, so $T_{a} = 350$. Newton's Law of Warming gives $T(t) = 350 + (40-350)e^{-kt}$, or after some simplification,  $T(t) = 350 - 310e^{-kt}$.  

 

To determine $k$, we use the fact that after $2$ hours, the roast is  $125^{\circ}\text{F}$, which means $T(2) = 125$.  This gives rise to the equation $350 - 310e^{-2k} = 125$ which yields $k = -\frac{1}{2} \ln \left( \frac{45}{62}  \right) \approx 0.1602$.  The temperature function is \[T(t) = 350 - 310 e^{\frac{t}{2} \ln \left( \frac{45}{62}  \right)} \approx 350- 310 e^{-0.1602 t}.\]


\item  To find when the roast is done, we set $T(t) = 165$.  This gives $350- 310 e^{-0.1602 t} = 165$ whose solution is $t = -\frac{1}{0.1602} \ln \left( \frac{37}{62}  \right) \approx 3.22$.  Hence, the roast is done after roughly $3$ hours and $15$ minutes. 
...
\begin{formula}[Logistic Growth] \index{growth model ! logistic} \index{logistic growth} \label{logisticgrowth}   

If a population behaves according to the assumptions of logistic growth, the number of organisms at time $t$, $N(t)$  is given by  \[N(t) =\frac{L}{1 + Ce^{-kLt}},\] where $N(0) = N_0$ is the initial population,  $L$ is the limiting population,\footnote{That is, $\lim_{t \rightarrow \infty} N(t) = L$} and $C$ is a measure of how much room there is to grow given by \[C = \frac{L}{N_0} - 1.\] and $k > 0$ is the constant of proportionality which satisfies the equation

\[  \frac{dN}{dt} = k \, N(t) \left(L - N(t)\right)\]
...
\item  We could simply note that $N(t)$ is written in the form of Formula \ref{logisticgrowth}, and identify $L = 84$.  However, to see better \textit{why} the answer is $84$, we proceed analytically.  
...
Hence, $\lim_{t \rightarrow \infty} N(t) = 84$.     This means that as time goes by, the number of people who will have heard the rumor approaches $8400$. 
...
\item  Graphing $y=N(t)$ below using desmos, we see $y=84$ is the horizontal asymptote of the graph, confirming our answer to number  \ref{ebrumorex}, and the graph intersects the line $y=42$ at $t \approx 7.937 \approx \ln(2799) $, which confirms our answer to number \ref{whenrumorex}.

% https://www.desmos.com/calculator/htusfcbpla
\begin{center}
\desmos{htusfcbpla}{800}{600}
\end{center}
...
% https://www.desmos.com/calculator/xzsiww38b8
\begin{center}
\desmos{xzsiww38b8}{800}{600}
\end{center}
...
\item  We have $L = 7$ and $H=50$ and we need to find $N$.  Solving the equation $50 = 7 \log_{2}(N)$ gives $N = 2^{50/7} \approx 141.323$, so we would need $142$ different symbols to choose from.\footnote{Since there are only $94$ distinct ASCII keyboard characters, to achieve this strength, the number of characters in the password should be increased.} 
...
Thinking back Section \ref{QuadraticFunctions}, we have desmos run a quadratic regression.  We find $N(t) \approx 16.713 t^2 +149.68t -233.15$ with $R^2 = 0.992$, indicating a pretty good fit.  

% https://www.desmos.com/calculator/1ne8t58cum
\begin{center}
\desmos{1ne8t58cum}{800}{600}
\end{center}
...
% https://www.desmos.com/calculator/7ea8xb0jbu
\begin{center}
    \desmos{7ea8xb0jbu}{800}{600}
\end{center}
...
As  written,  Formula \ref{lawofuninhibitedgrowth} gives uninhibited growth as  $N(t) = N_0e^{kt}$. Here, for simplicity's sake, we relabel $N_0 = a$ and $e^{k} = b$ so that we are looking for parameters $a$ and $b$ so that $N(t) = a \cdot b^{t}$.
...
% https://www.desmos.com/calculator/dhzzdqvflh
\begin{center}
\desmos{dhzzdqvflh}{800}{600}
\end{center}
...
% https://www.desmos.com/calculator/ucern1m1hc
 
 \begin{center}
\desmos{ucern1m1hc}{800}{600}
 \end{center}
