...
\begin{document}
	\author{Stitz-Zeager}
	\xmtitle{Exercises for Applications of Exponential and Logarithmic Functions}{}
...
\begin{question}
    In Exercises \ref{radioactivefirst} - \ref{radioactivelast},  we list some radioactive isotopes and their associated half-lives.  Assume that each decays according to the formula $A(t) = A_{0}e^{kt}$ where $A_{0}$ is the initial amount of the material and $k$ is the decay constant. For each isotope:
...
\begin{problem}\label{radioactivefirst}
...
\begin{problem}\label{radioactivelast}
...
\begin{problem}
 Under optimal conditions, the growth of a certain strain of \textit{E. Coli} is modeled by the Law of Uninhibited Growth $N(t) = N_{0} e^{kt}$ 
 where $N_{0}$ is the initial number of bacteria and $t$ is the elapsed time, measured in minutes. From numerous experiments, it has been determined that the doubling time of this organism is 20 minutes. Suppose 1000 bacteria are present initially.
...
\begin{solution}
    We find $\ds{\lim_{t \rightarrow \infty} P(t) =  120}$.  As time goes by, the Sasquatch Population in Bigfoot County will approach 120.  Graphically,  $y = P(x)$ has a horizontal asymptote $y=120$.
\end{solution}
...
\begin{solution}
     The relative rate of change of $A(t)$ over $\left[t, t+\frac{1}{n} \right]$ is $\frac{r}{n}$ which is the annual percentage rate divided by the number of compoundings per year -- that is,  the percentage growth rate over one compounding.
\end{solution}
...
\begin{solution}
    From the graph, it appears that $\ds{\lim_{x \rightarrow 0^{+}} y(x) = \infty}$.  This is due to the presence of the $\ln(x)$ term in the function.  This means that Fritzy will never catch Chewbacca, which makes sense since Chewbacca has a head start and Fritzy only runs as fast as he does.
\end{solution}
...
\[i(t) = \left\{ \begin{array}{rcl} 2 - 2e^{-10t} & \mbox{if} & 0 \leq t < 30 \\ 
...
\begin{problem}\label{APLcatsrevisited}
...
\begin{problem}\label{LorenzExFollowUp}
...
\begin{solution}
$L(x) = 0.005154 x^{2.106}$.  $L(90) \approx 67.3$ meaning the bottom $90 \%$ of wage earners take home $67.3 \%$ of the total national income.  Said differently, according to this model, the top $10 \%$ of wage earners take home $32.7 \%$ of the total national income.
\end{solution}
...
\begin{problem}\label{PainesvillePopulationManyPoints}
...
\begin{solution}
 We get:  $y = 2895.06 (1.0147)^{x}$.  Graphing this along with our answer from Exercise \ref{PainesvillePopulationTwoPoint} over the interval $[0,60]$ shows that they are pretty close. From this model, $y(150) \approx 25840$ which once again overshoots the actual data value.
\end{solution}
...
\begin{solution}
$P(150) \approx 17914$, so this model predicts 17,914 people in Painesville in 2010, a more conservative number than was recorded in the 2010 census.  We have $\ds{\lim_{t \rightarrow \infty} P(t) =  18691}$,  so the limiting population of Painesville based on this model is 18,691 people.
\end{solution}
\end{itemize}
...
\begin{solution}
We get $\ds{\lim_{x \rightarrow \infty} y = 242526}$, so the limiting population of Lake County based on this model is 242,526 people.
\end{solution}
\end{itemize}
