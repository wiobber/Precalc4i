...
Of all of the functions we study in this text, exponential  functions are possibly the ones which impact everyday life the most. This section introduces us to these functions while the rest of the chapter will more thoroughly explore their properties. 
 ...
 We make a table of function values and plot the corresponding points on a graph furnisged by Desmos below.
...
On the flip side, as $x \rightarrow \infty$, we find $f(100) = 2^{100}$, $f(1000) = 2^{1000}$, and so on, thus $\ds{\lim_{x \rightarrow \infty} 2^{x} = \infty}$.  We can see this graphically by scrolling towards the right (and up!) on the Desmos graph above.  
...
Hence, one solution to the problem is $F(x) = -2^{x+3} + 4$.  To check our answer, we leave it to the reader verify $F(-1) = 0$, $F(0) = -4$, $\ds{\lim_{x \rightarrow -\infty} F(x) = 4}$, $\ds{\lim_{x \rightarrow \infty} F(x) = -\infty}$ analytically as well as checking these values graphically using the GeoGebra interactive above.
