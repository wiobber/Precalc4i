...
Using desmos, we graph $f(x) = 2^{3x}$ and $g(x) = 16^{1-x}$ and see that they intersect at $x \approx 0.571  \approx \frac{4}{7}$.
 % https://www.desmos.com/calculator/vgz48cail0
\begin{center}
    \desmos{vgz48cail0}{800}{600}
\end{center}
...
Using desmos, we find the graphs of  $f(x) = 2000$ and $g(x) =  1000 \cdot 3^{-0.1 x}$ intersect at $x \approx -6.309 \approx -\frac{10\ln(2)}{\ln(3)} $.

% https://www.desmos.com/calculator/pv2dugdpjn
\begin{center}
\desmos{pv2dugdpjn}{800}{600}
\end{center}
...
Using desmos, we see the graphs of $f(x) = 9 \cdot 3^{x}$ and $g(x) = 7^{2x}$ intersect at $x  \approx 0.787 \approx  \frac{2 \ln(3)}{2\ln(7) - \ln(3)}$.

% https://www.desmos.com/calculator/tvahxyxai6
\begin{center}
    \desmos{tvahxyxai6}{800}{600}
\end{center}
...
Putting these two steps together, we simplify $\ln\left(e^{-2t}\right) = \ln\left( \frac{1}{9} \right)$ to   $-2t = -\ln(9)$.  We arrive at our solution, $t = \frac{\ln(9)}{2}$ which simplifies to $t = \ln(3)$. (Can you explain why?)  
 ...
 To check, we see the graphs of  $f(x) = 75$ and $g(x) = \frac{100}{1 + 3e^{-2x}}$, intersect at $x \approx 1.099 \approx \ln(3)$.  As usual, our graph below is courtesy of desmos.

% https://www.desmos.com/calculator/kqpadz2v1h
\begin{center}
\desmos{kqpadz2v1h}{800}{600}
\end{center}
...
Desmos shows the graphgs of $f(x) = 25^{x}$ and $g(x) = 5^{x} + 6$ intersect at $x \approx 0.683 \approx \frac{\ln(3)}{\ln(5)} $.

% https://www.desmos.com/calculator/vxcygzddiq
\begin{center}
    \desmos{vxcygzddiq}{800}{600}
\end{center}
...
Graphing $f(x) = \frac{e^{x} - e^{-x}}{2}$ and $g(x) = 5$ using desmos, we find an intersection at $x \approx 2.312 \approx \ln\left(5 + \sqrt{26}\right)$.

% https://www.desmos.com/calculator/u0x0hbap10
\begin{center}
\desmos{u0x0hbap10}{800}{600}
\end{center}
...
We strongly encourage the reader to check the remaining equations analytically as well.\footnote{Who knows?  Maybe your instructor will offer up a bonus point (or two!)}
...
Setting $r(x) = 0$ gives $2^{x^2-3x} - 16 = 0$ or $2^{x^2-3x} = 16$.  Since $16 = 2^{4}$ we have $2^{x^2-3x} = 2^{4}$.   By the one-to-one property of exponential functions, $x^2 -3x = 4$ which gives $x=4$ and $x=-1$.  We construct a sign diagram below.
...
Graphing $r(x) = 2^{x^2-3x} - 16$ using desmos,  we find it is on or above the line $y=0$ (the $x$-axis) precisely on the intervals  $(-\infty, -1]$ and $ [4, \infty)$ which checks our answer.

% https://www.desmos.com/calculator/olb42m5yup
\begin{center}
\desmos{olb42m5yup}{800}{600}
\end{center}
...
\frac{e^{x}}{e^{x}-4} - 3 & \leq & 0 & \\

\frac{e^{x}}{e^{x}-4} - \frac{3 \left(e^{x}-4\right)}{e^{x}-4} & \leq & 0 & \text{Common denomintors.} \\

\frac{12 - 2e^{x}}{e^{x}-4} & \leq & 0 & \\
...
We determine the signs of $r\left(\ln(5)\right)$ and $r\left(\ln(7)\right)$ similarly.\footnote{We could, of course, use the calculator, but what fun would that be?}   
...
Using desmos, we find the graph of $f(x) = \frac{e^{x}}{e^{x}-4}$ is below the graph of $g(x) = 3$ on $(-\infty,\ln(4)) \cup (\ln(6), \infty)$, and they intersect at $x  \approx 1.792 \approx  \ln(6)$.

% https://www.desmos.com/calculator/7zustjcohd
\begin{center}
\desmos{7zustjcohd}{800}{600}
\end{center}
...
Since $\frac{1}{2} < 1$, $ \ln\left(\frac{1}{2}\right) < 0$ and we get $r(\ln\left(\frac{1}{2}\right))$ is $(+)$.  Proceeding similarly, we find $r\left(\ln\left(\frac{3}{2}\right)\right)  < 0$ and $r(\ln(3)) > 0$. 
...
\end{image}
 ...
 Our solution corresponds to $r(t) < 0$ which occurs on $(0 ,\ln(2))$.  

Desmos confirms that the graph of $f(t) = t e^{2t} $ is below the graph of $g(t) = 4t$ on $(0 ,\ln(2))$.\footnote{Note: $\ln(2) \approx 0.693$.}

% https://www.desmos.com/calculator/t1dr2ragzl
\begin{center}
\desmos{t1dr2ragzl}{800}{600}
\end{center}
...
\begin{example}  \label{coffeewarmerex} Recall from Example \ref{exptempex}  the temperature of coffee $T$ (in degrees Fahrenheit) $t$ minutes after it is served can be modeled by $T(t) = 70 + 90 e^{-0.1 t}$.  When will the coffee be warmer than $100^{\circ}\text{F}$?


\begin{explanation} We need to find when $T(t) > 100$, that is, we need to solve  $70 + 90 e^{-0.1 t} > 100$.  
...
Using desmos, we graph $y=T(t)$ along with with the horizontal line $y = 100$.   We see the graph of $T$ is above the horizontal line to the left of the intersection point, which we leave to the reader to show is $(10 \ln(3), 100)$.

% https://www.desmos.com/calculator/ijdrhxov9w
\begin{center}
\desmos{ijdrhxov9w}{800}{600}
\end{center}

Hence, the coffee is warmer than $100^{\circ}$F up to $10 \ln(3) \approx 11$ minutes after it is served, or, said differently, it takes approximately 11 minutes for the coffee to cool to under $100^{\circ}$F.  
...
\begin{example}  \label{expfracinverse} The function $f(x) = \frac{5e^{x}}{e^{x}+1}$ is one-to-one. 

\begin{enumerate}

\item Find a formula for $f^{-1}(x)$.

\item  Solve $\frac{5e^{x}}{e^{x}+1} = 4$.

\end{enumerate}


\begin{explanation} \begin{enumerate} \item We start by writing $y=f(x)$, and interchange the roles of $x$ and $y$.  To solve for $y$, we first clear denominators and then isolate the exponential function.

\[ \begin{array}{rclr}
y & = & \frac{5e^{x}}{e^{x}+1} & \\  
x & = & \frac{5e^{y}}{e^{y}+1} & \text{Switch $x$ and $y$} \\  
...
We can check this fairly quickly algebraically.  Using  $e^{\ln(4)} = 4$, we find $\frac{5e^{\ln(4)}}{e^{\ln(4)}+1}  = \frac{5(4)}{4+1} = \frac{20}{5} = 4$. 
\end{enumerate}
\end{explanation}
\end{example}
...
\node[below] at (\testL,-0.9) {$0$};
\node[below] at (\testR,-0.9) {$2$};
...
\node[above] at ({(\xmin+\crit)/2},0.15) {$\nearrow$};
\node[above] at ({(\crit+\xmax-0.6)/2},0.15) {$\searrow$};
\node[right] at (\xmax+0.15,0.55) {$f(x)$};
...
\node[above] at ({(\crit+\xmax-0.6)/2},0.10) {C. Up};
\node[right] at (\xmax+0.15,0.55) {$f(x)$};
...
\item Below is our check usingh desmos.

% https://www.desmos.com/calculator/ekrm7o7ecc
\begin{center}
    \desmos{ekrm7o7ecc}{800}{600}
\end{center}
\end{enumerate}
\end{explanation}
