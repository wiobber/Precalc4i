...
Domain of $g$: $(-1, \infty)$\\
 ...
 Asymptote: $x = -1$ \\
 % need graphs, just adding function details
\end{solution}
\end{problem}
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    % Dashed vertical asymptote at x = 2
    \addplot[dashed, domain=-3:4] coordinates {(2,-3) (2,4)};

    % Function: y = -log_3(x - 2)
    \addplot[
      thick,
      ->,
      >=stealth,
    ...
    \node[font=\small, anchor=west] at (axis cs:2.3333,1) {$(2.33,1)$};
    \node[font=\small, anchor=west] at (axis cs:3,0) {$(3,0)$};
    \node[font=\small, anchor=west] at (axis cs:5,-1) {$(5,-1)$};

    % Function label
    \node[font=\small, anchor=south east] at (axis cs:9,3) {$y = g(x) = -\log_{3}(x-2)$};
...
\begin{tikzpicture}
  \begin{axis}[
    ...
    % Dashed vertical asymptote at t = 10
    \addplot[dashed, domain=-8:5] coordinates {(10,-8) (10,5)};

    % Function: y = log_{1.25}(-t + 1) + 2
    % parametric form: x = (1 - 1.25^(t - 2)) * 10, y = t
    \addplot[
      thick,
      ->,
      >=stealth,
    ...
    \node[font=\small, anchor=west] at (axis cs:2,1) {$(2,1)$};
    \node[font=\small, anchor=west] at (axis cs:0,2) {$(0,2)$};
    \node[font=\small, anchor=east] at (axis cs:-2.5,3) {$(-2.5,3)$};

    % Function label (caption)
    \node[font=\small, anchor=south east] at (axis cs:8,4)
      {$y = g(t) = \log_{1.25}(-t+1) + 2$};
