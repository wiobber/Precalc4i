\documentclass{ximera}

\begin{document}
	\author{Stitz-Zeager}
	\xmtitle{Exercises for Logarithmic Equations and Inequalities}{}

\mfpicnumber{1} \opengraphsfile{ExercisesforLogarithmicEquationsandInequalities} % mfpic settings added 


\label{ExercisesforLogarithmicEquationsandInequalities}

\begin{question}

In Exercises \ref{solvelogeqexfirst} - \ref{solvelogeqexlast}, solve the equation analytically.

\begin{problem}\label{solvelogeqexfirst}
$\log(3x-1) = \log(4-x)$

$x = \answer{\frac{5}{4}}$
\end{problem}

\begin{problem}
$\log_{2}\left(x^{3}\right) = \log_{2}(x)$

$x = \answer{1}$
\end{problem}

\begin{problem}
$\ln\left(8-t^2\right)=\ln(2-t)$ 

$t=\answer{-2}$
\end{problem}

\begin{problem}
$\log_{5}\left(18-t^2\right) = \log_{5}(6-t)$

\begin{selectAll}
    \choice[correct]{$t=-3$}
    \choice{$t=-2$}
    \choice{$t=1$}
    \choice{$t=2$}
    \choice[correct]{$t=4$}
    \choice{$t=6$}
  \end{selectAll}
\end{problem}


\begin{problem}
$\log_{3}(7-2x) = 2$

$x=\answer{-1}$
\end{problem}

\begin{problem}
$\log_{\frac{1}{2}} (2x-1) = -3$

$x=\answer{\frac{9}{2}}$
\end{problem}

\begin{problem}
$\ln\left(t^2-99\right) = 0$

\begin{selectAll}
    \choice{$t=-33$}
    \choice[correct]{$t=-10$}
    \choice{$t=-9$}
    \choice{$t=9$}
    \choice[correct]{$t=10$}
    \choice{$t=33$}
  \end{selectAll}
\end{problem}


\begin{problem}
$\log(t^2-3t) = 1$

\begin{selectAll}
    \choice{$t=-3$}
    \choice[correct]{$t=-2$}
    \choice{$t=1$}
    \choice[correct]{$t=5$}
    \choice{$t=6$}
    \choice{$t=8$}
  \end{selectAll}
\end{problem}

\begin{problem}
$\log_{125} \left(\frac{3x-2}{2x+3}\right)=\frac{1}{3}$

$x = \answer{-\frac{17}{7}}$
\end{problem}

\begin{problem}\label{sixfourRichterequ}
$\log\left(\frac{x}{10^{-3}}\right) = 4.7$

\begin{solution}
    $x = 10^{1.7}$
\end{solution}
\end{problem}

\begin{problem}\label{sixfourpHequ}
$-\log(x) = 5.4$

\begin{solution}
$x = 10^{-5.4}$
\end{solution}
\end{problem}

\begin{problem}\label{sixfourdecibelequ}
$10\log\left(\frac{x}{10^{-12}}\right) = 150$ 

\begin{solution}
    $x = 10^{3}$
\end{solution}
\end{problem}

\begin{problem}
$6-3\log_{5}(2t)=0$

$t=\answer{\frac{25}{2}}$
\end{problem}

\begin{problem}
$3\ln(t)-2=1-\ln(t)$

\begin{solution}
    $t=e^{3/4}$
\end{solution}
\end{problem}

\begin{problem}
$\log_{3}(t - 4) + \log_{3}(t + 4) = 2$

\begin{selectAll}
    \choice{$t=-\sqrt{7}$}
    \choice{$t=-5$}
    \choice[correct]{$t=5$}
    \choice{$t=\sqrt{7}$}
  \end{selectAll}
\end{problem}

\begin{problem}
$\log_{5}(2t + 1) + \log_{5}(t + 2) = 1$

$t = \answer{\frac{1}{2}}$
\end{problem}

\begin{problem}
$\log_{169}(3x + 7) - \log_{169}(5x - 9) = \frac{1}{2}$

$x=\answer{2}$
\end{problem}

\begin{problem}
$\ln(x+1) - \ln(x) = 3$

\begin{solution}
    $x = \frac{1}{e^3-1}$
\end{solution}
\end{problem}

\begin{problem}
$2\log_{7}(t) = \log_{7}(2) + \log_{7}(t+12)$

$t=\answer{6}$
\end{problem}

\begin{problem}
$\log(t) - \log(2) = \log(t+8)  - \log(t+2)$

$t=\answer{4}$
\end{problem}

\begin{problem}
$\log_{3}(x) = \log_{\frac{1}{3}}(x) + 8$

$x = \answer{81}$
\end{problem}

\begin{problem}
$\ln(\ln(x)) = 3$

\begin{solution}
$x = e^{e^3}$
\end{solution}
\end{problem}

\begin{problem}
$\left(\log(t)\right)^2=2\log(t)+15$    

\begin{selectAll}
    \choice{$t=10^{-5}$}
    \choice[correct]{$t=10^{-3}$}
    \choice{$t=10^{3}$}
    \choice[correct]{$t=10^{5}$}
  \end{selectAll}

\end{problem}

\begin{problem}\label{solvelogeqexlast}
$\ln(t^{2}) = (\ln(t))^{2}$

\begin{solution}
$t = 1, \, x = e^{2}$
\end{solution}
\end{problem}
\end{question}



\begin{question}

In Exercises \ref{solvelogineqexfirst} - \ref{solvelogineqexlast}, solve the inequality analytically.

\begin{problem}\label{solvelogineqexfirst}
$\frac{1 - \ln(t)}{t^{2}} < 0$ 

\begin{solution}
$(e, \infty)$
\end{solution}
\end{problem} 

\begin{problem}
$t\ln(t) - t > 0$

\begin{solution}
$(e, \infty)$
\end{solution}
\end{problem} 

\begin{problem}\label{sixfourdecibelineq}
$10\log\left(\frac{x}{10^{-12}}\right) \geq 90$    

\begin{solution}
$(10^{-3}, \infty)$
\end{solution}
\end{problem} 

\begin{problem}\label{sixfourRichterineq}
$5.6 \leq \log\left(\frac{x}{10^{-3}}\right) \leq 7.1$ 

\begin{solution}
$\left[10^{2.6}, 10^{4.1}\right]$
\end{solution}
\end{problem}

\begin{problem}\label{sixfourpHineq} 
$2.3 < -\log(x) < 5.4$ 

\begin{solution}
$\left(10^{-5.4}, 10^{-2.3}\right)$
\end{solution}
\end{problem}

\begin{problem}\label{solvelogineqexlast}
$\ln(t^{2}) \leq (\ln(t))^{2}$

\begin{solution}
$(0, 1] \cup [e^{2}, \infty)$
\end{solution}
\end{problem}

\end{question}

\begin{question}
In Exercises \ref{logeqcalcexfirst} - \ref{logeqcalcexlast}, use a graphing utility to help you solve the equation or  inequality.

\begin{problem}\label{logeqcalcexfirst}
$\ln(t) = e^{-t}$

$t\approx \answer{1.3098}$
\end{problem}

\begin{problem}
$\ln(x) = \sqrt[4]{x}$

\begin{solution}
$x \approx 4.177, \, x \approx 5503.665$
\end{solution}
\end{problem} 
  
\begin{problem}
$\ln(t^{2} + 1) \geq 5$

\begin{solution}
$\approx (-\infty, -12.1414) \cup (12.1414, \infty)$
\end{solution}
\end{problem} 

\begin{problem}\label{logeqcalcexlast}
$\ln(-2x^{3} - x^{2} + 13x - 6) < 0$

\begin{solution}
$\approx (-3.0281, -3) \cup (0.5, 0.5991) \cup (1.9299, 2)$
\end{solution}
\end{problem}

\end{question}

\begin{question}
In Exercises \ref{domaincomplicatedlogfirst} - \ref{domaincomplicatedloglast},  find the domain of the function.

\begin{problem}\label{domaincomplicatedlogfirst}
$r(x) =   \frac{x}{1 - \ln(x)}$

\begin{solution}
$(-\infty, e) \cup (e, \infty)$
\end{solution}
\end{problem}    

\begin{problem}
$R(x) = \frac{x \ln(x)}{1 - \ln(x)}$ 

\begin{solution}
$(0,e) \cup (e, \infty)$
\end{solution}
\end{problem}     

\begin{problem}
$s(t) = \sqrt{2 - \log(t)}$

\begin{solution}
$(0, 100]$
\end{solution}
\end{problem}

\begin{problem}
$c(t) =  (2 \ln(t) -1)^{\frac{2}{3}}$

\begin{solution}
$(0, \infty)$
\end{solution}
\end{problem}
    

\begin{problem}
$\ell(t) = \ln( \ln(t))$


\begin{solution}
$(1, \infty)$
\end{solution}
\end{problem}
  
\begin{problem}\label{domaincomplicatedloglast}
$L(x) = \log\left( \frac{x \ln(x)}{1 - \ln(x)} \right)$

\begin{solution}
$(1,e)$
\end{solution}
\end{problem}
\end{question}

\begin{problem}\label{onetooneexpexercise}
Since $f(x) = e^{x}$ is a strictly increasing function, if $a < b$ then $e^{a} < e^{b}$.  Use this fact to solve the inequality $\ln(2x + 1) < 3$ without a sign diagram. Use this technique to solve the inequalities in Exercises \ref{sixfourdecibelineq} - \ref{sixfourpHineq}. (Compare this to Exercise  \ref{onetoonelogexercise} in Section \ref{ExponentialEquationsandInequalities}.)

\begin{solution}
$-\frac{1}{2} < x < \frac{e^{3} - 1}{2}$, so $\left( -\frac{1}{2}, \frac{e^{3} - 1}{2}\right)$
\end{solution}
\end{problem} 

\begin{problem}
Solve $\ln(3 - y) - \ln(y) = 2x + \ln(5)$ for $y$.

\begin{solution}
$y = \frac{3}{5e^{2x} + 1}$
\end{solution}
\end{problem}

\begin{problem}
In Example \ref{logfracinverse} we found the inverse of $f(x) = \frac{\log(x)}{1-\log(x)}$ to be $f^{-1}(x) = 10^{\frac{x}{x+1}}$.

\begin{enumerate}

\item Algebraically check our answer by verifying  $\left(f^{-1} \circ f\right)(x) = x$ for all $x$ in the domain of $f$ and that $\left(f \circ f^{-1}\right)(x) = x$ for all $x$ in the domain of $f^{-1}$.

\item Find the range of $f$ by finding the domain of $f^{-1}$.

\item Let $g(x) = \frac{x}{1 - x}$ and $h(x) = \log(x)$.  Show that $f = g \circ h$ and $(g \circ h)^{-1} = h^{-1} \circ g^{-1}$.\\


NOTE:  We know this is true in general by Exercise \ref{fcircginverse} in Section \ref{InverseFunctions}, but it's nice to see a specific example of the property.

\end{enumerate}

\end{problem}
 
\begin{problem}\label{inversehyptangent} 
Let $f(x) = \frac{1}{2}\ln\left(\frac{1 + x}{1 - x}\right)$.  Compute $f^{-1}(x)$ and find its domain and range.   

\begin{solution}
$f^{-1}(x) = \frac{e^{2x} - 1}{e^{2x} + 1} = \frac{e^{x} - e^{-x}}{e^{x} + e^{-x}}$. 

To see why we rewrite this in this form, see  Exercise \ref{andtheresthyperbolic} in Section \ref{ParametricEquations}.

 The domain of $f^{-1}$ is $(-\infty, \infty)$ and its range is the same as the domain of $f$, namely $(-1, 1)$.
\end{solution}
\end{problem}

\begin{problem}
Explain the equation in Exercise \ref{sixfourRichterequ} and the inequality in Exercise \ref{sixfourRichterineq} above in terms of the Richter scale for earthquake magnitude.  (See Exercise \ref{Richterexercise} in Section \ref{ExponentialFunctions}.)    
\end{problem}

\begin{problem}
Explain the equation in Exercise \ref{sixfourdecibelequ} and the inequality in Exercise \ref{sixfourdecibelineq} above in terms of sound intensity level as measured in decibels.  (See Exercise \ref{decibelexercise} in Section \ref{ExponentialFunctions}.)   
\end{problem}

\begin{problem}
Explain the equation in Exercise \ref{sixfourpHequ} and the inequality in Exercise \ref{sixfourpHineq} above in terms of the pH of a solution.  (See Exercise \ref{pHexercise} in Section \ref{ExponentialFunctions}.)
\end{problem}

\begin{problem}\label{powerloggrowthex} 
\begin{enumerate} \item\label{numericalinvestigationlimitlnxoverx} With the help of your classmates, numerically and graphically investigate $\ds{\lim_{x \rightarrow \infty}}$ $\frac{\ln(x)}{x^{p}}$ for various real number powers, $p > 0$.

 \item\label{numericalinvestigationlimitlnxtimesx} With the help of your classmates, numerically and graphically investigate $\ds{\lim_{x \rightarrow 0^{+}}}$ $x^{p} \, \ln(x) $ for various real number powers, $p > 0$.

\item  What do \ref{numericalinvestigationlimitlnxoverx} and \ref{numericalinvestigationlimitlnxtimesx} suggest about the relative growth rates of powers of $x$ and $\ln(x)$?

\end{enumerate}
\end{problem}

\begin{question}
In Exercises \ref{logcurvesketchfirst}  - \ref{logcurvesketchlast} a function $f$ along with its derivatives $f'$ and $f''$ are given.

\begin{itemize}

\item  Find the domain of $f$.

\item  Find the $x$- and $y$-intercepts of the graph of each function, if any.

\item  Use limits to determine the end behavior and behavior at the endpoints of the domain.

\item  Use $f'$ to determine the open intervals over which $f$ is increasing or decreasing.

\item Determine the local extrema, if any.

\item  Use $f''$ to determine the open intervals over which the graph of $f$  is concave up or concave down.

\item  Determine the inflection points of the graph, if any.

\end{itemize}

\begin{problem}\label{logcurvesketchfirst}

$f(x) = \ln(x) - \ln(5-x)$,  $f'(x) = \frac{1}{x} + \frac{1}{5-x}$, $f''(x) = \frac{1}{(5-x)^2} - \frac{1}{x^2}$

\begin{solution}
\begin{itemize}  \item  Domain:  $(0, 5)$.

\item $x$-intercept:  $\left( \frac{5}{2}, 0\right)$;  there is no $y$-intercept.

\item $\ds{\lim_{x \rightarrow 0^{+}} f(x) = -\infty}$, $\ds{\lim_{x \rightarrow 5^{-}} f(x) = \infty}$;  we have two vertical asymptotes:  $x = 0$ and $x = 5$.

\item  $f$ is always increasing: $(0, 5)$.

\item There are no local extrema.

\item  The graph of $f$ is concave up on $\left(0, \frac{5}{2}\right)$ and concave down on $\left( \frac{5}{2}, 5\right)$.

\item  The inflection point is $\left( \frac{5}{2}, 0\right)$.

\end{itemize}
\end{solution}
\end{problem}

\begin{problem}\label{logcurvesketchlast} 
$f(x) = \frac{\ln(x)}{x}$, $f'(x) = \frac{1 - \ln(x)}{x^2}$, $f''(x) = \frac{2 \ln(x) - 3}{x^3}$.

\begin{solution}
\begin{itemize}  \item  Domain:  $(0, \infty)$.

\item $x$-intercept:  $\left( 1 , 0\right)$;  there is no $y$-intercept.

\item $\ds{\lim_{x \rightarrow 0^{+}} f(x) = -\infty}$, $\ds{\lim_{x \rightarrow \infty} f(x) = 0}$;  we have a vertical and horizontal asymptote: $x = 0$ and  $y = 0$.

\item  $f$ is  increasing on  $(0, e)$ and decreasing on $(e, \infty)$.

\item  There is a local (absolute) max at $\left(e, \frac{1}{e}\right)$.

\item  The graph of $f$ is concave up on $\left(e^{\frac{3}{2}}, \infty \right)$ and concave down on $\left(0 ,  e^{\frac{3}{2}} \right)$.

\item  The inflection point is $\left( e^{\frac{3}{2}},  \frac{3}{2 e^{\frac{3}{2}}} \right)$.

\end{itemize}
\end{solution}
\end{problem}

\end{question}




\end{document}
