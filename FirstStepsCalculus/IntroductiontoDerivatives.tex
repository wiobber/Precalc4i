...
We begin this section by revisiting (again!) the notion of \textbf{average velocity} - a concept we first encountered in  Example \ref{ARCRocketExample} in Section \ref{LinearFunctions} and later revisited in Example \ref{averagevelocityrocketex} in Section \ref{IntroRational}. 
...
\item  Find, and simplify:  $\overline{v}(\Delta t)  =  \frac{s(15+ \Delta t) - s(15)}{\Delta t}$, for $\Delta t \neq 0$.

\item  Find and interpret $\overline{v}(-1)$.

\item  \label{limitavgvelocity2} Find and interpret $\lim_{\Delta t \rightarrow 0}  \overline{v}(\Delta t)$.
...
\overline{v}(\Delta t)& = &  \frac{s(15+ \Delta t) - s(15)}{\Delta t} & \\  
 & = &  \frac{(-5(\Delta t)^2-50 \Delta t + 375) - 375}{\Delta t} & \\  
 & = &  \frac{\Delta t (-5 \Delta t - 50)}{\Delta t} & \\  
 & = &  \frac{\cancel{\Delta} t (-5 \Delta t - 50)}{\cancel{\Delta t}} & \\  
...
\item  Since $\overline{v}(\Delta t) = -5 \Delta t - 50$, for all values of $\Delta t$ near $\Delta t = 0$ (excluding $\Delta t = 0$),  Theorem \ref{limitsagree} applies.  We get   $\lim_{\Delta t \rightarrow 0}  \overline{v}(\Delta t) = \lim_{\Delta t \rightarrow 0}  -5 \Delta t - 50 = -5(0) - 50 = -50$, where we have used the fact that the function $f(\Delta t) = -5 \Delta t - 50$ is continuous to evaluate the limit.
...
\draw[-, line width=3pt, blue](1, -7.5)--(-3,2.5);
...
\[ v\left(t_{0}\right) = \lim_{\Delta t \rightarrow 0}  \frac{\Delta [s(t)]}{\Delta t} = \lim_{\Delta t \rightarrow 0}  \frac{s(t_{0} + \Delta t) - s(t_{0})}{\Delta t},\]
...
\begin{tabular}{ccc}
 ...
 Using desmos, we can more smoothly\footnote{math pun!} visualize this process using the interactive below.  By dragging the highlighted point along the graph of $y=s(t)$ closer to $(15, 375)$, we can observe the secant lines approaching the tangent line.

\begin{center}
 % https://www.desmos.com/calculator/ht4ljf0bsv
 \desmos{ht4ljf0bsv}{800}{600}
 \end{center}
...
Recall  in Section \ref{AverageRateofChange} the concept of the average rate of change of a function over the interval $[a,b]$  is the slope between the two points $(a, f(a))$ and $(b, f(b))$ and is given by \[  \frac{\Delta[f(x)]}{\Delta x} =  \frac{f(b)-f(a)}{b-a}.\]
...
% Point labels
\node[anchor=south east] at (axis cs:1.29,0.29)
    {$\bigl(a,f(a)\bigr)$};

\node[anchor=south east] at (axis cs:4,3)
    {$\bigl(b,f(b)\bigr)$};

% Secant slope label
\node[align=center] at (axis cs:5.6,3.1)
    {$m_{\mathrm{sec}}
      = \frac{f(b)-f(a)}{b-a}$};
...
\[ f'(a) = \lim_{h \rightarrow 0}  \frac{f(a+h) - f(a)}{h}, \quad \text{provided this limit exists.} \]
...
\begin{explanation} \begin{enumerate} \item We first find $m_{\text{tan}} = f'(-2)$ using Definition \ref{derivativedefn} with $a = -2$:  $ f'(-2) = \lim_{h \rightarrow 0}  \frac{f(-2+h) - f(-2)}{h}$. 
...
\[ \begin{array}{rclr}
 ...
 \frac{f(-2+h)-f(-2)}{h} & = &  \frac{(-h^2+7h-11) -(-11)}{h} & \\ 
 & = &  \frac{-h^2+7h}{h} & \text{simplify}\\ 
 & = &   \frac{h(-h+7)}{h} & \text{factor} \\ 
 & = &   \frac{\cancel{h}(-h+7)}{\cancel{h}} &  \text{cancel} \\  & = & -h+7\\ \end{array} \]

Finally, we get $ f'(-2) = \lim_{h \rightarrow 0}  \frac{f(-2+h) - f(-2)}{h} = \lim_{h \rightarrow 0} (-h+7) = -(0) + 7 = 7$.
...
Graphing $y = 7x+3$ and $y = f(x)$ near $(-2,-11)$ reveals the local linearity we would expect.  Using the desmos graph below, we can zoom in on the point $(-2,-11)$ to our heart's content until the graph of $y = f(x)$ and the tangent line become indistinguishable.

% https://www.desmos.com/calculator/jhyxrweogw
\begin{center}
\desmos{jhyxrweogw}{800}{600}
 \end{center}
 
 \item Since $x$ represents the number of hours \textbf{after} Noon, $x = -2$ corresponds to $2$ hours \textbf{before} Noon, or 10 AM.  Since the units of $f(x)$ are degrees Celsius and the units of $x$ are hours, the units of $f'(-2)$ are degrees Celsius per hour.  Since $f'(-2)$ is positive, we know the slope is positive, so the temperature is increasing.  Putting all this together,   $f'(-2) = 7$ means that at 10 AM, the temperature is rising at a rate of $7$ degrees Celsius  per hour.
...
It is worth noting that if we set $h = \Delta x$, and consider the graph $y = f(x)$, we get:  \[ f'(x) = \lim_{h \rightarrow 0}  \frac{f(x+h) - f(x)}{h} = \lim_{\Delta x \rightarrow 0}  \frac{f(x+\Delta x) - f(x)}{\Delta x}  = \lim_{\Delta x \rightarrow 0}  \frac{\Delta [f(x)] }{\Delta x} =  \lim_{\Delta x \rightarrow 0}  \frac{\Delta y }{\Delta x} ,\] which is why sometimes\footnote{This is the so-called  \href{https://en.wikipedia.org/wiki/Gottfried_Wilhelm_Leibniz}{\underline{Leibniz}} notation \ldots}  the derivative is denoted $\frac{dy}{dx}$.
...
\item  To find  $f'(x) = \lim_{h \rightarrow 0}  \frac{f(x+h) - f(x)}{h}$ we  first find $f(x+h)$:
...
\[ \begin{array}{rclr}
 ...
 & = &  \frac{-2xh-h^2+3h}{h} & \text{simplify}\\ 
 & = &   \frac{h(-2x - h + 3)}{h} & \text{factor} \\ 
 & = &   \frac{\cancel{h}(-2x-h+3)}{\cancel{h}} &  \text{cancel} \\ 
 & = & -2x-h+3\\ \end{array} \]

Our last step is to take the limit:   $f'(x) = \lim_{h \rightarrow 0}  \frac{f(x+h) - f(x)}{h} = \lim_{h \rightarrow 0} (-2x-h+3)$.  Notice here that we have two variables, $x$ and $h$, in the limit.  Of these two variables, we are taking the limit on the $h$:  $h \rightarrow 0$.  As far as $h$ is concerned, $x$ may as well be just another constant like the `$3$'.  Hence, $f'(x) = \lim_{h \rightarrow 0} (-2x-h+3)=  -2x - 0 + 3 = -2x+3$.
...
% https://www.desmos.com/calculator/euggn9v5hr
\begin{center}
\desmos{euggn9v5hr}{800}{600}
 \end{center}
...
% https://www.desmos.com/calculator/cqjid2sy7u 
\begin{center}
\desmos{cqjid2sy7u}{800}{600}
 \end{center}
...
\item  $f'(3)$ %$ \frac{f(3+h)-f(3)}{h}$ 

\item  The equation of the tangent line to the graph $y = f(x)$ at $(3, f(3))$.  

Check your answer graphically.

\item  $f'(x)$ %$ \frac{f(x+h)-f(x)}{h}$.

\end{enumerate}

\item  For $g(x) =  \frac{3}{2x+1}$, find and simplify:\footnote{A review of Section \ref{AppRatExpEqus} may be in order for this problem.}

\begin{enumerate}

\item  $g'(0)$ %$ \frac{g(\Delta x)-g(0)}{\Delta x}$

\item  The equation of the tangent line to the graph $y = g(x)$ at $(0, g(0))$.  

Check your answer graphically.
 
\item  $g'(x)$ %$ \frac{g(x+\Delta x)-g(x)}{\Delta x}$.
...
\item $r'(9)$ %$ \frac{r(9+\Delta t)-r(9)}{\Delta t}$ 
...
\item  The equation of the tangent line to the graph $y = r(t)$ at $(9, r(9))$.
 ...
 \item $r'(t)$ %$ \frac{r(t+\Delta t)- r(t)}{\Delta t}$.
...
\item \begin{enumerate} \item To find $f'(3) =\lim_{h \rightarrow 0}$ $\frac{f(3+h)-f(3)}{h}$ we first simplify $f(3+h)$:
...
\[ \begin{array}{rclr}
 ...
 \frac{f(3+h)-f(3)}{h} & = &  \frac{(4 + 5h + h^2) -4}{h} & \\ 
                                & = &  \frac{5h+h^2}{h} & \\ 
                                & = &   \frac{h(5+h)}{h} & \text{factor} \\ 
                                & = &   \frac{\cancel{h}(5+h)}{\cancel{h}} & \text{cancel}\\ 
                                & = & 5+h \\ \end{array} \]
 Hence, $f'(3) = \lim_{h \rightarrow 0} (5+h) = 5+0 = 5$.
 
 
 \item  The equation of the tangent line at $x = 3$ is:  $y = f'(3)(x-3) + f(3) = 5(x-3) + 4$, or $y = 5x-11$. We check graphically below.

 % https://www.desmos.com/calculator/qrzvujthok
 \begin{center}
\desmos{qrzvujthok}{800}{600}
 \end{center}
...
\frac{f(x+h)-f(x)}{h} & = & \frac{\left(x^2+2xh+h^2-x-h-2 \right)-\left(x^{2}-x-2 \right)}{h} & \\  
& = & \frac{x^2+2xh+h^2-x-h-2-x^2+x+2}{h} & \\ 
& = &  \frac{2xh+h^2-h}{h} & \\ 
& = &  \frac{h \left(2x+h-1\right)}{h} & \text{factor} \\ 
& = & \frac{\cancel{h} \left(2x+h-1\right)}{\cancel{h}} & cancel \\ 
& = & 2x+h-1. \\ \end{array} \]

Hence, $f'(x) = \lim_{h \rightarrow 0} (2x+h-1) = 2x + 0 - 1$ so $f'(x) = 2x-1$. Note that using this formula, we get  $f'(3) = 2(3)-1 = 5$ which checks our answer above.


\end{enumerate}

\item  \begin{enumerate} \item Next we find  $g'(0) = \lim_{h \rightarrow 0} \frac{g(0+h)-g(0)}{h}  = \frac{g(h) - g(0)}{h}$.
...
Since $g(h) = \frac{3}{2 h + 1}$ and $g(0) = \frac{3}{2(0)+1} = 3$, our difference quotient contains a complex fraction.  Thinking ahead, we need to (eventually) be able to cancel the factor `$h$' from the denominator $\frac{g(h) - g(0)}{h}$, so we begin by simplifying the complex fraction and see where that takes us:
 ...
 \frac{g(0+h)-g(0)}{h} & = &  \frac{ \frac{3}{2h+1}-3}{h} & \\
& = &   \frac{ \frac{3}{2h+1}-3}{h} \cdot  \frac{(2h+1)}{(2h+1)} & \\
& = &   \frac{3-3(2h+1)}{h(2h+1)}  & \\
& = &   \frac{3 - 6 h - 3}{h(2h+1)}  & \\
& = &   \frac{-6h}{h(2h+1)}  & \\
& = &   \frac{-6\cancel{h}}{\cancel{h}(2h+1)}  & \text{cancel} \\
& = &   \frac{-6}{2h+1}.  & \\  \end{array} \]
...
% https://www.desmos.com/calculator/otiohnzjqy
 \begin{center}
\desmos{otiohnzjqy}{800}{600}
 \end{center}
...
\frac{g(x+h)-g(x)}{h} & = & \frac{ \frac{3}{2x+2h+1}- \frac{3}{2x+1}}{h} & \\
& = &   \frac{ \frac{3}{2x+2h+1}- \frac{3}{2x+1}}{h} \cdot  \frac{(2x+2h+1)(2x+1)}{(2x+2h+1)(2x+1)} & \\
& = &  \frac{3(2x+1)-3(2x+2h+1)}{h(2x+2h+1)(2x+1)}  & \\
& = &  \frac{6x+3-6x-6h-3}{h(2x+2h+1)(2x+1)}  & \\
& = &   \frac{-6h}{h(2x+2h+1)(2x+1)}  & \\
& = &  \frac{-6\cancel{h}}{\cancel{h}(2x+2h+1)(2x+1)} & \text{cancel} \\
& = &   \frac{-6}{(2x+2h+1)(2x+1)}.  & \\  \end{array} \]
...
\item \begin{enumerate} \item To find $r'(9) = \lim_{h \rightarrow 0}\frac{r(9+h) - r(9)}{h}$, we start with $r(9+h) = \sqrt{9+h}$ and $r(9) = \sqrt{9} = 3$. Hence  our difference quotient is:

\[  \frac{r(9+h)-r(9)}{h} =  \frac{\sqrt{9+h} - 3}{h}.\]
...
\[ \begin{array}{rcll}  
 ...
 \frac{r(9+h) - r(9)}{h} & = &  \frac{\sqrt{9+h} - 3}{h} & \\ 
	 ...
	 & = &  \frac{\left(\sqrt{9+h} - 3 \right)}{h} \cdot  \frac{\left(\sqrt{9+h} + 3\right)}{\left(\sqrt{9+h} + 3\right)} & \text{Multiply by the conjugate.} \\ 
										
    & = &   \frac{\left(\sqrt{9+h}\right)^2 -(3)^2}{h\left(\sqrt{9+h} + 3\right)} & \text{Difference of Squares.}\\ 
												
	& = &   \frac{(9+h) - 9}{h\left(\sqrt{9+h} + 3\right)} & \\ 
												
	& = &   \frac{h}{h\left(\sqrt{9+h} + 3\right)} & \\ 
												 
	& = &   \frac{\cancelto{1}{h}}{\cancel{h}\left(\sqrt{9+h} + 3\right)} & \text{cancel} \\ 
										
										
	 & = &   \frac{1}{\sqrt{9+h} +3} & \\ \end{array}\]	
...
% https://www.desmos.com/calculator/bvbce8baci
 \begin{center}
\desmos{bvbce8baci}{800}{600}
 \end{center}
...
\[ \begin{array}{rcll}  
 ...
 \frac{r(t+h) - r(t)}{h} & = &  \frac{\sqrt{t+h} - \sqrt{t}}{h} & \\  
												
	 & = &  \frac{\left(\sqrt{t+h} - \sqrt{t}\right)}{h} \cdot  \frac{\left(\sqrt{t+h} + \sqrt{t}\right)}{\left(\sqrt{t+h} + \sqrt{t}\right)} & \text{Multiply by the conjugate.} \\  
										
    & = &   \frac{\left(\sqrt{t+h}\right)^2 - \left(\sqrt{t}\right)^2}{h\left(\sqrt{t+h} + \sqrt{t}\right)} & \text{Difference of Squares.}\\  
												
	& = &   \frac{(t+h) - t}{h\left(\sqrt{t+h} + \sqrt{t}\right)} & \\  
												
	 & = &   \frac{h}{h\left(\sqrt{t+h} + \sqrt{t}\right)} & \\  

& = &   \frac{\cancelto{1}{h}}{\cancel{h}\left(\sqrt{t+h} + \sqrt{t}\right)} & \\  

& = &   \frac{1}{\sqrt{t+h} + \sqrt{t}} & \\ \end{array}\]	


We get \[r'(t) =  \lim_{h \rightarrow 0} \frac{1}{\sqrt{t+h} + \sqrt{t}} = \frac{1}{\sqrt{t+0} + \sqrt{t}}  = \frac{1}{2 \sqrt{t}}.\]   We check $r'(9) = \frac{1}{2\sqrt{9}} = \frac{1}{2(3)} = \frac{1}{6}$ which matches our answer above.  
