...
\item  Solve $ \frac{x^3-2x+1}{x-1} =  \frac{1}{2}x-1$.

\item  Solve $ \frac{x^3-2x+1}{x-1} \geq  \frac{1}{2}x-1$.
...
\left( \frac{x^3-2x+1}{x-1}\right) \cdot 2(x-1) & = & \left(  \frac{1}{2}x-1 \right) \cdot 2(x-1) & \\  
...
\[ \begin{array}{rclr}
 ...
 \frac{x^3-2x+1}{x-1} & \geq &  \frac{1}{2}x-1 & \\  

 \frac{x^3-2x+1}{x-1}  -  \frac{1}{2} x + 1& \geq & 0& \\  

 \frac{2 \left(x^3-2x+1\right)}{2(x-1)}  -  \frac{x(x-1)}{2(x-1)} +  \frac{2(x-1)}{2(x-1)}& \geq & 0&\text{get a common denominator} \\  

 \frac{2\left(x^3-2x+1\right)-x(x-1)+2(x-1)}{2(x-1)} & \geq & 0 & \\  

 \frac{2x^3-x^2-x}{2x-2} & \geq & 0 & \text{expand} \\
...
{\bf Solution.} We begin by rewriting the terms with negative exponents as fractions and gathering all nonzero terms to one side of the inequality:

\[ \begin{array}{rclr}

2t (3t-2)^{-1} &  \leq & 3t^2 (3t-2)^{-2} & \\  

 \frac{2t}{3t-2} & \leq &  \frac{3t^2}{(3t-2)^2} & \\  


 \frac{2t}{3t-2}  -  \frac{3t^2}{(3t-2)^2}  & \leq & 0 &  \\  

 \frac{2t(3t-2)}{(3t-2)^2}  -  \frac{3t^2}{(3t-2)^2}  & \leq & 0  & \text{get a common denominator} \\  

 \frac{2t(3t-2) - 3t^2}{(3t-2)^2}  & \leq & 0  &\\  

 \frac{3t^2-4t}{(3t-2)^2}  & \leq & 0  & \text{expand} \\  
...
\begin{tikzpicture}[scale=1]font=\Large
        \begin{axis}[
         ...
         ymin=-3, ymax=1.5,
          ...
          height=2in,
          xtick={-1, 1,3 },
          xticklabels={$0$, $\frac{2}{3}$, $\frac{4}{3}$},
          tick style={line width=1.2pt},
            axis x line=center,
            axis y line=none,
            x axis line style={{Latex[length=5pt,width=3pt]}-{Latex[length=5pt,width=3pt]}, line width=1pt},
...
\node[anchor=north, red] at (-1.5,1) {$(+)$};
\node[anchor=north, blue] at (0,1) {$(-)$};
\node[anchor=north, blue] at (2,1) {$(-)$};
...
\[ \begin{array}{rclr}
 ...
 \frac{80x+150}{x} - 100 & < & 0 & \\  

 \frac{80x + 150 - 100x}{x} & < & 0 & \text{common denominator} \\  
...
\begin{example}  \label{boxnotopfixedvolume} A box with a square base and no top is to be constructed so that it has a volume of $1000$ cubic centimeters.  Let $x$ denote the width of the box, in centimeters as seen in the interactive below.  Feel free to move the slider for the width, $x$, and observe the base of the box remains a square while the height changes to maintain a volume of $1000$ cubic centimeters.
...
h(x) & \geq & x & \\  
 ...
 \frac{1000}{x^2} - x & \geq & 0 \\  

 \frac{1000-x^3}{x^2} & \geq & 0 & \text{common denominator} \\ 
...
% https://www.geogebra.org/classic/hwtdhxss
\begin{center}
\geogebra{hwtdhxss}{800}{600}
\end{center}
...
\item Use a graphing utility to fit a curve of the form $P =  \frac{k}{V}$ to these data.  
...
\item  We plot the pairs $(V, P)$ and run a regression, the results of which are below.  To our amazement, the graphing utility, in this case Desmos, reports $k \approx 1406.9$ with  $R^2 \approx 1$.  This means the data are a very good fit to the model $P = \frac{k}{V}$, or $PV = k$, hence verifying Boyle's Law for this set of data.

% https://www.desmos.com/calculator/ezeh4v35lo
\begin{center} 
\desmos{ezeh4v35lo}{800}{600} 
\end{center}
