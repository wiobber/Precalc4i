...
\begin{question}  In the remarks following Example \ref{expandedsinusoidex1}, we rewrote the function $f(t)$ in number  \ref{beatsprodsumex} from Example \ref{prodtosumtoprod} using a sum to product identity as a product of two sinusoids, one acting as a variable amplitude creating a `wave envelope.'  Using this discussion as a guide,  rewrite each of the functions $f(t)$ in Exercises \ref{beatexfirst} - \ref{beatexlast} as a product of sinusoids. Identify the functions which create the `wave envelope.' Check your answer by graphing  the function along with the `wave-envelope' using a graphing utility.
