...
\item Here we're using the variable $t$ instead $\theta$ which usually corresponds to a real number variable instead of an angle.  As usual,  we associate real numbers $t$ with angles $\theta$ measuring $t$ radians,\footnote{See our discussion of the wrapping function in Section \ref{RadianMeasure} if you need a review of how we associate real numbers with angles in radian measure.} so the the Pythagorean Identity works equally well for all real numbers $t$ as it does for all angles $\theta$.
...
\item  $\sin^{2}(x) \cos^{3}(x) = \sin^{2}(x) \left(1 - \sin^{2}(x)\right) \cos(x)$ \vphantom{$\frac{\sec(x)}{1 - \tan(x)}$}

\item  $\frac{\sec(t)}{1 - \tan(t)} = \frac{1}{\cos(t) - \sin(t)}$
...
\item  $6\sec(x) \tan(x) = \frac{3}{1-\sin(x)} - \frac{3}{1 + \sin(x)}$

\item  \label{pythconjex} $\frac{\sin(\theta)}{1 - \cos(\theta)} = \frac{1 + \cos(\theta)}{\sin(\theta)}$
...
\item Starting with the right hand side of $\tan(\theta) = \sin(\theta) \sec(\theta)$, we use $\sec(\theta) = \frac{1}{\cos(\theta)}$ and find:  \[ \sin(\theta) \sec(\theta) = \sin(\theta) \frac{1}{\cos(\theta)} = \frac{\sin(\theta)}{\cos(\theta)} = \tan(\theta),\]
...
\[ \begin{array}{rcl} \frac{\sec(t)}{1 - \tan(t)} & = & \frac{ \frac{1}{\cos(t)}}{1 - \frac{\sin(t)}{\cos(t)}} = \frac{ \frac{1}{\cos(t)}}{1 - \frac{\sin(t)}{\cos(t)}} \cdot \frac{\cos(t)}{\cos(t)} \\  
 & = & \frac{\left( \frac{1}{\cos(t)} \right) ( \cos(t) )}{\left(1 - \frac{\sin(t)}{\cos(t)}\right)(\cos(t))} = \frac{1}{(1)(\cos(t)) - \left(\frac{\sin(t)}{\cos(t)}\right)(\cos(t))} \\  
                                                           & = & \frac{1}{\cos(t) - \sin(t)}, \end{array} \]
...
\frac{3}{1-\sin(x)} - \frac{3}{1 + \sin(x)} & = & \frac{3(1 + \sin(x))}{(1-\sin(x))(1 + \sin(x))} - \frac{3(1-\sin(x))}{(1 + \sin(x))(1-\sin(x))} \\  
                                                        & = & \frac{3 + 3\sin(x)}{1 - \sin^{2}(x)} - \frac{3 - 3\sin(x)}{1 - \sin^{2}(x)} \\  
                                                        & = & \frac{(3 + 3\sin(x)) - (3 - 3\sin(x))}{1 - \sin^{2}(x)} \\                                                          																																	
                                                        & = & \frac{6 \sin(x)}{1 - \sin^{2}(x)} \end{array} \]
...
\frac{3}{1-\sin(x)} - \frac{3}{1 + \sin(x)} & = & \frac{6 \sin(x)}{1 - \sin^{2}(x)}= \frac{6 \sin(x)}{\cos^{2}(x)} \\  
& = &  6 \left(\frac{1}{\cos(x)}\right)\left( \frac{\sin(x)}{\cos(x)}\right) = 6 \sec(x) \tan(x). \\ \end{array} \]
...
\frac{\sin(\theta)}{1 - \cos(\theta)} & = & \frac{\sin(\theta)}{(1 - \cos(\theta))} \cdot \frac{(1 + \cos(\theta))}{(1 + \cos(\theta))} = \frac{\sin(\theta)(1 + \cos(\theta))}{(1 - \cos(\theta))(1 + \cos(\theta))} \\  
& = & \frac{\sin(\theta)(1 + \cos(\theta))}{1 - \cos^{2}(\theta)} = \frac{\sin(\theta)(1 + \cos(\theta))}{\sin^{2}(\theta)} \\  
& = & \frac{\cancel{\sin(\theta)}(1 + \cos(\theta))}{\cancel{\sin(\theta)}\sin(\theta)} = \frac{1 + \cos(\theta)}{\sin(\theta)} \end{array} \]
